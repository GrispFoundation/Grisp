...
type
    ...
    private
        ...
        FIdentities: TObjectDictionary<string,string>;
        FIdentityKinds: TObjectDictionary<string,string>;
...
function GSDA_HMACSHA256Hex(const ASecret: TBytes; const AValue: string): string;
begin
    Result := THashSHA2.GetHMACAsString(TEncoding.UTF8.GetBytes(AValue), ASecret, SHA256);
end;
...
begin
    ...
    FIdentities := TObjectDictionary<string,string>.Create([doOwnsValues]);
    FIdentityKinds := TObjectDictionary<string,string>.Create([doOwnsValues]);
...
var
    ...
    LBest: TGSDARevisionSnapshot;
begin
    LBest := nil;
    for LRevision in FRevisions.Values do
        LBest := LRevision;
    Result := LBest;
end;
