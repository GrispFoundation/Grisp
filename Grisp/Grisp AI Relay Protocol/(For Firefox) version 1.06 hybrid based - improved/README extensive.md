...
The GARP component becomes part of the Firefox source tree and is compiled into the resulting custom Firefox build.

Do not attempt to install it through Firefox's Add-ons Manager.
...
The supplied `aistarter\moz.build` handles its local subdirectories.
...
The exact registration file depends on the Firefox revision.
...
are installed and that the actor is registered using the exact name:
...
# 24. Recommended final environment
...
16. Keep production GARP secrets outside the source tree.

17. The GARP/1.24 specification remains authoritative
