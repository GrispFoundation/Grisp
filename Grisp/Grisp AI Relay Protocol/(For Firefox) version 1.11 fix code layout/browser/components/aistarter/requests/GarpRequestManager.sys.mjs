...
export class GarpRequestManager {
    ...
    startPrompt(connection, sid, rid, prompt, options = {}) {
        ...
        this.runPrompt(r).catch(e => this.failCandidate(r, e?.code || GarpErrorCode.PROVIDER_ERROR, e?.message || String(e), e));
        return r;
    }
    ...
    scheduleTimer(r, deadline, epoch, code, message) {
        ...
        setTimeout(() => {
            if (!this.requests.has(r.requestId) || r.terminalCommitted || epoch !== r.timeoutEpoch)
                return;
    ...
    async emit(r, type, payload) {
        ...
        return s.enqueue(`event:${type}`, () => {
            if (r.terminalCommitted && !type.startsWith("GENERATION_"))
                return null;
            const e = this.service.commitSessionEvent(s, type, payload, { broadcast: false });
            for (const c of this.subscribers.get(r.requestId) || [])
                if (c.authenticated)
                    c.sendEnvelope(e, false);
            return e;
        });
    }
    ...
    async runPrompt(r) {
        ...
        r.lastMessageCount = Number(snap.last_message_count || 0);
        r.baselineMessageCount = r.lastMessageCount;
    ...
    async observe(r) {
        ...
        for (;;) {
            ...
            let x;
            try {
                x = await this.service.callChild(s, "ObserveGeneration", { provider: s.provider, previous_message_count: r.lastMessageCount, previous_message_text: r.baselineMessageText, prompt_request_id: r.requestId });
            }
            catch (e) {
            ...
            const count = Number(x.message_count || 0), text = String(x.text ?? ""), observedMessages = Array.isArray(x.messages) && x.messages.length ? x.messages : [{ message_id: x.message_id || null, text }], identified = count > r.lastMessageCount || text !== r.baselineMessageText || x.authoritative_generation_started === true;
            r.lastMessageCount = count;
            r.baselineMessageText = text;
            r.lastPageGeneration = s.pageGeneration;
            r.actorInstance = s.actorInstance;
            if (identified && !r.responseStarted) {
                r.responseStarted = true;
                r.state = PromptState.GENERATING;
            }
            for (const observed of observedMessages) {
                const mid = observed.message_id || r.messageOrder[0] || uuid(), observedText = String(observed.text ?? ""), m = r.ensureMessage(mid);
                if (!m.started) {
                    m.started = true;
            ...
            if (r.responseStarted && !observedMessages.some(m => String(m.text ?? "") !== "") && text)
                stable++;
            ...
            if (r.responseStarted && x.done && stable >= 2) {
    ...
    async resolveCancellationStep(r, s) {
        ...
        while (monotonicNow() < deadline && !r.terminalCommitted && epoch === r.cancelEpoch) {
            try {
                ...
                const state = await this.service.callChild(s, "InspectProviderState", { provider: s.provider, prompt_request_id: r.requestId, previous_message_count: r.lastMessageCount, previous_message_text: r.baselineMessageText, provider_operation_epoch: r.providerOperationEpoch });
    ...
    async commitTerminalCandidate(c) {
        ...
        if (c.kind === "COMPLETED") {
            ...
            await this.emit(r, "GENERATION_COMPLETED", payload);
        }
            ...
            await this.emit(r, "GENERATION_CANCELLED", payload);
        }
        else {
            ...
            const payload = messages.length === 1 ? { prompt_request_id: r.requestId, message_id: messages[0].message_id, revision: messages[0].revision, error: r.error, partial: r.partial, response: messages[0].response || null, page_generation: String(s.pageGeneration) } : { prompt_request_id: r.requestId, error: r.error, partial: r.partial, messages, page_generation: String(s.pageGeneration) };
            await this.emit(r, "GENERATION_FAILED", payload);
        }
        return true;
    }
