...
The package uses Firefox `FINAL_TARGET_FILES` destinations so the runtime resource paths match the source layout. Firefox documents `FINAL_TARGET_FILES` as supporting destination subdirectories.

The resulting runtime paths are:

```text
resource:///modules/aistarter/AIAutomation.sys.mjs
resource:///modules/aistarter/garp/GarpConnection.sys.mjs
resource:///modules/aistarter/providers/ProviderRegistry.sys.mjs
resource:///modules/aistarter/requests/GarpRequestManager.sys.mjs
resource:///modules/aistarter/sessions/AIAutomationSession.sys.mjs

resource:///actors/AIAutomationParent.sys.mjs
resource:///actors/AIAutomationChild.sys.mjs
```

Do not replace these paths with source-tree URLs. The build copies the modules into the Firefox application output.
...
The main GARP module and its internal submodules are installed under:
