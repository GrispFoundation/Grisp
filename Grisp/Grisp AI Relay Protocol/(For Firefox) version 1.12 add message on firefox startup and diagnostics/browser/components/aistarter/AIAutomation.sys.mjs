        ...
        this.initializeSecurityState()
            .then(() => {
                ...
                try {
                    ...
                    dump(
                        `GARP/1.24: listening on 127.0.0.1:${PORT}\n`
                    );

                    this.pruneTimer = setInterval(
                        () =>
                            this.requestManager.prune(
                                Number(
                                    this.capabilities.limits.response_retention_ms
                                )
                            ),
                        60000
                    );
                    ...
                    Cu.reportError(
                        `GARP/1.24 startup failed: ${error}`
                    );
            ...
            .catch(error => {
                ...
                Cu.reportError(
                    `GARP/1.24 security initialization failed: ${error}`
                );
