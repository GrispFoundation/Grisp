...
The GARP component has exactly one component build file:

```text
R:\firefox\browser\components\aistarter\moz.build
```

Do not flatten the directory structure, and do not add subordinate `moz.build` files under its child directories.

## 13. Verify GARP module packaging

The supplied `aistarter\moz.build` uses the proven `EXTRA_JS_MODULES` namespace technique. It does **not** use `FINAL_TARGET` or `FINAL_TARGET_FILES`.
...
The actor files are ordinary modules under the same `aistarter` namespace and therefore use:
...
If `AIAutomation.sys.mjs` is not present under `dist\bin\modules\aistarter`, fix the `EXTRA_JS_MODULES` entries in `browser\components\aistarter\moz.build` before investigating BrowserGlue or actor registration.

## 14. Register the component directory
...
The top-level `aistarter\moz.build` packages all GARP modules. Do not add `DIRS` entries for `actors`, `diagnostics`, `garp`, `providers`, `requests`, or `sessions`.

## 15. Register the JSWindowActor

Register the actor in the `JSWINDOWACTORS` table used by the checkout. Use the existing registry; do not create a second registry just for GARP.
...
The startup integration must import:
...
The Firefox-side GARP implementation includes runtime-switchable diagnostics for investigating protocol, session, provider, DOM, and generation problems without attaching a native debugger.
...
Level 1 records the major GARP lifecycle, including connections, frames, authentication, session creation, state transitions, prompt acceptance, provider submission, generation events, errors, and cleanup.

### Enable detailed tracing

For the full diagnostic trace:
...
Level 2 additionally records child-actor calls and results, page-generation fencing, provider selection, DOM selector searches, input discovery, text insertion and verification, send-button discovery, submission verification, generation observation, continuation handling, and individual observation iterations.

### Diagnostics output

The diagnostics use Firefox `dump()` and therefore appear in the Firefox process console. `GARP_DEBUG` is an environment variable; it is not a special `./mach run --debug` option.

The trace is designed to expose the complete path:
...
GARP connection
  ...
  -> GENERATION_DELTA / GENERATION_PROGRESS
  -> GENERATION_COMPLETED / GENERATION_FAILED / GENERATION_CANCELLED
```

Diagnostic messages carry timestamps and a diagnostic level. Asynchronous operations also identify their logical operation where available, making interleaved callbacks easier to follow.

Large strings and nested values are truncated to prevent console flooding. Authentication secrets are never logged. Prompt text is represented by lengths rather than the full prompt contents.
...
Use `GARP_DEBUG=2` when the lifecycle trace shows where the operation stops but more detail is needed.

For a prompt-generation failure, retain the Firefox console output from `PROMPT` through the eventual timeout or terminal generation event. The most useful markers are:
...
Then check the existing `JSWINDOWACTORS` registration table and verify both actor modules exist under:
