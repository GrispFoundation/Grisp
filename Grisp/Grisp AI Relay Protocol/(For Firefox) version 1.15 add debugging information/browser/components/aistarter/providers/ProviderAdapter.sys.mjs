...
export class ProviderAdapter {
    ...
    async submitVerified(input, doc, options = {}) {
        ...
        if (options.simulateEnter) {
            ...
            garpDebug(2, "send action lookup", {
                provider: this.name,
                found: !!b,
                tag: b?.tagName || null,
                id: b?.id || null,
                classes: String(b?.className || ""),
                text: String(b?.textContent || "").slice(0, 120),
            }, options.debugLevel);
            if (!b)
        ...
        while (Date.now() < deadline) {
            if (this.isStreaming(doc) || this.responseNodes(doc).length > beforeCount)
                {
                    garpDebug(1, "submission verified by provider activity", {
                        provider: this.name,
                        strategy,
                        after_message_count: this.responseNodes(doc).length,
            ...
            if (after !== before)
                {
                    garpDebug(1, "submission verified by input change", {
                        provider: this.name,
                        strategy,
        ...
        garpDebug(1, "submission verification FAILED", {
            provider: this.name,
            strategy,
            final_message_count: this.responseNodes(doc).length,
            final_input_length: "value" in input ? String(input.value || "").length : String(input.textContent || "").length,
        }, options.debugLevel);
