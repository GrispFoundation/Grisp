...
import { garpDebug, garpError, getGarpDebugLevel } from "./GarpDebug.sys.mjs";
const PORT = 9999;
function readSecret() {
...
export class AIAutomationService {
    ...
    init() {
        ...
        garpDebug(1, "debug tracing enabled", {
            level: getGarpDebugLevel(),
        });
    ...
    onSocketAccepted(_s, t) {
        try {
            const connection = new GarpConnection(t, this);
            this.connections.add(connection);
            garpDebug(1, "connection accepted", {
    ...
    removeConnection(c) {
        garpDebug(1, "connection remove", {
            connections_before: this.connections.size,
    ...
    async handleHello(c, d) {
        garpDebug(1, "HELLO received", {
            request_id: d.request_id,
            client_name: d.message.payload.client_name,
            client_version: d.message.payload.client_version,
            offered_versions: d.message.payload.versions,
            offered_wire_versions: d.message.payload.wire_versions,
            offered_features: d.message.payload.features,
        });
        ...
        garpDebug(1, "HELLO challenge prepared", {
            request_id: d.request_id,
            negotiated_features: c.negotiatedFeatures,
        });
    ...
    async handleHelloAuth(c, d) {
        garpDebug(1, "HELLO_AUTH received", {
            request_id: d.request_id,
        });
        ...
        garpDebug(1, "authentication succeeded", {
            request_id: d.request_id,
            negotiated_features: c.negotiatedFeatures,
        });
    ...
    async handleCreateSession(c, d) {
        garpDebug(1, "CREATE_SESSION begin", {
            request_id: d.request_id,
            provider: d.message.payload.provider,
            tab_id: d.message.payload.tab_id ?? null,
        });
        ...
        this.prepareSession(s).catch(e => {
            garpError("session preparation rejected", {
            ...
            this.failSessionPreparation(s, e);
        });
    }
    ...
    async handlePrompt(c, d) {
        garpDebug(1, "PROMPT command entered", {
            request_id: d.request_id,
            session_id: d.session_id,
            prompt_length: String(d.message.payload.prompt ?? "").length,
            options: d.message.payload.options ?? {},
        });
        const s = this.sessionManager.get(d.session_id);
        if (s.ownerConnection && s.ownerConnection !== c)
            throw new GarpError(GarpErrorCode.INVALID_MESSAGE_STATE, "Connection does not own session");
        const r = this.requestManager.startPrompt(c, s.sessionId, d.request_id, d.message.payload.prompt, d.message.payload.options);
        garpDebug(1, "PROMPT accepted internally", {
    ...
    async prepareSession(s) {
        ...
        let attempt = 0;
        garpDebug(1, "session preparation begin", {
            session_id: s.sessionId,
            provider: s.provider,
            tab_id: s.tabId,
            page_generation: String(s.pageGeneration),
        ...
        while (monotonicNow() < deadline) {
            attempt++;
            if (s.state === "CLOSED") {
                garpDebug(1, "session preparation stopped: closed", {
                    session_id: s.sessionId,
                    attempt,
                });
                return;
            ...
            try {
                garpDebug(2, "PrepareSession child call", {
                    session_id: s.sessionId,
                    attempt,
                    provider: s.provider,
                });
                ...
                garpDebug(2, "PrepareSession child result", {
                    session_id: s.sessionId,
                    attempt,
                    result: r,
                });
                ...
                if (r.input_ready) {
                    s.state = "READY";
                    garpDebug(1, "SESSION_READY commit", {
                        session_id: s.sessionId,
                        provider: s.provider,
                        tab_id: s.tabId,
                        url: r.url || s.pageURL,
                ...
                garpDebug(1, "session preparation exception", {
                    session_id: s.sessionId,
                    attempt,
                    code: e?.code || null,
                    message: e?.message || String(e),
    ...
    async callChild(s, operation, payload = {}) {
        const debugLevel = getGarpDebugLevel();
        ...
        garpDebug(2, "child call preparation", {
            operation,
            session_id: s.sessionId,
        ...
        const actorChanged = !!s.actorObject && s.actorObject !== actor;
        const urlChanged = !!s.pageURL && s.pageURL !== currentURL;
        if (actorChanged || urlChanged) {
            const adapter = this.providerRegistry.get(s.provider);
            const providerContext = !!adapter && adapter.matchesURL(currentURL);
            s.incrementPageGeneration(currentURL);
            if (providerContext) {
                await this.commitSessionEvent(s, "NAVIGATION", {
                    page_generation: String(s.pageGeneration),
                    url: currentURL,
                });
            }
            else {
                await this.commitSessionEvent(s, "NAVIGATION_DRIFT", {
                    page_generation: String(s.pageGeneration),
                    reason: "UNEXPECTED_URL",
                });
                if (s.activeRequestId)
                    throw new GarpError(GarpErrorCode.NAVIGATION_DRIFT, "URL changed outside provider context during active prompt", { reason: "UNEXPECTED_URL" });
            }
        }
        s.actorObject = actor;
        if (!s.actorInstance)
        ...
        const result = await actor.sendQuery(`GARP:${operation}`, { ...payload, ...fence, actor_instance: s.actorInstance, debug_level: debugLevel });
        garpDebug(2, "child call result", {
            operation,
            session_id: s.sessionId,
            provider: payload.provider ?? null,
            prompt_request_id: payload.prompt_request_id ?? null,
