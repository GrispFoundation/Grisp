...
## Runtime startup diagnostics and timers

The service startup path emits explicit diagnostics for service initialization,
security-state initialization, socket binding, and startup failures.

Parent-side timers use Firefox-native `nsITimer` objects rather than bare
`setInterval()` or `setTimeout()` globals, which are not available in this
privileged module context. Provider-page timers such as `window.setTimeout()`
remain page-context operations and are not changed.
