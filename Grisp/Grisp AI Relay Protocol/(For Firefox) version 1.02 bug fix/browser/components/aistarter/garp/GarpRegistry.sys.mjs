...
const TYPE_NAME_BY_ID = new Map(Object.entries(GarpMessageType).map(([name, id]) => [id, name.toLowerCase()]));
const TYPE_ID_BY_NAME = new Map(Object.entries(GarpMessageType).map(([name, id]) => [name.toLowerCase(), id]));
...
export function messageTypeId(name) {
  return TYPE_ID_BY_NAME.get(String(name || "").toLowerCase()) || 0;
}
