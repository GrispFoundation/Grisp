...
type
    ...
    private
        ...
        function ReceiveFrame(
            ...
            const AMaxPayloadBytes: Cardinal = GARP_APPLICATION_MAX_PAYLOAD
        ): Boolean;
        function ReceiveUntilRequest(
            const ARequestID: string;
            const AAcceptedTypes: array of TGARPMessageType;
            out AFrame: TGARPFrame;
            const ATimeoutMS: Cardinal = 30000;
            const AMaxPayloadBytes: Cardinal = GARP_APPLICATION_MAX_PAYLOAD
        ): Boolean;
    ...
    public
        ...
        function Prompt(
            ...
            const AForceFocus: Boolean = False;
            const ASimulateEnter: Boolean = False
        ): TJSONObject;
        ...
        function GetEvents(
            const AFromSequence: UInt64;
            const AMaxEvents: Cardinal = 4096
        ): TJSONObject;
...
constructor TGARPClient124.Create(
    const AHost: string;
    const APort: Integer;
    const ASecret: string
);
begin
    ...
    FOfferedFeatures :=
        GARPCanonicalFeatureList([
            GARP_FEATURE_EXT_PONG,
            GARP_FEATURE_EXT_REPLAY_STORE
        ]);
end;
...
procedure TGARPClient124.DebugFrame(
    const APrefix: string;
    const AFrameBytes: TBytes;
    const AFrame: TGARPFrame
);
begin
    ...
    DebugLog(
        Format(
            '%s bytes=%d type=%s request_id=%s session_id=%s',
            [
                APrefix,
                Length(AFrameBytes),
                GARPMessageTypeToName(AFrame.MessageType),
                AFrame.RequestID,
                AFrame.SessionID
            ]
        )
...
function TGARPClient124.BuildMessage(
    const ATypeName: string;
    const APayload: TJSONValue;
    const ARequestID: string;
    const ASessionID: string
...
procedure TGARPClient124.SendFrame(
    const AMessageType: TGARPMessageType;
    const AMessage: TJSONObject;
    const ARequestID: string;
    const ASessionID: string
...
function TGARPClient124.ReceiveFrame(
    out AFrame: TGARPFrame;
    const ATimeoutMS: Cardinal;
...
function TGARPClient124.IsAcceptedType(
    const AMessageType: TGARPMessageType;
    const AAcceptedTypes: array of TGARPMessageType
): Boolean;
var
...
function TGARPClient124.HasNegotiatedFeature(
    const AFeature: string
): Boolean;
var
    LFeature: string;
begin
    for LFeature in FNegotiatedFeatures do
    begin
        if SameText(LFeature, AFeature) then
            Exit(True);
    end;
...
function TGARPClient124.GetErrorText(
    const AFrame: TGARPFrame
): string;
var
...
begin
    if AFrame.MessageJSON = nil then
        Exit('GARP ERROR with no JSON payload');
    ...
    try
        ...
        Result := Format(
            '%s: %s',
            [
                LPayload.GetValue<string>('code', 'ERROR'),
                LPayload.GetValue<string>('message', Result)
            ]
        );
    except
...
procedure TGARPClient124.StoreCapabilities(
    const AFrame: TGARPFrame
);
begin
    FCapabilities.Free;
    FCapabilities :=
        AFrame.MessageJSON.Clone as TJSONObject;
end;

procedure TGARPClient124.StoreBrowserStatus(
    const AFrame: TGARPFrame
);
begin
    FBrowserStatus.Free;
...
procedure TGARPClient124.HandleUnsolicitedFrame(
    var AFrame: TGARPFrame
);
var
    LPong: TJSONObject;
begin
...
function TGARPClient124.ReceiveUntilRequest(
    const ARequestID: string;
    const AAcceptedTypes: array of TGARPMessageType;
    out AFrame: TGARPFrame;
    const ATimeoutMS: Cardinal;
...
begin
    ...
    begin
        ...
        LRemaining :=
            ATimeoutMS - Cardinal(LNow - LStart);

        if LRemaining > 1000 then
            LRemaining := 1000;
        ...
        try
            ...
            {
              ARequestID = '' is used when the caller is explicitly
              waiting for an unsolicited envelope such as CAPABILITIES
              or BROWSER_STATUS.  In that case the frame must be returned
              to the caller rather than consumed as background state.
            }
            if (ARequestID = '') and
               (LFrame.RequestID = '') then
            begin
                if IsAcceptedType(
                    LFrame.MessageType,
                    AAcceptedTypes
...
function TGARPClient124.ConstantTimeEqual(
    const ALeft,
    ARight: TBytes
): Boolean;
var
...
begin
    ...
    for LIndex := 0 to High(ALeft) do
        LDiff :=
            LDiff or
            (ALeft[LIndex] xor ARight[LIndex]);
    ...
    try
        ...
        try
            ...
            for LIndex := 0 to High(FOfferedFeatures) do
                LOfferedFeaturesJSON.Add(
                    FOfferedFeatures[LIndex]
                );

            LHelloPayload.AddPair(
                'versions',
                TJSONArray.Create
                    .Add(GARP_SEMANTIC_VERSION)
            );
            LHelloPayload.AddPair(
                'wire_versions',
                TJSONArray.Create
                    .Add(GARP_WIRE_VERSION)
            );
            LHelloPayload.AddPair(
                'features',
                LOfferedFeaturesJSON
            );
            LHelloPayload.AddPair(
                'client_nonce',
                GARPBase64UrlEncode(FClientNonce)
            );

            { BuildMessage transfers ownership of LHelloPayload to LMessage. }
            LMessage := BuildMessage(
                'HELLO',
                LHelloPayload,
                GARPNewUUID
            );
        ...
        if not ReceiveUntilRequest(
            FRequestID,
            [GARP_HELLO_CHALLENGE, GARP_ERROR],
            LFrame,
            GARP_HANDSHAKE_TIMEOUT,
            GARP_HANDSHAKE_MAX_PAYLOAD
        ) then
            raise EGARPProtocolError.Create(
                'Timed out waiting for HELLO_CHALLENGE'
            );
        ...
        try
            ...
            LChallenge :=
                GARPJSONObjectPayload(LFrame);

            FServerName :=
                LChallenge.GetValue<string>(
                    'server_name',
                    ''
                );
            FServerVersion :=
            ...
            if LSelectedVersion <>
               GARP_SEMANTIC_VERSION then
                raise EGARPProtocolError.Create(
                    'Server selected an unsupported semantic version'
                );

            if LSelectedWire <>
               GARP_WIRE_VERSION then
                raise EGARPProtocolError.Create(
                    'Server selected an unsupported wire version'
                );
            ...
            LFinalFeaturesJSON :=
                LChallenge.GetValue<TJSONArray>(
                    'features'
                );

            if LFinalFeaturesJSON = nil then
                raise EGARPProtocolError.Create(
                    'HELLO_CHALLENGE has no features'
                );

            SetLength(
                LFinalFeatures,
                LFinalFeaturesJSON.Count
            );

            for LIndex := 0 to
                LFinalFeaturesJSON.Count - 1 do
                LFinalFeatures[LIndex] :=
                    LFinalFeaturesJSON.Items[LIndex].Value;
        ...
        try
            ...
            { BuildMessage transfers ownership of LAuthPayload to LMessage. }
            LMessage := BuildMessage(
                'HELLO_AUTH',
                LAuthPayload,
                FRequestID
        ...
        try
            ...
            if Length(LServerProof) <> 32 then
                raise EGARPProtocolError.Create(
                    'HELLO_ACK server_proof must decode to 32 bytes'
                );
            ...
            if not ConstantTimeEqual(
                ...
                raise EGARPProtocolError.Create(
                    'HELLO_ACK server proof verification failed'
                );
        ...
        begin
            if not ReceiveUntilRequest(
                ...
                raise EGARPProtocolError.Create(
                    'Timed out waiting for post-authentication state'
                );
            ...
            try
                ...
                if LFrame.MessageType =
                   GARP_CAPABILITIES then
                    StoreCapabilities(LFrame)
                else if LFrame.MessageType =
                        GARP_BROWSER_STATUS then
...
begin
    ...
    if not ReceiveUntilRequest(
        LRequestID,
        [GARP_RESPONSE, GARP_ERROR],
        LFrame,
        30000
    ) then
        raise EGARPProtocolError.Create(
            'Timed out waiting for GET_CAPABILITIES response'
        );
    ...
    try
        ...
        Result :=
            LFrame.MessageJSON.Clone as TJSONObject;
    finally
        LFrame.MessageJSON.Free;
        LFrame.MessageJSON := nil;
    end;
...
begin
    ...
    if FBrowserStatus = nil then
        raise EGARPProtocolError.Create(
            'No BROWSER_STATUS has been received yet'
        );

    Result :=
        FBrowserStatus.Clone as TJSONObject;
end;
...
begin
    ...
    if not ReceiveUntilRequest(
        ...
        raise EGARPProtocolError.Create(
            'Timed out waiting for LIST_TABS response'
        );
...
function TGARPClient124.OpenTab(
    const AURL: string
): TJSONObject;
var
    LPayload: TJSONObject;
...
begin
    ...
    if not ReceiveUntilRequest(
        ...
        raise EGARPProtocolError.Create(
            'Timed out waiting for OPEN_TAB response'
        );
...
function TGARPClient124.CloseTab(
    const ATabID: string
): TJSONObject;
var
    LPayload: TJSONObject;
...
begin
    ...
    if not ReceiveUntilRequest(
        ...
        raise EGARPProtocolError.Create(
            'Timed out waiting for CLOSE_TAB response'
        );
...
function TGARPClient124.SelectTab(
    const ATabID: string
): TJSONObject;
var
    LPayload: TJSONObject;
...
begin
    ...
    if not ReceiveUntilRequest(
        ...
        raise EGARPProtocolError.Create(
            'Timed out waiting for SELECT_TAB response'
        );
...
function TGARPClient124.CreateSession(
    const AProvider: string;
    const ATabID: string
): TJSONObject;
var
...
begin
    ...
    if not ReceiveUntilRequest(
        ...
        raise EGARPProtocolError.Create(
            'Timed out waiting for CREATE_SESSION response'
        );
    ...
    try
        ...
        if LReturnedSessionID = '' then
            raise EGARPProtocolError.Create(
                'CREATE_SESSION response did not contain session_id'
            );
...
function TGARPClient124.WaitForSessionReady(
    const ASessionID: string;
    const ATimeoutMS: Cardinal
): TJSONObject;
var
...
begin
    ...
    begin
        ...
        if LNow - LStart >= ATimeoutMS then
            raise EGARPProtocolError.Create(
                'Timed out waiting for SESSION_READY'
            );
...
begin
    ...
    if not ReceiveUntilRequest(
        ...
        raise EGARPProtocolError.Create(
            'Timed out waiting for GET_SESSION response'
        );
...
function TGARPClient124.Prompt(
    const APrompt: string;
    const ATimeoutMS: Integer;
    const AAutoContinue: Boolean;
    const AForceFocus: Boolean;
    const ASimulateEnter: Boolean
): TJSONObject;
var
    LPayload: TJSONObject;
    LOptions: TJSONObject;
...
begin
    ...
    if ATimeoutMS <= 0 then
        raise EGARPProtocolError.Create(
            'Prompt timeout must be positive'
        );
    ...
    if not ReceiveUntilRequest(
        ...
        raise EGARPProtocolError.Create(
            'Timed out waiting for PROMPT acceptance'
        );
...
function TGARPClient124.WaitForCompletion(
    const ARequestID: string;
    const ATimeoutMS: Cardinal
): TJSONObject;
var
...
begin
    ...
    begin
        ...
        if LNow - LStart >= ATimeoutMS then
            raise EGARPProtocolError.Create(
                'Timed out waiting for a terminal prompt event'
            );
        ...
        try
            ...
            if not (
                LFrame.MessageType in [
                    ...
                    GARP_GENERATION_CANCELLED,
                    GARP_INPUT_SUBMITTED,
                    GARP_PROVIDER_ERROR,
                    GARP_PROVIDER_AUTH_REQUIRED,
                    GARP_PROVIDER_RATE_LIMITED,
                    GARP_DIAGNOSTIC
                ]
            ) then
                Continue;
...
function TGARPClient124.GetPromptStatus(
    const ARequestID: string
): TJSONObject;
var
    LPayload: TJSONObject;
...
begin
    ...
    if not ReceiveUntilRequest(
        ...
        raise EGARPProtocolError.Create(
            'Timed out waiting for GET_PROMPT_STATUS'
        );
...
function TGARPClient124.GetResponse(
    const ARequestID: string
): TJSONObject;
var
    LPayload: TJSONObject;
...
begin
    ...
    if not ReceiveUntilRequest(
        ...
        raise EGARPProtocolError.Create(
            'Timed out waiting for GET_RESPONSE'
        );
...
function TGARPClient124.CancelPrompt(
    const ARequestID: string
): TJSONObject;
var
    LPayload: TJSONObject;
...
begin
    ...
    if not ReceiveUntilRequest(
        ...
        raise EGARPProtocolError.Create(
            'Timed out waiting for CANCEL_PROMPT'
        );
...
function TGARPClient124.ContinuePrompt(
    const ARequestID: string
): TJSONObject;
var
    LPayload: TJSONObject;
...
begin
    ...
    if not ReceiveUntilRequest(
        ...
        raise EGARPProtocolError.Create(
            'Timed out waiting for CONTINUE_PROMPT'
        );
...
function TGARPClient124.GetEvents(
    const AFromSequence: UInt64;
    const AMaxEvents: Cardinal
): TJSONObject;
var
    LPayload: TJSONObject;
    LMessage: TJSONObject;
...
begin
    ...
    if not HasNegotiatedFeature(
        GARP_FEATURE_EXT_REPLAY_STORE
    ) then
        raise EGARPProtocolError.Create(
            'ext-replay-store was not negotiated'
        );
    ...
    try
        LPayload.AddPair(
            'from_sequence',
            AFromSequence.ToString
        );
        LPayload.AddPair(
            'max_events',
            TJSONNumber.Create(AMaxEvents)
        );
    ...
    if not ReceiveUntilRequest(
        ...
        raise EGARPProtocolError.Create(
            'Timed out waiting for GET_EVENTS'
        );
...
function TGARPClient124.SubscribeResponse(
    const ARequestID: string
): TJSONObject;
var
    LPayload: TJSONObject;
...
begin
    ...
    if not ReceiveUntilRequest(
        ...
        raise EGARPProtocolError.Create(
            'Timed out waiting for SUBSCRIBE_RESPONSE'
        );
...
begin
    ...
    if not ReceiveUntilRequest(
        ...
        raise EGARPProtocolError.Create(
            'Timed out waiting for CLOSE_SESSION response'
        );
