...
begin
    ...
    Writeln('  --prompt <text>           Create/use session and run prompt');
...
begin
    ...
    if not (
        Result.RunConformance or
        Result.RunSelfTest or
        Result.DoPing or
        Result.DoCapabilities or
        Result.DoBrowserStatus or
        Result.DoListTabs or
        Result.DoSession or
        Result.DoPrompt
    ) then
        PrintHelp;
            ...
            begin
                if AOptions.Provider = '' then
                    raise Exception.Create(
                        '--prompt requires --session <provider>'
                    );
...
begin
    ...
    try
        ...
        LResponse :=
            LClient.Prompt(
                'Reply with exactly: GARP/1.24 TEST OK',
                120000,
                True
            );
        try
            Writeln(
                '[PASS] PROMPT accepted by RESPONSE'
            );
            PrintJSONObject(
                'PROMPT RESPONSE',
                LResponse
