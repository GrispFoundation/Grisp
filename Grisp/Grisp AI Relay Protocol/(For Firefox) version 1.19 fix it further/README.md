...
The Firefox-side GARP implementation contains runtime-switchable tracing. Debugging is disabled by default, so normal runs do not emit the detailed trace.
...
Level 1 traces GARP connections, received and sent message types, authentication milestones, session creation and state transitions, prompt acceptance, child operations, provider submission, generation events, failures, and connection cleanup.
...
Level 2 additionally traces child-actor calls and results, page-generation fencing, DOM selector searches, DeepSeek input/send-button discovery, focus and input verification, submission verification, DOM generation observations, and each generation observation iteration.

The trace is written with `dump()` to the Firefox process console, including timestamps and diagnostic levels. The parent GARP service reads `GARP_DEBUG` from the environment and passes the selected level to the content-side GARP actor so both sides use the same setting.

The diagnostic layer truncates large string values and nested structures to limit console volume. Authentication secrets are never logged, and prompt contents are represented by lengths rather than full prompt text.

`GARP_DEBUG` is an environment variable, not a special `./mach run --debug` option. Use `export` in the MozillaBuild shell before `./mach run`.
