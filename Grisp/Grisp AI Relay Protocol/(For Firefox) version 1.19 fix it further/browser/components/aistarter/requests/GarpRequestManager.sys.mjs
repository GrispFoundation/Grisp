...
export class GarpRequestManager {
    ...
    async observe(r) {
        ...
        for (;;) {
            ...
            const count = Number(x.message_count || 0), text = String(x.text ?? ""), observedMessages = Array.isArray(x.messages) && x.messages.length ? x.messages : text.trim().length > 0 ? [{ message_id: x.message_id || null, text }] : [], identified = count > r.lastMessageCount || text !== r.baselineMessageText || x.authoritative_generation_started === true;
            r.lastMessageCount = count;
            r.baselineMessageText = text;
            r.lastPageGeneration = s.pageGeneration;
            r.actorInstance = s.actorInstance;
            if (identified && !r.responseStarted && observedMessages.length > 0) {
            ...
            if (r.responseStarted && x.authoritative_completion === true && x.streaming === false && x.continuation_required !== true) {
