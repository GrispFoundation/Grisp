...
procedure PrintHelp;
begin
    Writeln('GARP/1.24 Delphi Test Client v0.05');
    Writeln;
...
const
    LPromptText = 'Reply with exactly: GARP/1.24 TEST OK';
var
    LClient: TGARPClient124;
    LResponse: TJSONObject;
    LProvider: string;
    LFirstPromptID: string;
    LSecondPromptID: string;
    LSessionID: string;
    LFirstResponseText: string;
    LSecondResponseText: string;
    LSessionState: string;
begin
    Writeln('=== GARP/1.24 live conformance smoke test v0.05 ===');
    ...
    try
        ...
        LSessionID := LClient.SessionID;
        AssertTrue('session ID established', LSessionID <> '');

        Writeln('--- PROMPT #1 ---');
        LResponse := LClient.Prompt(LPromptText, 120000, True);
        try
            Writeln('[PASS] PROMPT #1 accepted by RESPONSE');
            PrintJSONObject('PROMPT #1 RESPONSE', LResponse);
        finally
            LResponse.Free;
        end;
        LFirstPromptID := LClient.RequestID;

        LResponse := LClient.WaitForCompletion(LFirstPromptID, 120000);
        try
            PrintJSONObject('PROMPT #1 TERMINAL EVENT', LResponse);
            AssertEqual(
                'PROMPT #1 terminal event',
                'GENERATION_COMPLETED',
                LResponse.GetValue<string>('type', '')
            );
            LFirstResponseText := LResponse.GetValue<string>('payload.response', '');
            AssertEqual(
                'PROMPT #1 response text',
                'GARP/1.24 TEST OK',
                LFirstResponseText
            );
            Writeln('[PASS] PROMPT #1 GENERATION_COMPLETED');
        finally
            LResponse.Free;
        end;

        LResponse := LClient.GetSession;
        try
            PrintJSONObject('SESSION AFTER PROMPT #1', LResponse);
            LSessionState := LResponse.GetValue<string>('payload.state', '');
            AssertEqual('session READY after PROMPT #1', 'READY', LSessionState);
            AssertEqual('session ID preserved after PROMPT #1', LSessionID, LClient.SessionID);
        finally
            LResponse.Free;
        end;

        Writeln('--- PROMPT #2 (identical response text) ---');
        LResponse := LClient.Prompt(LPromptText, 120000, True);
        try
            Writeln('[PASS] PROMPT #2 accepted by RESPONSE');
            PrintJSONObject('PROMPT #2 RESPONSE', LResponse);
        finally
            LResponse.Free;
        end;
        LSecondPromptID := LClient.RequestID;

        AssertTrue('PROMPT request IDs are distinct', LFirstPromptID <> LSecondPromptID);

        LResponse := LClient.WaitForCompletion(LSecondPromptID, 120000);
        try
            PrintJSONObject('PROMPT #2 TERMINAL EVENT', LResponse);
            AssertEqual(
                'PROMPT #2 terminal event',
                'GENERATION_COMPLETED',
                LResponse.GetValue<string>('type', '')
            );
            LSecondResponseText := LResponse.GetValue<string>('payload.response', '');
            AssertEqual(
                'PROMPT #2 response text',
                'GARP/1.24 TEST OK',
                LSecondResponseText
            );
            AssertEqual(
                'PROMPT #2 response identical to PROMPT #1',
                LFirstResponseText,
                LSecondResponseText
            );
            Writeln('[PASS] PROMPT #2 GENERATION_COMPLETED');
            Writeln('[PASS] same-session identical-response baseline exercised');
        finally
            LResponse.Free;
        end;
        ...
        LResponse := LClient.GetSession;
        try
            PrintJSONObject('SESSION AFTER PROMPT #2', LResponse);
            LSessionState := LResponse.GetValue<string>('payload.state', '');
            AssertEqual('session READY after PROMPT #2', 'READY', LSessionState);
            AssertEqual('session ID preserved after PROMPT #2', LSessionID, LClient.SessionID);
        finally
            LResponse.Free;
        end;
