...
begin
    ...
    Writeln;
    Writeln('Examples:');
    Writeln('  GARP101.Test.exe --secret development-secret --conformance');
    Writeln('  GARP101.Test.exe --secret development-secret --session deepseek --prompt "Hello"');
end;
...
begin
    ...
    begin
        ...
        begin
            Inc(LIndex);
            if LIndex > ParamCount then raise Exception.Create('--host requires a value');
            Result.Host := ParamStr(LIndex);
        end
        else if LArg = '--port' then
        begin
            Inc(LIndex);
            if LIndex > ParamCount then raise Exception.Create('--port requires a value');
            Result.Port := StrToInt(ParamStr(LIndex));
        end
        else if LArg = '--secret' then
        begin
            Inc(LIndex);
            if LIndex > ParamCount then raise Exception.Create('--secret requires a value');
        ...
        begin
            Inc(LIndex);
            if LIndex > ParamCount then raise Exception.Create('--session requires a provider');
        ...
        begin
            Inc(LIndex);
            if LIndex > ParamCount then raise Exception.Create('--prompt requires text');
    ...
    if not Result.RunConformance and not Result.RunSelfTest and
       not Result.DoPing and not Result.DoCapabilities and
       not Result.DoBrowserStatus and not Result.DoListTabs and
       not Result.DoSession and not Result.DoPrompt then
        PrintHelp;
end;
...
begin
    ...
    try
        AssertEqual('magic/version decode', GARP_PROTOCOL, LFrame.MessageJSON.GetValue<string>('protocol', ''));
        ...
        if Length(LFrameBytes) <> GARP_FIXED_HEADER_LENGTH + Length(TEncoding.UTF8.GetBytes(LFrame.MessageJSON.ToJSON)) then
            raise Exception.Create('Encoded frame length invariant failed');
...
begin
    LClient := TGARPClient101.Create(AOptions.Host, AOptions.Port, AOptions.Secret);
    try
        ...
        begin
            ...
            try PrintJSONObject('CAPABILITIES', LResponse); finally LResponse.Free; end;
        end;
        if AOptions.DoBrowserStatus then
        begin
            LResponse := LClient.GetBrowserStatus;
            try PrintJSONObject('BROWSER_STATUS', LResponse); finally LResponse.Free; end;
        end;
        if AOptions.DoListTabs then
        begin
            LResponse := LClient.ListTabs;
            try PrintJSONObject('LIST_TABS', LResponse); finally LResponse.Free; end;
        end;
        ...
        begin
            ...
            begin
                if AOptions.Provider = '' then
                    raise Exception.Create('--prompt requires --session <provider> when no session is active');
                LResponse := LClient.CreateSession(AOptions.Provider);
                LResponse.Free;
            end;
            LResponse := LClient.Prompt(AOptions.PromptText);
            try
                PrintJSONObject('PROMPT_ACK', LResponse);
            finally
                LResponse.Free;
            end;
            LResponse := LClient.WaitForCompletion(LClient.RequestID, 180000);
            try
                PrintJSONObject('FINAL', LResponse);
            finally
                LResponse.Free;
            end;
        end;
...
begin
    ...
    if AOptions.Secret = '' then
        raise Exception.Create('GARP secret not supplied. Use --secret or GARP_SECRET.');
    ...
    try
        ...
        LSessionProvider := AOptions.Provider;
        if LSessionProvider = '' then
            LSessionProvider := 'deepseek';
        Writeln('Creating session for provider: ', LSessionProvider);
        LResponse := LClient.CreateSession(LSessionProvider);
        try
        ...
        Writeln('Waiting for provider session to become ready...');
        Sleep(1500);
        LResponse := LClient.Prompt('Reply with exactly: GARP/1.01 TEST OK', True, 120000, True);
        try
        ...
        LResponse := LClient.WaitForCompletion(LClient.RequestID, 120000);
        try
            PrintJSONObject('GENERATION_RESULT', LResponse);
            if SameText(LResponse.GetValue<string>('type', ''), 'generation_completed') then
                Writeln('[PASS] GENERATION_COMPLETED')
            else
                Writeln('[INFO] Generation ended with: ', LResponse.GetValue<string>('type', 'unknown'));
        finally
            LResponse.Free;
        end;
...
begin
    ...
    try
        ...
        begin
            Writeln('GARP connection closed by Firefox: ', E.Message);
            ExitCode := 2;
        end;
        on E: Exception do
        begin
