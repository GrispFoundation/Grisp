...
The protocol contract implemented here is the normative document **GARP/1.24** (`Gateway AI Runtime Protocol`, wire version `0x00`). Section references in this README (`S…`) refer to that specification.
...
Provider adapters are isolated. A provider DOM change should normally require a change only to that provider adapter. See S119 and S167.
...
GARP/1.24 requires mutual challenge-response authentication using a deployment-provisioned shared secret (S16). The secret MUST contain at least **32 raw bytes** (S16.2).
...
## Replay protection (S19)

GARP/1.24 requires that authentication replay protection survive gateway process restarts for at least 24 hours (S19). **The current development build keeps the replay-protection store in memory only.** A production conformance build MUST persist this store durably, and if the replay state cannot be re-established, the affected shared secret MUST be disabled and rotated (S19.1). This limitation is documented rather than silently ignored.
...
Per S4.3, the gateway binds only to loopback. It does not expose an unauthenticated GARP listener on a non-loopback interface.

# 11. Wire protocol

GARP/1.24 uses a binary length-framed TCP protocol (S4, S6). Every frame consists of a fixed 48-byte header followed by the frame payload.
...
All multibyte integer fields are big-endian (S5). The fixed header is **48 bytes** and `header_len` MUST be exactly `48` (S6.4).

The frame payload is a UTF-8 JSON **Common Envelope** (S11):
...
- `garp` is the semantic version string, `"1.24"` (S11.2).
- `type` is the ASCII uppercase registry name (S11.3).
- `request_id` and `session_id` are always present; JSON `null` corresponds to the all-zero nil UUID in the binary header (S7.2).
- `sequence` is always present as either a uint64-string or `null` (S11.7).
- All base GARP message and payload objects are **closed** unless explicitly declared open (S9.3). Unknown members MUST cause `INVALID_ARGUMENT`.
- Duplicate JSON object member names MUST be rejected (S9.2).
- All `uint64` fields are encoded as decimal strings matching `^(0|[1-9][0-9]{0,19})$` (S10.1). JavaScript number precision MUST NOT be relied upon for these values.

Two payload length limits exist and MUST NOT be conflated (S6.5, S23):
...
The base GARP/1.24 message registry is defined in S13.1. The message codes this implementation uses are:
...
Extension message codes MUST be allocated outside the base registry (S13.2).

# 13. Handshake

The handshake is mutual and follows S16-S17:
...
Both `client_nonce` and `server_nonce` MUST decode to exactly **16 raw bytes** and are represented in JSON as **unpadded Base64URL**. Padding characters MUST be rejected (S16.4, S16.5).

## Authentication transcript (S16.6)
...
The proof is `HMAC-SHA256(shared_secret, transcript)`, exactly 32 raw bytes, represented in JSON as unpadded Base64URL. Comparison MUST be constant-time (S16.7).
...
Both the *offered* feature set and the *final negotiated* feature set are cryptographically bound into the transcript, so the required-feature marker (`!`) cannot be silently stripped (S16.9).

## Feature negotiation (S15, S138)
...
After successful authentication, the server emits, in exactly this order (S21, S115):
...
- `uint64` limit fields are `uint64-string` per S10.1 and bounded per S10.5.
...
Provider entries (S22.1) contain `id`, `version`, `state`, `input_strategies`, and a closed `capabilities` object with exactly these boolean members: `streaming`, `cancellation`, `continuation`, `diagnostics`. Additional members MUST NOT be present in base GARP. Providers are sorted by `id` in raw UTF-8 byte order.

# 16. Keepalive and PING (S99-S101)

- A received `PING` is a normal command and is answered by a normal `RESPONSE` frame. **PING does not cause a PONG.**
- `PONG` is emitted only by the `ext-pong` keepalive mechanism, and only when `ext-pong` is present in the final negotiated feature set (S101).
- When `ext-pong` is negotiated and `keepalive.enabled` is `true`, the gateway maintains two monotonic timestamps per connection (`last_outbound_protocol_time` and `last_inbound_authenticated_time`) and emits unsolicited PONG frames on the interval. All timing decisions MUST use a monotonic clock; raw TCP byte activity MUST NOT be used as a substitute (S101 rule 8).
...
A session has a stable immutable `session_id` (S28). Provider, `tab_id`, `url`, `page_generation`, and `state` are mutable.

Session states are the complete set defined in S29:
...
The session transition matrix is normative in S31. In this build:

- `RESET_SESSION` increments `page_generation`, invalidates stale actors, preserves `session_id`, and preserves the session event sequence (S32, S40, S153).
- A tab index change does not change `tab_id` (S43).
- A JSWindowActor replacement does not change `session_id` or `tab_id`, but it does increment `page_generation` (S33, S168).

`page_generation` is a **uint64-string** (S33). It never decrements and never resets to zero during `RESET_SESSION`. Overflow produces `PAGE_GENERATION_OVERFLOW`.

# 18. Prompt behavior

Prompt states are the complete set defined in S48:
...
Only one active prompt is permitted per session. A second simultaneous `PROMPT` against a `READY` session receives `SESSION_BUSY` (S51).

## Prompt text is opaque (S49.1, S121)
...
- The `partial` flag on the resulting `GENERATION_FAILED` follows the historical rule in S76: `partial = true` iff authoritative response content has already been committed at least once for the prompt. A later `REPLACE` producing an empty string MUST NOT revert `partial` to `false`.

## Cancellation (S72-S73)

- `CANCEL_PROMPT` is valid for `PENDING`, `SUBMITTING`, `GENERATING`, `WAITING_CONTINUATION`.
- A second `CANCEL_PROMPT` while in `CANCELLING` fails with `INVALID_MESSAGE_STATE` (S27.1). `CANCELLING` is nonterminal.
- `CANCEL_PROMPT` against a terminal prompt fails with `PROMPT_ALREADY_TERMINAL`.
- `GENERATION_CANCELLED` requires positive provider cessation evidence (S73). Submission success is **not** cessation evidence. If cessation cannot be authoritatively established, the prompt MUST submit a `RECOVERY_FAILED` terminal candidate and the session MUST enter `RECOVERING` (S64.4, S75).

## Terminal arbitration (S74)

Each prompt has exactly one terminal commit: `COMPLETED`, `FAILED`, or `CANCELLED`. The session arbiter serializes all terminal candidates and the first valid candidate that commits wins. Once committed, the prompt can never reopen (S74).

**This build currently uses a simplified arbiter:** cancellation commits directly to `CANCELLED` after requesting provider cancellation, and completion commits directly on adapter-reported cessation. A production conformance build MUST implement the first-commit-wins arbiter with full fencing (S91) and recovery integration (S75, S93-S96).
...
All session-scoped asynchronous events have (S11.4, S85):
...
Sequence numbers (S86, S87):

- First event: `"1"`.
- Increment by `1` per event.
- Monotonic, arbiter-allocated, never repeat, never reset.
- Survive `RESET_SESSION` (S153).
- Overflow beyond `uint64` produces `EVENT_SEQUENCE_OVERFLOW` and fails closed.

Connection-scoped messages (`CAPABILITIES`, `BROWSER_STATUS`, `PONG`) use `sequence = null` and do not consume session history (S89, S100).

# 20. Error codes

The base error registry has 47 codes defined in S27. Relevant codes this build emits include:
...
A receiver MUST validate a frame in the order specified in S8. This build performs, in order:

1. fixed header availability;
2. magic;
3. wire version (`0x00`);
4. `header_len == 48`;
5. `payload_len` against the effective limit (S6.5);
6. `flags == 0`;
7. `reserved == 0`;
...
See S171 for the reference negotiation sequence.

# 23. Provider adapter behavior

Provider-specific selectors and DOM logic live exclusively under `providers/` (S119). The generic child actor contains no provider selector table.
...
Per S167, an adapter MUST define authoritative evidence for input readiness, submission, generation, cessation, and errors. Absence of new DOM activity is not sufficient proof of cessation without an authoritative invariant (S55).

Per S42.2, absence of positive authentication evidence is NOT by itself negative evidence. The adapter MUST NOT submit a `PROVIDER_AUTH_REQUIRED` terminal candidate based solely on the absence of a positive observation.

# 24. Actor fencing (S91, S92, S168)
...
Every asynchronous provider/browser operation whose result may outlive the initiating actor is associated with the fencing context:
    ...
    provider            (nullable; see S91)
    session_id
    page_generation
    actor_instance
    prompt_request_id   (nullable; see S91)
...
Firefox must never silently change the semantic AI response while transporting it (S120).
...
- **Level 4 - Recovery:** partial.
  ...
  - Not yet complete: full first-commit-wins terminal arbiter (S74), durable replay-protection store (S19), authoritative cessation evidence for `GENERATION_CANCELLED` (S73), and full recovery-driven terminal resolution (S75).
...
A production conformance build MUST complete the Level 4 items before claiming full Level 4 conformance per S162.
...
Open that provider manually in the local Firefox profile and sign in. GARP does not automate credential entry (S118).

## Provider DOM unsupported

The affected provider adapter needs an update. Do not add the selector to the generic child actor unless the selector is truly provider-independent (S119).
...
Replay protection detected a reused authenticated handshake transcript. This is expected behaviour (S19). If this is not the intended outcome, verify the client is generating a fresh 16-byte `client_nonce` on each connection.
