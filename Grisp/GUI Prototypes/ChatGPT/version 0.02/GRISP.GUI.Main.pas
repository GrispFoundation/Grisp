...
type
    ...
    private
        ...
        procedure InputKeyDown(ParaSender: TObject; var Key: Word; Shift: TShiftState);
    ...
    private
        ...
        procedure HeaderMouseDown(ParaSender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
procedure TGrispChatView.InputKeyDown(ParaSender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TGrispChatView.SendInput(ParaSender: TObject);
var
    vText: string;
begin
...
begin
    ...
    vInputY := Max(0, Height - 106);
    mInputEdit.SetBounds(30, vInputY + 13, Max(100, Width - 135), 28);

    if Assigned(mSendButton) then
        mSendButton.SetBounds(Max(30, Width - 94), vInputY + 10, 76, 30);
end;
...
begin
    ...
    vWords := TStringList.Create;
    try
        ExtractStrings([' ', #9], [], PChar(ParaBody), vWords);
        vLine := '';
        vLineCount := 1;
        ...
        begin
            if (vLine <> '') and
                (ParaCanvas.TextWidth(vLine + ' ' + vWords[vIndex]) > ParaWidth - 28) then
            begin
                Inc(vLineCount);
                vLine := vWords[vIndex];
            end
    ...
    vHeight := Max(70, 46 + (vLineCount * 19));
...
procedure TGrispMainForm.HeaderDblClick(ParaSender: TObject);
begin
...
begin
    ...
    mExplorerButton.Active := mExplorerVisible;
    ...
    mSessionsButton.Active := mSessionsVisible;
