...
begin
  Caption := 'GRISP OS - Delphi 13 Florence Prototype (VCL)';
  Width := 1600;
  Height := 950;
  Position := poScreenCenter;
  Color := TGRISPColors.BG_DARK;
  Font.Name := 'Segoe UI';
  Font.Size := 9;
  Font.Color := TGRISPColors.TEXT_MAIN;
  DoubleBuffered := True;
  ...
  AddChat(crAssistant,
    ...
    '7. WorldState - State',
    'Analyzing...' + sLineBreak + 'Identified 7...' + sLineBreak + 'Preparing...', True);
end;
...
begin
  ...
  memoThink.Lines.Text := 'Analyzing...' + sLineBreak + 'Identified 7...' + sLineBreak + 'Preparing...';
...
begin
  ...
  lbl.Left := 12;
  lbl.Top := 8;
  lbl.Caption := 'ACTIVE: GRISP://Workspace 42.3GB | Delphi 13 | VCL | SEC No Threats';
  lbl.Font.Size := 8;
...
begin
  ...
  begin
    ...
    if HasPlan then memoChat.Lines.Add('[Plan: 4 steps, see right panel]' + sLineBreak);
  end;
...
begin
  ...
  if Assigned(btnSend) then btnSend.Left := pnlChatInput.Width - 56;
  if Assigned(edtChat) then edtChat.Width := pnlChatInput.Width - 120;
end;
