...
type
  ...
  public
    constructor Create(AOwner: TComponent); override;
    procedure Setup(ABody, ABubble: TPanel);
  ...
  private
    ...
    FMessages: TList<TPanel>;

    procedure BuildUI;
    procedure PopulateData;
    procedure AddMessage(const Sender, Text, ThinkText: string; IsUser: Boolean);
    ...
    procedure RecalculateChatLayout;
...
begin FHover := True; Invalidate; end;

procedure TNavButton.CMMouseLeave(var Msg: TMessage);
begin FHover := False; Invalidate; end;
...
//  TThinkHeader
// ═══════════════════════════════════════════════════════════════════
constructor TThinkHeader.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  BevelOuter := bvNone;
  Color := CLR_HEAD;
...
procedure TThinkHeader.Setup(ABody, ABubble: TPanel);
begin
  FBody := ABody;
  FBubble := ABubble;
end;

procedure TThinkHeader.Paint;
begin
  Canvas.Brush.Color := CLR_HEAD;
  Canvas.FillRect(ClientRect);
  ...
  Canvas.Font.Size := 8;
  Canvas.Font.Style := [fsBold];
  Canvas.Font.Color := CLR_CYAN;
  Canvas.Brush.Style := bsClear;

  if FExpanded then
    Canvas.TextOut(10, (Height - Canvas.TextHeight('▼')) div 2, '▼')
  else
    Canvas.TextOut(10, (Height - Canvas.TextHeight('▶')) div 2, '▶');

  Canvas.TextOut(26, (Height - Canvas.TextHeight('Thinking')) div 2, 'Thinking');
end;

procedure TThinkHeader.Click;
begin
  inherited;
  if not Assigned(FBody) or not Assigned(FBubble) then Exit;

  FExpanded := not FExpanded;
  FBody.Visible := FExpanded;

  if Assigned(Parent) and (Parent is TPanel) then
  begin
    if FExpanded then
      Parent.Height := Height + FBody.Height + 4
    else
      Parent.Height := Height + 4;
  end;

  if Assigned(Parent) and Assigned(Parent.Parent) and (Parent.Parent is TPanel) then
  begin
    TPanel(Parent.Parent).Height := 26 + FBubble.Height + 4
      + TPanel(Parent).Height + 4;
  end;

  Invalidate;
  if Assigned(MainForm) then
    MainForm.RecalculateChatLayout;
end;
...
begin
  ...
  AddMessage('GRISP Assistant',
    'Hello Alex, how can I help you with GRISP today?' + sLineBreak +
    'I can help with analysis, planning, or code generation. Just ask.',
    'Waiting for user input...|Parsing GRISP environment...|Ready to assist.',
    False);
...
begin
  case Key of
    VK_F2: begin FShowLeftPanel := not FShowLeftPanel; LayoutPanels; end;
    VK_F3: begin FShowRightPanel := not FShowRightPanel; LayoutPanels; end;
    VK_ESCAPE: Close;
  end;
end;
...
begin
  ...
  FClockLbl.Width := 80;
  FClockLbl.Left := 1300; FClockLbl.Top := 17;
  ...
  FRightPlanBox.Left := 0; FRightPlanBox.Top := 30;
  ...
  FRightWorkBox.Left := 0; FRightWorkBox.Top := 240;
  ...
  FSessionList.Left := 0; FSessionList.Top := 280;
  ...
  FInput.Left := 16; FInput.Top := 14;
  FInput.Width := 500; FInput.Height := 32;
  ...
  FSendBtn.Width := 44; FSendBtn.Height := 32; FSendBtn.Top := 14;
  ...
  FStatusDot.Caption := '●';
  FStatusDot.Left := 16; FStatusDot.Top := 7;
  ...
  FStatusText.Left := 32; FStatusText.Top := 8;
...
procedure TMainForm.RecalculateChatLayout;
var
  I, Y: Integer;
begin
  Y := 0;
...
begin
  ...
  try
    ...
    FTree.Items.AddChild(Root, 'Cloud   R');
...
type TPlanRec = record Icon, Text, Status: string; end;
var
...
type TWorkRec = record Text, Priority: string; end;
var
...
// ═══════════════════════════════════════════════════════════════════
//  AddMessage — met BubblePanel: TChatMessagePanel (FIX)
// ═══════════════════════════════════════════════════════════════════
procedure TMainForm.AddMessage(const Sender, Text, ThinkText: string;
  IsUser: Boolean);
var
  MsgPanel, HdrPanel, ThinkPanel, ThinkBody: TPanel;
  BubblePanel: TChatMessagePanel;
  SenderLbl, TimeLbl, LineLbl: TLabel;
  ThinkHdr: TThinkHeader;
  Bmp: TBitmap;
  R: TRect;
  BubbleH, TextH, TextW, BodyH: Integer;
  ThinkLines: TArray<string>;
  K: Integer;
begin
  ...
  HdrPanel.Left := 0; HdrPanel.Top := 0;
  ...
  SenderLbl.Left := 4; SenderLbl.Top := 2;
  ...
  BubblePanel.BevelOuter := bvNone;
  BubblePanel.IsUser := IsUser;

  LineLbl := TLabel.Create(BubblePanel);
  LineLbl.Parent := BubblePanel;
  LineLbl.Caption := Text;
  LineLbl.Left := 14; LineLbl.Top := 10;
  LineLbl.Width := TextW;
  LineLbl.AutoSize := False;
  LineLbl.WordWrap := True;
  LineLbl.Font.Name := 'Segoe UI';
  LineLbl.Font.Size := 10;
  LineLbl.Font.Color := CLR_TEXT;
  LineLbl.Transparent := True;

  Bmp := TBitmap.Create;
  try
    Bmp.Canvas.Font.Assign(LineLbl.Font);
    R := Rect(0, 0, TextW, 0);
  ...
  // ── Think panel ──
  if (not IsUser) and (Trim(ThinkText) <> '') then
  begin
    ThinkPanel := TPanel.Create(MsgPanel);
    ThinkPanel.Parent := MsgPanel;
    ThinkPanel.Left := 0;
    ThinkPanel.Top := 26 + BubbleH + 4;
    ThinkPanel.Width := MsgPanel.Width;
    ThinkPanel.Anchors := [akLeft, akTop, akRight];
    ThinkPanel.BevelOuter := bvNone;
    ThinkPanel.Color := CLR_MAIN;
    ThinkPanel.ParentBackground := False;

    ThinkLines := ThinkText.Split(['|']);

    BodyH := Length(ThinkLines) * 18 + 10;
    ThinkBody := TPanel.Create(ThinkPanel);
    ThinkBody.Parent := ThinkPanel;
    ThinkBody.Left := 0; ThinkBody.Top := 24;
    ThinkBody.Width := ThinkPanel.Width;
    ThinkBody.Height := BodyH;
    ThinkBody.Anchors := [akLeft, akTop, akRight];
    ThinkBody.BevelOuter := bvNone;
    ThinkBody.Color := CLR_THINK_BG;
    ThinkBody.ParentBackground := False;

    for K := 0 to High(ThinkLines) do
    begin
      LineLbl := TLabel.Create(ThinkBody);
      LineLbl.Parent := ThinkBody;
      LineLbl.Caption := '›  ' + Trim(ThinkLines[K]);
      LineLbl.Left := 14;
      LineLbl.Top := 5 + K * 18;
      LineLbl.Font.Name := 'Segoe UI';
      LineLbl.Font.Size := 8;
      LineLbl.Font.Color := CLR_DGRAY;
      LineLbl.Transparent := True;
    end;

    ThinkHdr := TThinkHeader.Create(ThinkPanel);
    ThinkHdr.Parent := ThinkPanel;
    ThinkHdr.Left := 0; ThinkHdr.Top := 0;
    ThinkHdr.Width := ThinkPanel.Width;
    ThinkHdr.Height := 24;
    ThinkHdr.Anchors := [akLeft, akTop, akRight];
    ThinkHdr.Setup(ThinkBody, BubblePanel);
    ThinkHdr.Expanded := True;

    ThinkPanel.Height := ThinkHdr.Height + ThinkBody.Height + 4;

    MsgPanel.Height := 26 + BubbleH + 4 + ThinkPanel.Height + 4;
  end
  else
  begin
    MsgPanel.Height := 26 + BubbleH + 6;
  end;

  FMessages.Add(MsgPanel);
...
begin
  ...
  ThinkText :=
    'Analyzing: ' + Copy(UserText, 1, 50) + '...|' +
    'Cross-referencing knowledge base and context...|' +
    'Evaluating 3 possible responses...|' +
    'Formulating structured answer...';

  AddMessage('GRISP Assistant',
    'I received your message: "' + UserText + '"' + sLineBreak +
    'Here is my response — click ▼ Thinking above to see my reasoning.',
    ThinkText,
    False);
end;
