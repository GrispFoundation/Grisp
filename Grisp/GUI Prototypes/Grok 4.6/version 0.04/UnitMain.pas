...
type
  ...
  private
    C_BG, C_PANEL, C_PANEL2, C_TEXT, C_TEXT_DIM, C_GREEN, C_USER: TColor;
...
begin
  ...
  // Left - iets breder gemaakt
  pnlLeft := TPanel.Create(Self);
  pnlLeft.Parent := Self;
  pnlLeft.Align := alLeft;
  pnlLeft.Width := 300;               // was 280 → nu 300
...
begin
  ...
  begin
    edtInput.Width := pnlBottom.ClientWidth - 134;
    btnSend.Left := pnlBottom.ClientWidth - 116;
    btnSend.Top := 14;

    // Belangrijk: forceer juiste breedte zodat geen horizontale scrollbar ontstaat
    memWorkPlan.Width := pnlWorkPlan.ClientWidth - 30;
  end;
end;
...
begin
  ...
  // Kortere, schonere tekst + rechten dichterbij
...
begin
  ...
  memChat.SelText := #13#10 + AText;

  // === Betrouwbaar naar beneden scrollen ===
  memChat.SelStart := memChat.GetTextLen;
  memChat.SelLength := 0;
  SendMessage(memChat.Handle, EM_SCROLLCARET, 0, 0);
  SendMessage(memChat.Handle, WM_VSCROLL, SB_BOTTOM, 0);
end;
