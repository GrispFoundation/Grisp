...
const
  ...
  // Eigen event-type constanten — voorkomt botsing met de Win32 API-procedure
  // mouse_event (Delphi is case-insensitive).
  MY_KEY_EVENT   = $0001;
  MY_MOUSE_EVENT = $0002;
...
var
  ...
  // Gebruikersvoorkeur (F1/F2/F3/F5)
  WantLeft:  Boolean = True;
  WantRight: Boolean = True;
  WantThink: Boolean = True;
  MouseEnabled: Boolean = True;

  // Effectieve staat — per frame opnieuw berekend
...
begin
  ...
  if EffLeft  then ColL := EnsureRange(ScreenW div 7, 22, 28) else ColL := 0;
  if EffRight then ColR := EnsureRange(ScreenW div 5, 30, 40) else ColR := 0;
  ...
  begin
    if EffRight then begin
      EffRight := False; ColR := 0;
      ColC := ScreenW - ColL - 4;
    end;
    if (ColC < 30) and EffLeft then begin
      EffLeft := False; ColL := 0;
      ColC := ScreenW - 4;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      HasPress := ((BS and FROM_LEFT_1ST_BUTTON_PRESSED) <> 0)
               or ((BS and RIGHTMOST_BUTTON_PRESSED) <> 0);
      HasWheel := (Rec.Event.MouseEvent.dwEventFlags and MY_MOUSE_WHEELED) <> 0;
      if not (HasPress or HasWheel) then Continue;
  ...
  case Ev.Kind of
    ...
    ikMouse:
      begin
        ...
        begin
          if (Ev.MouseY >= 4) and (Ev.MouseY <= ScreenH - 2) then
            ChatScroll := 0;
        end;
      end;
  end;
