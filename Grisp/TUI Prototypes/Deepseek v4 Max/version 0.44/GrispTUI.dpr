...
begin
  ...
  begin
    ...
    begin
      ...
      { Wheel heeft prioriteit — niet door button-detectie laten lopen. }
...
begin
  ...
  { v0.44: aantal verticale scheidingslijnen hangt af van welke
    panelen zichtbaar zijn. 2 buitenranden + 1 per zichtbaar paneel. }
  ColCenter := ScreenWidth - ColLeft - ColRight - 2;
  if ShowLeftPanel then Dec(ColCenter);
  if ShowRightPanel then Dec(ColCenter);
...
begin
  ...
  try
    ContentW := ColLeft - 1;
    ResBodyH := 0; SesBodyH := 0;
    if not ResCollapsed then
    begin ResBodyH := LeftSplitRow - 2; if ResBodyH < 0 then ResBodyH := 0; end;
    SesTop := LeftSplitRow + 1;
    if not SesCollapsed then
    begin SesBodyH := ContentHeight - SesTop - 2; if SesBodyH < 0 then SesBodyH := 0; end;
    ...
    if ResCollapsed then TitleText := TitleText + ' ' + GLYPH_COLLAPSED
    ...
    if SesCollapsed then TitleText := TitleText + ' ' + GLYPH_COLLAPSED
    else TitleText := TitleText + ' ' + GLYPH_EXPANDED;
    AddPlainRow(BG_PANEL + FG_CYAN + ANSI_BOLD + TitleText);
    if not SesCollapsed then
    begin
...
var
  ...
  begin
    ...
    if R1 = R2 then begin A := C1 - 1; B := C2; end
    else if Row = R1 then begin A := C1 - 1; B := Length(PlainList[Row]); end
...
begin
  ...
  try
    ...
    { v0.44: reserveer ruimte voor ' ' + klok + ' ' + X. }
    TopBar := PadToWidth(TopBar,
      ScreenWidth - 2 - VisibleLength(ClockStr) - VisibleLength(XBtn) - 2, BG_BAR);
...
{ v0.44: robuuste wheel-delta + expliciete chat-routing. }
...
begin
  ...
  begin
    ...
    begin
      if Ev.MouseY < 4 + LeftSplitRow then Tgt := 30
      else Tgt := 31;
    end
    else if ShowRightPanel
...
begin
  ...
  try
    ...
    { v0.44: initiele render zodat alle MaxScroll-variabelen geldig zijn
      voordat de eerste input-poll plaatsvindt. }
