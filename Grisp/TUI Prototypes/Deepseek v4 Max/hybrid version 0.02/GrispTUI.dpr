...
const
  ...
  // Unicode symbols as codepoints (source stays ASCII)
  S_DIAMOND   = #$25C8;
  S_FDIAMOND  = #$25C6;
  S_RTRI      = #$25B8;
  S_PTRI      = #$25B6;
  ...
  BG_MAIN   = ESC + '[48;2;10;14;26m';       // #0A0E1A
  BG_PANEL  = ESC + '[48;2;13;21;36m';       // #0D1524
  BG_BAR    = ESC + '[48;2;8;12;22m';        // #080C16
  BG_INPUT  = ESC + '[48;2;16;27;48m';       // brighter than panel
  BG_USER   = ESC + '[48;2;30;58;138m';      // #1E3A8A
  BG_AI     = ESC + '[48;2;22;40;66m';       // more distinct from BG_MAIN
...
// ═══════════════════════════════════════════════════════════════════
//  Left pane — content-hugging sections
// ═══════════════════════════════════════════════════════════════════
...
begin
  ...
  try
    ...
    AddLine(BG_PANEL + FG_BORDER + ' ' + Rep(S_H, LEFT_W - 3));
    ...
    AddLine(BG_PANEL);  // single spacer
    ...
    // Any leftover rows: blank panel fill
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        SenderCol := FG_AMBER + BOLD;
        Prefix    := ' [sys]';
      end;

      // Sender row: prefix on left, timestamp right-aligned
      TimePart := '[' + M.Time + ']';
      Buf := BG_MAIN + SenderCol + Prefix;
      RightPad := CenterW - 3 - VisLen(Buf) - Length(TimePart);
      ...
      AddLine(BG_MAIN);  // spacer between messages
    end;
...
// ═══════════════════════════════════════════════════════════════════
//  Right pane — content-hugging sections
// ═══════════════════════════════════════════════════════════════════
...
begin
  L := TList<string>.Create;
  try
    // Content-hugging: each section gets exactly what it needs.
    // Headers + dividers consume 3 * 2 = 6 rows.
    Budget := Height - 6;
    ...
    // Squeeze if we exceed the budget
