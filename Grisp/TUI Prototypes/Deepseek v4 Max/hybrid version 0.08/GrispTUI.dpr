{ ============================================================================
  ...
  ----------------------------------------------------------------------------
  v0.17-wijzigingen
    • PadToWidth kapt nu af (ANSI-bewust) — geen overflow meer bij smalle
      kolommen.
    • Pane-bottom-separator gebruikt nu GLYPH_TEENORTH (┴) i.p.v. TEEAST (┬).
    • Volwaardige multi-line input editor:
        - Enter         → nieuwe regel
        - Ctrl/Alt/Shift+Enter → versturen
        - ← → ↑ ↓       → cursor verplaatsen (↑↓ volgt wrapped regels)
        - Home / End    → begin / einde van de visuele regel
        - Backspace / Delete
        - Ctrl+V        → plakken (Windows klembord)
        - F5            → muis uit → terminal selectie/kopiëren
  ============================================================================ }
...
const
  ...
  { Unicode glyphs — allemaal via codepoints, bestand blijft ASCII-only }
  GLYPH_DIAMOND   = #$25C8;
  GLYPH_FDIAMOND  = #$25C6;
  GLYPH_RTRI      = #$25B8;
  GLYPH_PTRI      = #$25B6;
  ...
  GLYPH_HORZ      = #$2500;   // ─
  GLYPH_VERT      = #$2502;   // │
  GLYPH_TEEDOWN   = #$252C;   // ┬  (top separator)
  GLYPH_TEEWEST   = #$2524;   // ┤
  GLYPH_TEEUP     = #$2534;   // ┴  (bottom separator)
  GLYPH_TEEEAST   = #$251C;   // ├
...
type
  ...
  { Eén visuele regel in de input editor. }
  TVisualLine = record
    BufStart : Integer;   // 1-based positie in InputBuffer van eerste karakter
    Text     : string;    // zichtbare tekst (zonder \n en zonder wrap)
  end;
...
var
  ...
  { Input editor }
  InputBuffer : string;      // bevat \n voor regeleinden
  InputCursor : Integer = 0; // 0..Length(InputBuffer)
  InputScroll : Integer = 0; // eerste zichtbare visuele regel
...
{ Behoudt alle ANSI-codes, kapt zichtbare tekens af bij MaxVisible. }
...
{ Pad (of kap) S naar TargetWidth zichtbare tekens. Fix voor bug 1. }
...
{ Bouw visuele regels uit InputBuffer — houdt BufStart bij voor cursor-mapping. }
...
{ Vind de visuele (rij, kolom) van Cursor binnen Lines. }
  ...
  try
    ...
    if ScreenWidth >= 150 then
      ...
      Hints := '  ' + FG_DGRAY + '[F1]R' + FG_TEXT + ' '
             ...
             + FG_TEXT + ' ' + FG_DGRAY + '[F4]S' + FG_TEXT + ' '
             + FG_DGRAY + '[F5]M' + FG_TEXT + ' ' + FG_DGRAY + '[Esc]Q'
    else Hints := '';
    ...
    { Bug-fix: bottom separator gebruikt nu GLYPH_TEEUP (┴) i.p.v. TEEAST (┬). }
    ...
    { Bepaal caret VisRow/VisCol en pas InputScroll aan. }
    ...
    begin
      { Zorg dat de caret zichtbaar is. }
...
begin
  ...
  { Enter: nieuwe regel, of versturen met modifier. }
  if Ev.Ch = #13 then
  begin
    if Ev.ModCtrl or Ev.ModAlt or Ev.ModShift then
    begin
  ...
  if (Ord(Ev.Ch) >= 32) and not SimActive and not Ev.ModCtrl and not Ev.ModAlt then
    InsertAtCaret(Ev.Ch);
end;
