...
const
  ...
  GLYPH_SCROLL    = #$2588;
  GLYPH_COLLAPSED = #$25B8;  { klein driehoekje voor ingeklapte sectie }
  GLYPH_EXPANDED  = #$25BE;
...
var
  ...
  { Collapse state per sectie — dubbelklik op titel toggelt }
  ...
  SelAnchorCol  : Integer = 1;   { 1-based inclusive }
...
begin
  ...
  if ResCollapsed and SesCollapsed then
    LeftSplitRow := 1
  else if ResCollapsed then
    LeftSplitRow := 1
  else if SesCollapsed then
    LeftSplitRow := ContentHeight - 2
  else
  begin
  ...
  { Rechterpaneel: 3 secties + 2 dividers = ContentHeight }
  TotalRight := ContentHeight;

  FixedOverhead := 2;   { 2 dividers tussen de secties }
  ExpandedCount := 0;
  if not PlanCollapsed then  begin Inc(FixedOverhead, 2); Inc(ExpandedCount); end   { title+divider }
  else Inc(FixedOverhead, 1);                                                       { title only }
  ...
  if PlanCollapsed then RightPlanRows := 1
  else
  begin
    RightPlanRows := 2 + PerBody;
    if Extra > 0 then begin Inc(RightPlanRows); Dec(Extra); end;
    if UserPlanRows > 0 then RightPlanRows := UserPlanRows;
  end;

  if WorkCollapsed then RightWorkRows := 1
  else
  begin
    RightWorkRows := 2 + PerBody;
    if Extra > 0 then begin Inc(RightWorkRows); Dec(Extra); end;
    if UserWorkRows > 0 then RightWorkRows := UserWorkRows;
  end;

  if ThinkCollapsed then RightThinkRows := 1
  else
  begin
    RightThinkRows := TotalRight - 2 - RightPlanRows - RightWorkRows;
    if RightThinkRows < 3 then RightThinkRows := 3;
  end;

  PlanNeed  := RightPlanRows;
  WorkNeed  := RightWorkRows;
  ThinkNeed := RightThinkRows;
  { laatste sanity: if sum != ContentHeight, adjust }
...
begin
  ...
  try
    ...
    // Resources header (met collapse glyph)
...
var
  ...
  BodyW : Integer;
  S1, S2, R1, R2, C1, C2, TmpI : Integer;
  ...
  begin
    ...
    { A, B zijn 0-based [start, end) offsets in de plain tekst. }
    if R1 = R2 then
    begin
      A := C1 - 1;
      B := C2;         { C2 is 1-based inclusive → 0-based exclusive = C2 }
    end
    else if Row = R1 then
    begin
      A := C1 - 1;
      B := Length(PlainList[Row]);
    end
    else if Row = R2 then
    begin
      A := 0;
...
begin
  ...
  try
    ...
    begin
      ...
      { v0.41: geen brackets meer rond timestamps. }
...
begin
  ...
  try
    ...
    begin
      PlanBodyH := RightPlanRows - 2;
      if PlanBodyH < 0 then PlanBodyH := 0;
    end;
    if not WorkCollapsed then
    begin
      WorkBodyH := RightWorkRows - 2;
      if WorkBodyH < 0 then WorkBodyH := 0;
    end;
    if not ThinkCollapsed then
    begin
      ThinkBodyH := RightThinkRows - 2;
      if ThinkBodyH < 0 then ThinkBodyH := 0;
    end;
    ...
    { THINKING — v0.41: geen [F3] hint, geen hidden state. }
...
begin
  ...
  CloseStart := ScreenWidth - 1 - XW + 1 - 1 + 1;   { = ScreenWidth - XW }
...
{ v0.41: 1-based inclusive selectie-extractie. }
function ExtractSelectionText : string;
var R1, R2, C1, C2, R, TmpI, TmpC : Integer; Plain, Trimmed : string;
begin
  Result := '';
  if not SelActive then Exit;
  R1 := SelAnchorRow; C1 := SelAnchorCol;
  ...
  begin
    ...
    if R1 = R2 then
      Result := Result + Copy(Plain, C1, C2 - C1 + 1)
    else if R = R1 then
    begin
      Result := Result + Copy(Plain, C1, MaxInt);
      Result := Result + #13#10;
    end
    else if R = R2 then
...
begin
  ...
  { v0.41: 1-based inclusive. +1 zodat de laatste kolom de laatste char raakt. }
...
{ v0.41: check of we op een sectietitel klikken. }
...
begin
  ...
  begin
    if Y = 4 then begin Result := 40; Exit; end   { RESOURCES }
    else if Y = 4 + LeftSplitRow + 1 then begin Result := 41; Exit; end;  { SESSIONS }
  end;
  { Rechterpaneel }
  if ShowRightPanel and (X >= ColLeft + ColCenter + 4) then
  begin
    if Y = 4 then begin Result := 42; Exit; end   { CURRENT PLAN }
    else if Y = 4 + RightPlanRows + 1 then begin Result := 43; Exit; end  { WORK ITEMS }
    else if Y = 4 + RightPlanRows + 1 + RightWorkRows + 1 then
    begin Result := 44; Exit; end;   { THINKING }
  end;
end;
...
begin
  ...
  if Ev.MouseButton = 1 then
  begin
    { v0.41: dubbelklik op sectietitel toggelt collapse. }
    ...
    begin
      ...
      begin
        RecomputeLayout;
        NeedsRedraw := True;
        LastClickTime := 0;  { voorkom triple-click toggling }
        Exit;
      end;
    end;
