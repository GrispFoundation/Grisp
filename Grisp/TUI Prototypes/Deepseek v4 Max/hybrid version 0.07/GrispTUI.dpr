{ ============================================================================
  GRISP OS — Terminal UI
  ----------------------------------------------------------------------------
  Een single-file Delphi-console-applicatie die een volledig functionele
  multi-pane TUI neerzet voor de GRISP/GARP runtime. Alles in true-color
  ANSI, met muis-ondersteuning, sleepbare dividers en een gesimuleerde
  GARP/1.26 prompt-levenscyclus.

  Bestandsnaam : GrispTUI.dpr
  Platform     : Windows (Winapi console), Windows Terminal aanbevolen
  Compiler     : Delphi 10.4+ (Florence getest)

  Sneltoetsen
    F1     Linkerpaneel aan/uit
    F2     Rechterpaneel aan/uit
    F3     Thinking-sectie in-/uitklappen
    F4     Verzenden of simulatie-stap
    F5     Muis aan/uit (uit = tekst selecteren mogelijk)
    F6     Alle handmatige paneelformaten resetten
    Esc    Afsluiten
    PgUp / PgDn / Home / End   Chat scrollen
  ============================================================================ }

program GrispTUI;

{$APPTYPE CONSOLE}
{$R *.res}

uses
  Winapi.Windows,
  System.SysUtils,
  System.Classes,
  System.Generics.Collections,
  System.Math;

{ ============================================================================
  SECTIE 1 — Constanten
  ============================================================================ }

const
  APP_NAME    = 'GRISP OS';
  APP_VERSION = 'v0.16';

  { --- Unicode glyphs (via codepoints, bestand blijft ASCII-only) --- }
  GLYPH_DIAMOND   = #$25C8;   // ◈  sectiekop
  GLYPH_FDIAMOND  = #$25C6;   // ◆  alternatief
  GLYPH_RTRI      = #$25B8;   // ▸  boom-item
  GLYPH_PTRI      = #$25B6;   // ▶  running-item
  GLYPH_CHECKED   = #$2611;   // ☑  plan done
  GLYPH_UNCHECKED = #$2610;   // ☐  plan queued
  GLYPH_GT        = #$203A;   // ›  thinking-prefix
  GLYPH_BLOCK     = #$258C;   // ▌  AI-bubble linkerrand
  GLYPH_DOT       = #$25CF;   // ●  status-indicator
  GLYPH_UP        = #$25B2;   // ▲  scrolled-indicator

  { --- Box drawing --- }
  GLYPH_TOPLEFT   = #$250C;   // ┌
  GLYPH_TOPRIGHT  = #$2510;   // ┐
  GLYPH_BOTLEFT   = #$2514;   // └
  GLYPH_BOTRIGHT  = #$2518;   // ┘
  GLYPH_HORZ      = #$2500;   // ─
  GLYPH_VERT      = #$2502;   // │
  GLYPH_TEEAST    = #$252C;   // ┬
  GLYPH_TEEWEST   = #$2524;   // ┤
  GLYPH_TEENORTH  = #$2534;   // ┴
  GLYPH_TEESOUTH  = #$251C;   // ├

  { --- ANSI control --- }
  ESC = #27;
  ANSI_RESET  = ESC + '[0m';
  ANSI_BOLD   = ESC + '[1m';

  { --- Voorgrondkleuren (24-bit true color) --- }
  FG_CYAN   = ESC + '[38;2;34;211;238m';   // #22D3EE  primaire accent
  FG_PURPLE = ESC + '[38;2;59;130;246m';   // #3B82F6  thinking
  FG_BORDER = ESC + '[38;2;51;65;85m';     // #334155  randen
  FG_USER   = ESC + '[38;2;147;197;253m';  // #93C5FD  user-naam
  FG_TEXT   = ESC + '[38;2;255;255;255m';  // #FFFFFF  gewone tekst
  FG_GRAY   = ESC + '[38;2;148;163;184m';  // #94A3B8  secondary
  FG_DGRAY  = ESC + '[38;2;100;116;139m';  // #64748B  tertiary
  FG_GREEN  = ESC + '[38;2;34;197;94m';    // #22C55E  success
  FG_AMBER  = ESC + '[38;2;245;158;11m';   // #F59E0B  running
  FG_SLATE  = ESC + '[38;2;56;189;248m';   // #38BDF8  accent
  FG_RED    = ESC + '[38;2;239;68;68m';    // #EF4444  error

  { --- Achtergrondkleuren --- }
  BG_MAIN  = ESC + '[48;2;10;14;26m';      // #0A0E1A  canvas
  BG_PANEL = ESC + '[48;2;13;21;36m';      // #0D1524  zijpanelen
  BG_BAR   = ESC + '[48;2;8;12;22m';       // #080C16  top/status balk
  BG_INPUT = ESC + '[48;2;16;27;48m';      // #101B30  invoerveld
  BG_USER  = ESC + '[48;2;30;58;138m';     // #1E3A8A  user-bubbel
  BG_AI    = ESC + '[48;2;22;40;66m';      // #16284 2 AI-bubbel

  { --- Cursor & scherm --- }
  CURSOR_HIDE     = ESC + '[?25l';
  CURSOR_SHOW     = ESC + '[?25h';
  SCREEN_ALT_ON   = ESC + '[?1049h';
  SCREEN_ALT_OFF  = ESC + '[?1049l';
  SCREEN_CLEAR    = ESC + '[2J' + ESC + '[H';
  CURSOR_HOME     = ESC + '[H';

  { --- Gedragsconstanten --- }
  SIM_INTERVAL_MS = 900;    // tijdsduur per simulatie-stap
  MIN_W_LEFT      = 110;    // onder deze breedte: linkerpaneel auto-uit
  MIN_W_RIGHT     = 90;     // onder deze breedte: rechterpaneel auto-uit
  MIN_PANE_WIDTH  = 20;     // minimale breedte per zijpaneel
  MIN_CHAT_WIDTH  = 30;     // minimale breedte chatkolom
  MAX_INPUT_ROWS  = 10;     // maximale hoogte van de input area

  { --- Console input event-types (eigen constanten om botsing met de
        Win32-procedure mouse_event te vermijden; Delphi is case-insensitive). }
  EVT_KEY           = $0001;
  EVT_MOUSE         = $0002;
  EVT_MOUSE_WHEELED = $0004;

{ ============================================================================
  SECTIE 2 — Types
  ============================================================================ }

type
  TChatRole = (crUser, crAssistant, crSystem);

  TChatMessage = record
    Role : TChatRole;
    Text : string;
    Time : string;
  end;

  TPlanStatus = (psDone, psRunning, psQueued);

  TPlanItem = record
    Text   : string;
    Status : TPlanStatus;
  end;

  TWorkPriority = (wpHigh, wpMedium, wpLow);

  TWorkItem = record
    Text : string;
    Prio : TWorkPriority;
  end;

  TSession = record
    Name : string;
    Meta : string;
  end;

  TTreeItem = record
    Name  : string;
    Depth : Integer;
    Perm  : string;
  end;

  TInputKind = (ikNone, ikChar, ikSpecial, ikMouse);

  TInputEvent = record
    Kind        : TInputKind;
    Ch          : WideChar;
    VK          : Word;
    MouseX      : Integer;   // 1-based schermkolom
    MouseY      : Integer;   // 1-based schermrij
    MouseButton : Integer;   // 0 = geen, 1 = links, 2 = rechts
    WheelDelta  : Integer;   // +120 per klik omhoog
  end;

{ ============================================================================
  SECTIE 3 — Globale toestand
  ============================================================================ }

var
  { Console handles }
  StdOutHandle : THandle;
  StdInHandle  : THandle;

  { Actueel schermformaat en kolombreedtes (1-based) }
  ScreenWidth  : Integer;
  ScreenHeight : Integer;
  ColLeft      : Integer;   // breedte van het linkerpaneel
  ColCenter    : Integer;   // breedte van de chatkolom
  ColRight     : Integer;   // breedte van het rechterpaneel

  { Verticale indeling van de hoofdpanelen }
  ContentHeight : Integer;  // aantal rijen voor de 3 panelen samen
  InputHeight   : Integer;  // aantal rijen van de input area (1..MAX_INPUT_ROWS)

  { Horizontale indeling binnen de zijpanelen }
  LeftSplitRow  : Integer;  // rij-index (0-based binnen ContentHeight) van de
                            // divider tussen RESOURCES en SESSIONS
  RightPlanRows : Integer;  // hoogte van CURRENT PLAN
  RightWorkRows : Integer;  // hoogte van WORK ITEMS
                            // (Thinking krijgt de rest)

  { Applicatiedata }
  ChatMessages  : TList<TChatMessage>;
  PlanItems     : TList<TPlanItem>;
  WorkItems     : TList<TWorkItem>;
  ThinkingLines : TList<string>;
  SessionList   : TList<TSession>;
  TreeList      : TList<TTreeItem>;

  CurrentSessionName : string;
  ActiveSessionIndex : Integer = 0;

  { Invoerveld }
  InputBuffer : string;

  { Paneelzichtbaarheid — gescheiden in wens (gebruiker) en effectief
    (berekend per frame op basis van schermbreedte). }
  WantLeftPanel  : Boolean = True;
  WantRightPanel : Boolean = True;
  WantThinking   : Boolean = True;
  MouseEnabled   : Boolean = True;

  ShowLeftPanel  : Boolean = True;   // effectief
  ShowRightPanel : Boolean = True;   // effectief
  ShowThinking   : Boolean = True;   // effectief

  { Door gebruiker gezette afmetingen (0 = automatisch) }
  UserColLeft    : Integer = 0;
  UserColRight   : Integer = 0;
  UserInputRows  : Integer = 0;
  UserLeftSplit  : Integer = 0;
  UserPlanRows   : Integer = 0;
  UserWorkRows   : Integer = 0;

  { Drag-state voor sleepbare dividers }
  IsDragging    : Boolean = False;
  DragTarget    : Integer = 0;   // 1=L|C, 2=C|R, 10=input boven, 11=L-split,
                                 // 12=R-plan/work, 13=R-work/think
  HoverTarget   : Integer = 0;   // zelfde codering als DragTarget

  { Chat scroll }
  ChatScroll    : Integer = 0;   // regels terug vanaf onderkant
  ChatMaxScroll : Integer = 0;

  { Simulatie }
  QuitRequested : Boolean = False;
  SimActive     : Boolean = False;
  SimStep       : Integer = 0;
  SimTimer      : Cardinal = 0;
  PromptState   : string = 'READY';
  StatusKind    : Integer = 0;    // 0=groen, 1=amber, 2=rood

{ ============================================================================
  SECTIE 4 — Console plumbing
  ============================================================================ }

procedure ReadTerminalSize(out W, H: Integer);
var
  Info : TConsoleScreenBufferInfo;
begin
  if GetConsoleScreenBufferInfo(StdOutHandle, Info) then
  begin
    W := Info.srWindow.Right  - Info.srWindow.Left + 1;
    H := Info.srWindow.Bottom - Info.srWindow.Top  + 1;
  end
  else
  begin
    W := 140;
    H := 40;
  end;

  if W < 60 then W := 60;
  if H < 20 then H := 20;
end;

procedure WriteAtomic(const S : string);
var
  Written : DWORD;
begin
  if S = '' then Exit;
  WriteConsoleW(StdOutHandle, PWideChar(S), Length(S), Written, nil);
end;

procedure ApplyInputMode;
var
  Mode : DWORD;
begin
  GetConsoleMode(StdInHandle, Mode);
  Mode := Mode or ENABLE_EXTENDED_FLAGS or ENABLE_WINDOW_INPUT;

  if MouseEnabled then
    Mode := (Mode or ENABLE_MOUSE_INPUT) and not ENABLE_QUICK_EDIT_MODE
  else
    Mode := (Mode and not ENABLE_MOUSE_INPUT) or ENABLE_QUICK_EDIT_MODE;

  SetConsoleMode(StdInHandle, Mode);
end;

procedure InitializeConsole;
var
  Mode : DWORD;
begin
  StdOutHandle := GetStdHandle(STD_OUTPUT_HANDLE);
  StdInHandle  := GetStdHandle(STD_INPUT_HANDLE);

  { VT-processing aan, wrap-at-EOL uit (voorkomt lijn-verschuivingen). }
  GetConsoleMode(StdOutHandle, Mode);
  SetConsoleMode(StdOutHandle,
    (Mode or ENABLE_VIRTUAL_TERMINAL_PROCESSING or ENABLE_PROCESSED_OUTPUT)
    and not ENABLE_WRAP_AT_EOL_OUTPUT);

  SetConsoleOutputCP(CP_UTF8);
  SetConsoleCP(CP_UTF8);
  SetConsoleTitle(PChar(APP_NAME + ' ' + APP_VERSION + ' — Terminal UI'));

  ApplyInputMode;
end;

function ReadInputEvent : TInputEvent;
var
  Available  : DWORD;
  Record_    : INPUT_RECORD;
  Read       : DWORD;
  ButtonState: DWORD;
  WheelRaw   : SmallInt;
begin
  Result.Kind := ikNone;

  while True do
  begin
    Available := 0;
    if not GetNumberOfConsoleInputEvents(StdInHandle, Available) then Exit;
    if Available = 0 then Exit;
    if not ReadConsoleInputW(StdInHandle, Record_, 1, Read) then Exit;

    { --- Toetsaanslagen --- }
    if Record_.EventType = EVT_KEY then
    begin
      if not Record_.Event.KeyEvent.bKeyDown then Continue;

      Result.VK := Record_.Event.KeyEvent.wVirtualKeyCode;
      Result.Ch := Record_.Event.KeyEvent.UnicodeChar;

      if (Result.Ch = #0) and (Result.VK <> 0) then
        Result.Kind := ikSpecial
      else if Result.Ch <> #0 then
        Result.Kind := ikChar
      else
        Result.Kind := ikNone;

      Exit;
    end;

    { --- Muisgebeurtenissen --- }
    if Record_.EventType = EVT_MOUSE then
    begin
      ButtonState := Record_.Event.MouseEvent.dwButtonState;

      Result.Kind := ikMouse;
      Result.MouseX := Record_.Event.MouseEvent.dwMousePosition.X + 1;
      Result.MouseY := Record_.Event.MouseEvent.dwMousePosition.Y + 1;

      if (ButtonState and FROM_LEFT_1ST_BUTTON_PRESSED) <> 0 then
        Result.MouseButton := 1
      else if (ButtonState and RIGHTMOST_BUTTON_PRESSED) <> 0 then
        Result.MouseButton := 2
      else
        Result.MouseButton := 0;

      if (Record_.Event.MouseEvent.dwEventFlags and EVT_MOUSE_WHEELED) <> 0 then
      begin
        WheelRaw := SmallInt(ButtonState shr 16);
        Result.WheelDelta := WheelRaw;
      end
      else
        Result.WheelDelta := 0;

      Exit;
    end;

    { Onbekende gebeurtenissen negeren, doorgaan met lezen. }
  end;
end;

{ ============================================================================
  SECTIE 5 — Tekst-helpers
  ============================================================================ }

{ Zichtbare lengte van een string, ANSI-escape-sequenties niet meegerekend. }
function VisibleLength(const S : string) : Integer;
var
  I       : Integer;
  InEscape: Boolean;
begin
  Result := 0;
  InEscape := False;

  for I := 1 to Length(S) do
  begin
    if InEscape then
    begin
      if CharInSet(S[I], ['a'..'z', 'A'..'Z']) then
        InEscape := False;
    end
    else if S[I] = #27 then
      InEscape := True
    else
      Inc(Result);
  end;
end;

{ Vul S aan tot TargetWidth zichtbare tekens, met PadBackground als achtergrond. }
function PadToWidth(const S : string; TargetWidth : Integer;
                    const PadBackground : string) : string;
var
  Visible : Integer;
begin
  Visible := VisibleLength(S);

  if Visible >= TargetWidth then
    Result := S
  else
    Result := S + ANSI_RESET + PadBackground +
              StringOfChar(' ', TargetWidth - Visible);
end;

{ Eenvoudige word-wrap. Splitst op spaties; lange woorden worden hard
  afgebroken op MaxWidth. Geeft altijd minstens één regel terug. }
function WrapParagraph(const Text : string; MaxWidth : Integer) : TArray<string>;
var
  Lines : TList<string>;
  Words : TArray<string>;
  Cur   : string;
  I     : Integer;
begin
  Lines := TList<string>.Create;
  try
    Words := Text.Split([' ']);
    Cur := '';

    for I := 0 to High(Words) do
    begin
      if Cur = '' then
        Cur := Words[I]
      else if Length(Cur) + 1 + Length(Words[I]) <= MaxWidth then
        Cur := Cur + ' ' + Words[I]
      else
      begin
        Lines.Add(Cur);
        Cur := Words[I];
      end;
    end;

    if Cur <> '' then Lines.Add(Cur);
    if Lines.Count = 0 then Lines.Add('');

    Result := Lines.ToArray;
  finally
    Lines.Free;
  end;
end;

{ Multi-line wrap: splitst eerst op #10, wrapt elke regel apart. }
function WrapMultiline(const Text : string; MaxWidth : Integer) : TArray<string>;
var
  Segments  : TArray<string>;
  Output    : TList<string>;
  WrappedSeg: TArray<string>;
  I, J      : Integer;
begin
  Output := TList<string>.Create;
  try
    Segments := Text.Split([#10]);

    for I := 0 to High(Segments) do
    begin
      WrappedSeg := WrapParagraph(Segments[I], MaxWidth);
      for J := 0 to High(WrappedSeg) do
        Output.Add(WrappedSeg[J]);
    end;

    if Output.Count = 0 then Output.Add('');

    Result := Output.ToArray;
  finally
    Output.Free;
  end;
end;

function CurrentShortTime : string;
begin
  Result := FormatDateTime('hh:nn', Now);
end;

function CurrentLongTime : string;
var
  Y, M, D     : Word;
  H, Mn, S, Ms: Word;
  Hour12      : Word;
  AmPm        : string;
begin
  DecodeDate(Now, Y, M, D);
  DecodeTime(Now, H, Mn, S, Ms);

  if H < 12 then AmPm := 'am' else AmPm := 'pm';
  Hour12 := H mod 12;
  if Hour12 = 0 then Hour12 := 12;

  Result := Format('%d %s %d, %.2d:%.2d:%.2d %s',
    [D, FormatSettings.LongMonthNames[M], Y, Hour12, Mn, S, AmPm]);
end;

function RepeatChar(const C : Char; N : Integer) : string;
begin
  if N <= 0 then Result := ''
  else Result := StringOfChar(C, N);
end;

{ ============================================================================
  SECTIE 6 — Layout-berekening
  ============================================================================ }

{ Berekent ColLeft/ColCenter/ColRight, ContentHeight, InputHeight, en de
  horizontale paneel-splitsingen op basis van het huidige schermformaat en
  de door de gebruiker gezette afmetingen. Wordt aangeroepen bij startup,
  bij elke resize, en na elke drag-operatie. }
procedure RecomputeLayout;
var
  AvailableRows : Integer;
begin
  ReadTerminalSize(ScreenWidth, ScreenHeight);

  { Paneelzichtbaarheid: gebruiker wil + minimale breedte. }
  ShowLeftPanel  := WantLeftPanel  and (ScreenWidth >= MIN_W_LEFT);
  ShowRightPanel := WantRightPanel and (ScreenWidth >= MIN_W_RIGHT);
  ShowThinking   := WantThinking;

  { Kolombreedtes: gebruikerswaarde heeft voorrang, anders automatisch. }
  if ShowLeftPanel then
  begin
    if UserColLeft > 0 then ColLeft := UserColLeft
    else ColLeft := EnsureRange(ScreenWidth div 7, 22, 28);
    if ColLeft < MIN_PANE_WIDTH then ColLeft := MIN_PANE_WIDTH;
  end
  else
    ColLeft := 0;

  if ShowRightPanel then
  begin
    if UserColRight > 0 then ColRight := UserColRight
    else ColRight := EnsureRange(ScreenWidth div 5, 30, 40);
    if ColRight < MIN_PANE_WIDTH then ColRight := MIN_PANE_WIDTH;
  end
  else
    ColRight := 0;

  ColCenter := ScreenWidth - ColLeft - ColRight - 4;

  { Als de chat te smal wordt: eerst rechterpaneel krimpen, dan linker. }
  if ColCenter < MIN_CHAT_WIDTH then
  begin
    if ShowRightPanel then
    begin
      ColRight := ColRight - (MIN_CHAT_WIDTH - ColCenter);
      if ColRight < MIN_PANE_WIDTH then ColRight := MIN_PANE_WIDTH;
      ColCenter := ScreenWidth - ColLeft - ColRight - 4;
    end;

    if (ColCenter < MIN_CHAT_WIDTH) and ShowLeftPanel then
    begin
      ColLeft := ColLeft - (MIN_CHAT_WIDTH - ColCenter);
      if ColLeft < MIN_PANE_WIDTH then ColLeft := MIN_PANE_WIDTH;
      ColCenter := ScreenWidth - ColLeft - ColRight - 4;
    end;

    if ColCenter < 10 then ColCenter := 10;
  end;

  { Hoogte van de input area }
  if UserInputRows > 0 then InputHeight := UserInputRows
  else InputHeight := 1;

  if InputHeight < 1 then InputHeight := 1;
  if InputHeight > MAX_INPUT_ROWS then InputHeight := MAX_INPUT_ROWS;
  if InputHeight > ScreenHeight div 3 then InputHeight := ScreenHeight div 3;

  { ContentHeight = totaal - 7 (topbar+borders) - InputHeight }
  ContentHeight := ScreenHeight - 7 - InputHeight;
  if ContentHeight < 6 then
  begin
    ContentHeight := 6;
    InputHeight := ScreenHeight - 7 - ContentHeight;
    if InputHeight < 1 then InputHeight := 1;
  end;

  { Linkerpaneel: Resources + divider + Sessions = ContentHeight }
  if UserLeftSplit > 0 then LeftSplitRow := UserLeftSplit
  else LeftSplitRow := (ContentHeight - 1) div 2;

  if LeftSplitRow < 3 then LeftSplitRow := 3;
  if LeftSplitRow > ContentHeight - 4 then LeftSplitRow := ContentHeight - 4;

  { Rechterpaneel: Plan + divider + Work + divider + Think = ContentHeight }
  AvailableRows := ContentHeight - 2;

  if UserPlanRows > 0 then RightPlanRows := UserPlanRows
  else RightPlanRows := AvailableRows div 3;

  if UserWorkRows > 0 then RightWorkRows := UserWorkRows
  else RightWorkRows := AvailableRows div 3;

  if RightPlanRows < 3 then RightPlanRows := 3;
  if RightWorkRows < 3 then RightWorkRows := 3;

  while (RightPlanRows + RightWorkRows) > AvailableRows - 3 do
  begin
    if RightWorkRows > 3 then Dec(RightWorkRows)
    else if RightPlanRows > 3 then Dec(RightPlanRows)
    else Break;
  end;
end;

{ ============================================================================
  SECTIE 7 — Applicatiedata
  ============================================================================ }

procedure InitializeApplicationData;
var
  Session : TSession;
  Message_ : TChatMessage;
  Plan    : TPlanItem;
  Work    : TWorkItem;
  Tree    : TTreeItem;
begin
  ChatMessages  := TList<TChatMessage>.Create;
  PlanItems     := TList<TPlanItem>.Create;
  WorkItems     := TList<TWorkItem>.Create;
  ThinkingLines := TList<string>.Create;
  SessionList   := TList<TSession>.Create;
  TreeList      := TList<TTreeItem>.Create;

  CurrentSessionName := 'Architecture Review';

  { --- Resources boom --- }
  Tree.Name := 'Local';         Tree.Depth := 0; Tree.Perm := '';    TreeList.Add(Tree);
  Tree.Name := 'Projects';      Tree.Depth := 1; Tree.Perm := 'RWX'; TreeList.Add(Tree);
  Tree.Name := 'GRISP';         Tree.Depth := 2; Tree.Perm := 'RWX'; TreeList.Add(Tree);
  Tree.Name := 'MME-Registry';  Tree.Depth := 2; Tree.Perm := 'RW';  TreeList.Add(Tree);
  Tree.Name := 'Agent-Network'; Tree.Depth := 2; Tree.Perm := 'RW';  TreeList.Add(Tree);
  Tree.Name := 'Data';          Tree.Depth := 1; Tree.Perm := 'RW';  TreeList.Add(Tree);
  Tree.Name := 'Models';        Tree.Depth := 2; Tree.Perm := 'RW';  TreeList.Add(Tree);
  Tree.Name := 'Downloads';     Tree.Depth := 2; Tree.Perm := 'R';   TreeList.Add(Tree);
  Tree.Name := 'Cloud';         Tree.Depth := 1; Tree.Perm := 'R';   TreeList.Add(Tree);

  { --- Sessies --- }
  Session.Name := 'Architecture Review'; Session.Meta := '24 Sep - 14:30'; SessionList.Add(Session);
  Session.Name := 'MME Schema Draft';    Session.Meta := '24 Sep - 12:10'; SessionList.Add(Session);
  Session.Name := 'Agent Graph';         Session.Meta := '23 Sep - 18:22'; SessionList.Add(Session);
  Session.Name := 'VFS Refactor Notes';  Session.Meta := '23 Sep - 09:15'; SessionList.Add(Session);

  { --- Begroeting --- }
  Message_.Role := crAssistant;
  Message_.Text := 'Hello Alex, how can I help you with GRISP today?';
  Message_.Time := CurrentShortTime;
  ChatMessages.Add(Message_);

  Message_.Text := 'I can help with analysis, planning, or code generation. Just ask.';
  Message_.Time := CurrentShortTime;
  ChatMessages.Add(Message_);

  { --- Plan items --- }
  Plan.Text := 'Analyze VFS Layer';           Plan.Status := psDone;    PlanItems.Add(Plan);
  Plan.Text := 'Extract MME schema';          Plan.Status := psDone;    PlanItems.Add(Plan);
  Plan.Text := 'Map Agent Network';           Plan.Status := psRunning; PlanItems.Add(Plan);
  Plan.Text := 'Draft DNA report';            Plan.Status := psQueued;  PlanItems.Add(Plan);
  Plan.Text := 'Validate permissions matrix'; Plan.Status := psQueued;  PlanItems.Add(Plan);

  { --- Work items --- }
  Work.Text := 'Implement GRISP runtime'; Work.Prio := wpHigh;   WorkItems.Add(Work);
  Work.Text := 'Add graph visualization'; Work.Prio := wpMedium; WorkItems.Add(Work);
  Work.Text := 'Write unit tests';        Work.Prio := wpMedium; WorkItems.Add(Work);
  Work.Text := 'Update LSBP integration'; Work.Prio := wpLow;    WorkItems.Add(Work);

  { --- Thinking stream --- }
  ThinkingLines.Add('Waiting for user input...');
  ThinkingLines.Add('Parsing GRISP environment...');
  ThinkingLines.Add('Ready to assist.');
end;

procedure ShutdownApplicationData;
begin
  ChatMessages.Free;
  PlanItems.Free;
  WorkItems.Free;
  ThinkingLines.Free;
  SessionList.Free;
  TreeList.Free;
end;

{ ============================================================================
  SECTIE 8 — Paneel-rendering
  ----------------------------------------------------------------------------
  Elke Render-functie geeft EXACT ContentHeight regels terug, elk met
  exact de juiste kolombreedte (via PadToWidth). Dit garandeert dat de
  verticale randen op alle rijen exact uitlijnen.
  ============================================================================ }

{ ---- Linkerpaneel: RESOURCES boven, SESSIONS onder ---- }
function RenderLeftPane : TArray<string>;
var
  Lines     : TList<string>;
  I         : Integer;
  Indent    : Integer;
  NameWidth : Integer;
  SessRows  : Integer;
  Buf       : string;
  PermColor : string;
  NameField : string;
  DividerCol: string;
  Item      : TTreeItem;
  Session   : TSession;

  procedure AddLine(const S : string);
  begin
    Lines.Add(PadToWidth(S, ColLeft, BG_PANEL) + ANSI_RESET);
  end;

begin
  Lines := TList<string>.Create;
  try
    SessRows := ContentHeight - 1 - LeftSplitRow;

    { --- RESOURCES sectie --- }
    AddLine(BG_PANEL + FG_CYAN + ANSI_BOLD + ' ' + GLYPH_FDIAMOND + ' RESOURCES');
    AddLine(BG_PANEL + FG_BORDER + ' ' + RepeatChar(GLYPH_HORZ, ColLeft - 2));

    { Loop gaat tot LeftSplitRow-3: 2 header-rijen + divider = 3 overhead. }
    for I := 0 to LeftSplitRow - 3 do
    begin
      if I < TreeList.Count then
      begin
        Item := TreeList[I];
        Indent := 1 + Item.Depth * 2;
        NameWidth := ColLeft - Indent - 8;
        if NameWidth < 4 then NameWidth := 4;

        if Item.Perm = '' then
          Buf := BG_PANEL + FG_CYAN + StringOfChar(' ', Indent)
               + GLYPH_RTRI + ' ' + ANSI_BOLD + Item.Name
        else
        begin
          if Item.Perm = 'RWX' then PermColor := FG_CYAN
          else if Item.Perm = 'RW' then PermColor := FG_AMBER
          else PermColor := FG_GRAY;

          NameField := Item.Name;
          if Length(NameField) > NameWidth then
            NameField := Copy(NameField, 1, NameWidth);
          NameField := NameField + StringOfChar(' ', NameWidth - Length(NameField));

          Buf := BG_PANEL + StringOfChar(' ', Indent) + FG_TEXT + GLYPH_RTRI + ' '
               + NameField + ' ' + PermColor + Item.Perm;
        end;

        AddLine(Buf);
      end
      else
        AddLine(BG_PANEL);
    end;

    { Vul tot aan de divider-rij. }
    while Lines.Count < LeftSplitRow do
      AddLine(BG_PANEL);

    { Divider (oplichtend bij hover/drag). }
    if (HoverTarget = 11) or (IsDragging and (DragTarget = 11)) then
      DividerCol := FG_CYAN
    else
      DividerCol := FG_BORDER;

    Lines.Add(BG_PANEL + DividerCol + RepeatChar(GLYPH_HORZ, ColLeft) + ANSI_RESET);

    { --- SESSIONS sectie --- }
    AddLine(BG_PANEL + FG_CYAN + ANSI_BOLD + ' ' + GLYPH_FDIAMOND + ' SESSIONS');
    AddLine(BG_PANEL + FG_BORDER + ' ' + RepeatChar(GLYPH_HORZ, ColLeft - 2));

    for I := 0 to SessRows - 3 do
    begin
      if I < SessionList.Count then
      begin
        Session := SessionList[I];

        if I = ActiveSessionIndex then
          Buf := BG_PANEL + FG_CYAN + ANSI_BOLD + ' ' + GLYPH_RTRI + ' ' + Session.Name
        else
          Buf := BG_PANEL + FG_TEXT + ' ' + GLYPH_RTRI + ' ' + Session.Name;

        AddLine(Buf);
        AddLine(BG_PANEL + FG_GRAY + '    ' + Session.Meta);
      end
      else
        AddLine(BG_PANEL);
    end;

    while Lines.Count < ContentHeight do
      AddLine(BG_PANEL);

    Result := Lines.ToArray;
  finally
    Lines.Free;
  end;
end;

{ ---- Middenpaneel: chat met scroll-venster ---- }
function RenderCenterPane : TArray<string>;
var
  VisibleLines : TList<string>;
  FullBuffer   : TList<string>;
  I, J         : Integer;
  TotalLines   : Integer;
  WindowStart  : Integer;
  WindowEnd    : Integer;
  BodyHeight   : Integer;
  Msg          : TChatMessage;
  Wrapped      : TArray<string>;
  Buf          : string;
  SenderColor  : string;
  Prefix       : string;
  TimePart     : string;
  RightPad     : Integer;
  Header       : string;

  procedure Emit(const S : string);
  begin
    VisibleLines.Add(PadToWidth(S, ColCenter, BG_MAIN) + ANSI_RESET);
  end;

begin
  VisibleLines := TList<string>.Create;
  FullBuffer   := TList<string>.Create;
  try
    { --- Vaste kop --- }
    Header := BG_MAIN + ' ' + FG_CYAN + ANSI_BOLD + GLYPH_FDIAMOND + ' CHAT'
            + ANSI_RESET + BG_MAIN + FG_DGRAY + ' - ' + FG_TEXT + CurrentSessionName;

    if ChatScroll > 0 then
      Header := Header + ANSI_RESET + BG_MAIN + '  ' + FG_AMBER + GLYPH_UP
              + ' scrolled +' + IntToStr(ChatScroll);

    VisibleLines.Add(PadToWidth(Header, ColCenter, BG_MAIN) + ANSI_RESET);
    VisibleLines.Add(PadToWidth(BG_MAIN + ' ' + FG_BORDER
      + RepeatChar(GLYPH_HORZ, ColCenter - 2), ColCenter, BG_MAIN) + ANSI_RESET);

    { --- Volledige berichtenbuffer opbouwen --- }
    for I := 0 to ChatMessages.Count - 1 do
    begin
      Msg := ChatMessages[I];

      case Msg.Role of
        crUser:
          begin
            SenderColor := FG_USER + ANSI_BOLD;
            Prefix := ' You';
          end;
        crAssistant:
          begin
            SenderColor := FG_CYAN + ANSI_BOLD;
            Prefix := ' ' + GLYPH_DIAMOND + ' GRISP Assistant';
          end;
        else
          begin
            SenderColor := FG_AMBER + ANSI_BOLD;
            Prefix := ' [sys]';
          end;
      end;

      { Senderreg met tijdstempel rechts uitgelijnd. }
      TimePart := '[' + Msg.Time + ']';
      Buf := BG_MAIN + SenderColor + Prefix;
      RightPad := ColCenter - 1 - VisibleLength(Buf) - Length(TimePart);
      if RightPad < 1 then RightPad := 1;
      Buf := Buf + StringOfChar(' ', RightPad) + FG_DGRAY + TimePart + ' ';
      FullBuffer.Add(Buf);

      { Body-regels. }
      Wrapped := WrapParagraph(Msg.Text, ColCenter - 8);
      for J := 0 to High(Wrapped) do
      begin
        case Msg.Role of
          crUser:
            Buf := BG_MAIN + '  ' + BG_USER + FG_TEXT + '  ' + Wrapped[J]
                 + '  ' + ANSI_RESET + BG_MAIN;
          crAssistant:
            Buf := BG_MAIN + '  ' + BG_AI + FG_SLATE + GLYPH_BLOCK
                 + BG_AI + FG_TEXT + ' ' + Wrapped[J] + '  ' + ANSI_RESET + BG_MAIN;
          else
            Buf := BG_MAIN + '  ' + FG_GRAY + '    ' + Wrapped[J];
        end;
        FullBuffer.Add(Buf);
      end;

      FullBuffer.Add(BG_MAIN);
    end;

    { --- Scrollvenster berekenen --- }
    BodyHeight := ContentHeight - 2;
    if BodyHeight < 1 then BodyHeight := 1;

    TotalLines := FullBuffer.Count;
    if TotalLines > BodyHeight then
      ChatMaxScroll := TotalLines - BodyHeight
    else
      ChatMaxScroll := 0;

    if ChatScroll > ChatMaxScroll then ChatScroll := ChatMaxScroll;
    if ChatScroll < 0 then ChatScroll := 0;

    WindowEnd := TotalLines - ChatScroll;
    if WindowEnd < 0 then WindowEnd := 0;

    WindowStart := WindowEnd - BodyHeight;
    if WindowStart < 0 then WindowStart := 0;

    for I := WindowStart to WindowEnd - 1 do
      Emit(FullBuffer[I]);

    while VisibleLines.Count < ContentHeight do
      Emit(BG_MAIN);

    Result := VisibleLines.ToArray;
  finally
    VisibleLines.Free;
    FullBuffer.Free;
  end;
end;

{ ---- Rechterpaneel: CURRENT PLAN / WORK ITEMS / THINKING ---- }
function RenderRightPane : TArray<string>;
var
  Lines         : TList<string>;
  I             : Integer;
  ThinkingRows  : Integer;
  DividerRow1   : Integer;
  DividerRow2   : Integer;
  Buf           : string;
  Symbol        : string;
  SymbolColor   : string;
  PriorityColor : string;
  PriorityLabel : string;
  DividerColor1 : string;
  DividerColor2 : string;
  Plan          : TPlanItem;
  Work          : TWorkItem;

  procedure AddLine(const S : string);
  begin
    Lines.Add(PadToWidth(S, ColRight, BG_PANEL) + ANSI_RESET);
  end;

begin
  Lines := TList<string>.Create;
  try
    ThinkingRows := ContentHeight - 2 - RightPlanRows - RightWorkRows;
    if ThinkingRows < 3 then ThinkingRows := 3;

    DividerRow1 := RightPlanRows;
    DividerRow2 := RightPlanRows + 1 + RightWorkRows;

    if (HoverTarget = 12) or (IsDragging and (DragTarget = 12)) then
      DividerColor1 := FG_CYAN
    else
      DividerColor1 := FG_BORDER;

    if (HoverTarget = 13) or (IsDragging and (DragTarget = 13)) then
      DividerColor2 := FG_CYAN
    else
      DividerColor2 := FG_BORDER;

    { --- CURRENT PLAN --- }
    AddLine(BG_PANEL + FG_CYAN + ANSI_BOLD + ' ' + GLYPH_FDIAMOND + ' CURRENT PLAN');
    AddLine(BG_PANEL + FG_BORDER + ' ' + RepeatChar(GLYPH_HORZ, ColRight - 2));

    for I := 0 to RightPlanRows - 3 do
    begin
      if I < PlanItems.Count then
      begin
        Plan := PlanItems[I];

        case Plan.Status of
          psDone:
            begin
              Symbol := GLYPH_CHECKED;
              SymbolColor := FG_GREEN;
            end;
          psRunning:
            begin
              Symbol := GLYPH_PTRI;
              SymbolColor := FG_AMBER;
            end;
          else
            begin
              Symbol := GLYPH_UNCHECKED;
              SymbolColor := FG_DGRAY;
            end;
        end;

        Buf := BG_PANEL + ' ' + SymbolColor + Symbol + ' ' + FG_TEXT + Plan.Text;
      end
      else
        Buf := BG_PANEL;

      AddLine(Buf);
    end;

    while Lines.Count < DividerRow1 do
      AddLine(BG_PANEL);

    { Divider 1 (Plan/Work). }
    Lines.Add(BG_PANEL + DividerColor1 + RepeatChar(GLYPH_HORZ, ColRight) + ANSI_RESET);

    { --- WORK ITEMS --- }
    AddLine(BG_PANEL + FG_CYAN + ANSI_BOLD + ' ' + GLYPH_FDIAMOND + ' WORK ITEMS');
    AddLine(BG_PANEL + FG_BORDER + ' ' + RepeatChar(GLYPH_HORZ, ColRight - 2));

    for I := 0 to RightWorkRows - 3 do
    begin
      if I < WorkItems.Count then
      begin
        Work := WorkItems[I];

        case Work.Prio of
          wpHigh:
            begin
              PriorityColor := FG_RED;
              PriorityLabel := '[HIGH]';
            end;
          wpMedium:
            begin
              PriorityColor := FG_AMBER;
              PriorityLabel := '[MED ]';
            end;
          else
            begin
              PriorityColor := FG_GREEN;
              PriorityLabel := '[LOW ]';
            end;
        end;

        Buf := BG_PANEL + ' ' + FG_TEXT + Work.Text + '  '
             + PriorityColor + PriorityLabel;
      end
      else
        Buf := BG_PANEL;

      AddLine(Buf);
    end;

    while Lines.Count < DividerRow2 do
      AddLine(BG_PANEL);

    { Divider 2 (Work/Think). }
    Lines.Add(BG_PANEL + DividerColor2 + RepeatChar(GLYPH_HORZ, ColRight) + ANSI_RESET);

    { --- THINKING --- }
    if ShowThinking then
      AddLine(BG_PANEL + FG_PURPLE + ANSI_BOLD + ' ' + GLYPH_FDIAMOND
            + ' THINKING  ' + FG_DGRAY + '[F3]')
    else
      AddLine(BG_PANEL + FG_DGRAY + ' ' + GLYPH_FDIAMOND
            + ' THINKING  [F3 - hidden]');

    AddLine(BG_PANEL + FG_BORDER + ' ' + RepeatChar(GLYPH_HORZ, ColRight - 2));

    for I := 0 to ThinkingRows - 3 do
    begin
      if ShowThinking and (I < ThinkingLines.Count) then
        Buf := BG_PANEL + ' ' + FG_BORDER + GLYPH_GT + ' '
             + FG_GRAY + ThinkingLines[I]
      else
        Buf := BG_PANEL;

      AddLine(Buf);
    end;

    while Lines.Count < ContentHeight do
      AddLine(BG_PANEL);

    Result := Lines.ToArray;
  finally
    Lines.Free;
  end;
end;

{ ============================================================================
  SECTIE 9 — Volledige frame samenstellen
  ----------------------------------------------------------------------------
  Rij-indeling (1-based):
    1                    bovenrand
    2                    topbar (titel + klok)
    3                    paneel-top-separator
    4..(4+ContentHeight-1)      drie panelen naast elkaar
    (4+ContentHeight)           paneel-bottom-separator (sleepbaar)
    (5+ContentHeight)..(5+ContentHeight+InputHeight-1)   input area
    (5+ContentHeight+InputHeight)                        status-separator
    (6+ContentHeight+InputHeight)                        statusbalk
    (7+ContentHeight+InputHeight)                        onderrand
  Totaal = ContentHeight + InputHeight + 7 = ScreenHeight.
  ============================================================================ }
function RenderFrame : string;
var
  SB              : TStringBuilder;
  LeftPane        : TArray<string>;
  CenterPane      : TArray<string>;
  RightPane       : TArray<string>;
  I               : Integer;
  RowIndex        : Integer;
  CaretRow        : Integer;
  CaretCol        : Integer;
  AvailInputWidth : Integer;
  VisualScroll    : Integer;
  LineIdx         : Integer;
  FirstInputRow   : Integer;
  TitleLine       : string;
  TopBar          : string;
  SeparatorTop    : string;
  SeparatorBottom : string;
  InputLine       : string;
  StatusLine      : string;
  MouseLabel      : string;
  Hints           : string;
  ClockString     : string;
  SepColorLeft    : string;
  SepColorRight   : string;
  SepColorBottom  : string;
  AllInputLines   : TArray<string>;
  LineText        : string;
  PromptPrefix    : string;

begin
  SB := TStringBuilder.Create;
  try
    SB.Append(CURSOR_HIDE);
    SB.Append(CURSOR_HOME);

    { --- Kleuren van de sleepbare scheiders --- }
    SepColorLeft := FG_BORDER;
    if (HoverTarget = 1) or (IsDragging and (DragTarget = 1)) then
      SepColorLeft := FG_CYAN;

    SepColorRight := FG_BORDER;
    if (HoverTarget = 2) or (IsDragging and (DragTarget = 2)) then
      SepColorRight := FG_CYAN;

    { --- Rij 1: bovenrand --- }
    SB.Append(FG_BORDER + GLYPH_TOPLEFT
            + RepeatChar(GLYPH_HORZ, ScreenWidth - 2)
            + GLYPH_TOPRIGHT + ANSI_RESET + #13#10);

    { --- Rij 2: topbar --- }
    if ScreenWidth >= 150 then
      Hints := '   ' + FG_DGRAY + '[F1]' + FG_TEXT + ' Res'
             + '  ' + FG_DGRAY + '[F2]' + FG_TEXT + ' Right'
             + '  ' + FG_DGRAY + '[F3]' + FG_TEXT + ' Think'
             + '  ' + FG_DGRAY + '[F4]' + FG_TEXT + ' Send'
             + '  ' + FG_DGRAY + '[F5]' + FG_TEXT + ' Mouse'
             + '  ' + FG_DGRAY + '[F6]' + FG_TEXT + ' Reset'
             + '  ' + FG_DGRAY + '[Esc]' + FG_TEXT + ' Quit'
    else if ScreenWidth >= 110 then
      Hints := '  ' + FG_DGRAY + '[F1]R' + FG_TEXT + ' '
             + FG_DGRAY + '[F2]Rt' + FG_TEXT + ' '
             + FG_DGRAY + '[F3]T' + FG_TEXT + ' '
             + FG_DGRAY + '[F4]S' + FG_TEXT + ' '
             + FG_DGRAY + '[F5]M' + FG_TEXT + ' '
             + FG_DGRAY + '[Esc]Q'
    else
      Hints := '';

    TitleLine := BG_BAR + ' ' + FG_CYAN + ANSI_BOLD + GLYPH_DIAMOND + ' '
               + APP_NAME + ' ' + FG_DGRAY + APP_VERSION
               + ANSI_RESET + BG_BAR + Hints;

    ClockString := ' ' + CurrentLongTime + '  ';
    TopBar := PadToWidth(TitleLine, ScreenWidth - 2 - Length(ClockString), BG_BAR)
            + FG_CYAN + ClockString;

    SB.Append(FG_BORDER + GLYPH_VERT + ANSI_RESET
            + PadToWidth(TopBar, ScreenWidth - 2, BG_BAR) + ANSI_RESET
            + FG_BORDER + GLYPH_VERT + ANSI_RESET + #13#10);

    { --- Rij 3: paneel-top-separator --- }
    if ShowLeftPanel and ShowRightPanel then
      SeparatorTop := GLYPH_TEESOUTH + RepeatChar(GLYPH_HORZ, ColLeft)
                    + SepColorLeft + GLYPH_TEEAST + FG_BORDER
                    + RepeatChar(GLYPH_HORZ, ColCenter)
                    + SepColorRight + GLYPH_TEEAST + FG_BORDER
                    + RepeatChar(GLYPH_HORZ, ColRight) + GLYPH_TEEWEST
    else if ShowRightPanel then
      SeparatorTop := GLYPH_TEESOUTH
                    + RepeatChar(GLYPH_HORZ, ScreenWidth - ColRight - 3)
                    + SepColorRight + GLYPH_TEEAST + FG_BORDER
                    + RepeatChar(GLYPH_HORZ, ColRight) + GLYPH_TEEWEST
    else if ShowLeftPanel then
      SeparatorTop := GLYPH_TEESOUTH + RepeatChar(GLYPH_HORZ, ColLeft)
                    + SepColorLeft + GLYPH_TEEAST + FG_BORDER
                    + RepeatChar(GLYPH_HORZ, ScreenWidth - ColLeft - 3)
                    + GLYPH_TEEWEST
    else
      SeparatorTop := GLYPH_TEESOUTH
                    + RepeatChar(GLYPH_HORZ, ScreenWidth - 2) + GLYPH_TEEWEST;

    SB.Append(FG_BORDER + SeparatorTop + ANSI_RESET + #13#10);

    { --- Panelen renderen --- }
    if ShowLeftPanel then LeftPane := RenderLeftPane
    else LeftPane := nil;

    CenterPane := RenderCenterPane;

    if ShowRightPanel then RightPane := RenderRightPane
    else RightPane := nil;

    { --- Rijen 4..(3+ContentHeight): paneelinhoud --- }
    for RowIndex := 0 to ContentHeight - 1 do
    begin
      SB.Append(FG_BORDER + GLYPH_VERT + ANSI_RESET);

      if ShowLeftPanel then
      begin
        if RowIndex < Length(LeftPane) then SB.Append(LeftPane[RowIndex])
        else SB.Append(PadToWidth('', ColLeft, BG_PANEL) + ANSI_RESET);
        SB.Append(SepColorLeft + GLYPH_VERT + ANSI_RESET);
      end;

      if RowIndex < Length(CenterPane) then SB.Append(CenterPane[RowIndex])
      else SB.Append(PadToWidth('', ColCenter, BG_MAIN) + ANSI_RESET);

      if ShowRightPanel then
      begin
        SB.Append(SepColorRight + GLYPH_VERT + ANSI_RESET);
        if RowIndex < Length(RightPane) then SB.Append(RightPane[RowIndex])
        else SB.Append(PadToWidth('', ColRight, BG_PANEL) + ANSI_RESET);
      end;

      SB.Append(FG_BORDER + GLYPH_VERT + ANSI_RESET + #13#10);
    end;

    { --- Paneel-bottom-separator (sleepbaar voor input-hoogte) --- }
    if (HoverTarget = 10) or (IsDragging and (DragTarget = 10)) then
      SepColorBottom := FG_CYAN
    else
      SepColorBottom := FG_BORDER;

    if ShowLeftPanel and ShowRightPanel then
      SeparatorBottom := GLYPH_TEESOUTH + RepeatChar(GLYPH_HORZ, ColLeft)
                       + SepColorBottom + GLYPH_TEEAST + FG_BORDER
                       + RepeatChar(GLYPH_HORZ, ColCenter)
                       + SepColorBottom + GLYPH_TEEAST + FG_BORDER
                       + RepeatChar(GLYPH_HORZ, ColRight) + GLYPH_TEEWEST
    else
      SeparatorBottom := GLYPH_TEESOUTH
                       + RepeatChar(GLYPH_HORZ, ScreenWidth - 2) + GLYPH_TEEWEST;

    SB.Append(SepColorBottom + SeparatorBottom + ANSI_RESET + #13#10);

    { --- Input area (multi-line, met scroll) --- }
    AvailInputWidth := ScreenWidth - 5;
    if AvailInputWidth < 5 then AvailInputWidth := 5;

    AllInputLines := WrapMultiline(InputBuffer, AvailInputWidth);

    if Length(AllInputLines) > InputHeight then
      VisualScroll := Length(AllInputLines) - InputHeight
    else
      VisualScroll := 0;

    FirstInputRow := 5 + ContentHeight;

    for I := 0 to InputHeight - 1 do
    begin
      LineIdx := VisualScroll + I;

      if LineIdx < Length(AllInputLines) then
        LineText := AllInputLines[LineIdx]
      else
        LineText := '';

      if (LineIdx = 0) and (VisualScroll = 0) then
        PromptPrefix := ' > '
      else
        PromptPrefix := '   ';

      if SimActive then
      begin
        if I = 0 then
          InputLine := BG_INPUT + ' ' + FG_AMBER + ANSI_BOLD + '...'
                     + ANSI_RESET + BG_INPUT + FG_AMBER + ' Processing ['
                     + PromptState + ']'
        else
          InputLine := BG_INPUT;
      end
      else
        InputLine := BG_INPUT + FG_CYAN + ANSI_BOLD + PromptPrefix
                   + ANSI_RESET + BG_INPUT + FG_TEXT + LineText;

      SB.Append(FG_BORDER + GLYPH_VERT + ANSI_RESET
              + PadToWidth(InputLine, ScreenWidth - 2, BG_INPUT) + ANSI_RESET
              + FG_BORDER + GLYPH_VERT + ANSI_RESET + #13#10);
    end;

    { --- Caretpositie --- }
    if Length(AllInputLines) = 0 then
    begin
      CaretRow := FirstInputRow;
      CaretCol := 5;
    end
    else
    begin
      CaretRow := FirstInputRow + (Length(AllInputLines) - 1 - VisualScroll);
      CaretCol := 5 + Length(AllInputLines[High(AllInputLines)]);
      if CaretCol > ScreenWidth - 2 then CaretCol := ScreenWidth - 2;
    end;

    { --- Status-separator --- }
    SB.Append(FG_BORDER + GLYPH_TEESOUTH
            + RepeatChar(GLYPH_HORZ, ScreenWidth - 2)
            + GLYPH_TEEWEST + ANSI_RESET + #13#10);

    { --- Statusbalk --- }
    case StatusKind of
      0 : TitleLine := FG_GREEN;
      1 : TitleLine := FG_AMBER;
      else TitleLine := FG_RED;
    end;

    if MouseEnabled then MouseLabel := 'on' else MouseLabel := 'off';

    StatusLine := BG_BAR + ' ' + TitleLine + GLYPH_DOT + ANSI_RESET
                + BG_BAR + ' ' + FG_TEXT + 'Connected'
                + ' ' + FG_DGRAY + GLYPH_VERT + ANSI_RESET
                + BG_BAR + ' ' + FG_GRAY + 'Model:' + FG_TEXT + ' GPT-5'
                + ' ' + FG_DGRAY + GLYPH_VERT + ANSI_RESET
                + BG_BAR + ' ' + FG_GRAY + 'Prompt:'
                + TitleLine + ' ' + PromptState
                + ' ' + FG_DGRAY + GLYPH_VERT + ANSI_RESET
                + BG_BAR + ' ' + FG_GRAY + 'Session:' + FG_TEXT + ' '
                + IntToStr(ActiveSessionIndex + 1) + '/'
                + IntToStr(SessionList.Count)
                + ' ' + FG_DGRAY + GLYPH_VERT + ANSI_RESET
                + BG_BAR + ' ' + FG_GRAY + 'Mouse:' + FG_TEXT + ' '
                + MouseLabel
                + ' ' + FG_DGRAY + GLYPH_VERT + ANSI_RESET
                + BG_BAR + ' ' + FG_GRAY + 'Perms:' + FG_CYAN + ' RWX';

    SB.Append(FG_BORDER + GLYPH_VERT + ANSI_RESET
            + PadToWidth(StatusLine, ScreenWidth - 2, BG_BAR) + ANSI_RESET
            + FG_BORDER + GLYPH_VERT + ANSI_RESET + #13#10);

    { --- Onderrand --- }
    SB.Append(FG_BORDER + GLYPH_BOTLEFT
            + RepeatChar(GLYPH_HORZ, ScreenWidth - 2)
            + GLYPH_BOTRIGHT + ANSI_RESET);

    { --- Cursor --- }
    if not SimActive then
    begin
      SB.Append(ESC + '[' + IntToStr(CaretRow) + ';'
              + IntToStr(CaretCol) + 'H');
      SB.Append(CURSOR_SHOW);
    end
    else
      SB.Append(CURSOR_HIDE);

    Result := SB.ToString;
  finally
    SB.Free;
  end;
end;

{ ============================================================================
  SECTIE 10 — Simulatie van een GARP/1.26 prompt-levenscyclus
  ============================================================================ }

procedure AppendUserMessage(const Text : string);
var
  Msg : TChatMessage;
begin
  Msg.Role := crUser;
  Msg.Text := Text;
  Msg.Time := CurrentShortTime;
  ChatMessages.Add(Msg);
end;

procedure AppendAssistantMessage(const Text : string);
var
  Msg : TChatMessage;
begin
  Msg.Role := crAssistant;
  Msg.Text := Text;
  Msg.Time := CurrentShortTime;
  ChatMessages.Add(Msg);
end;

procedure BeginSimulation;
begin
  SimActive := True;
  SimStep := 0;
  SimTimer := GetTickCount;
  PromptState := 'PENDING';
  StatusKind := 1;
  ChatScroll := 0;

  ThinkingLines.Clear;
  ThinkingLines.Add('SIR: parsing user prompt...');
  ThinkingLines.Add('Building structured representation...');
  ThinkingLines.Add('Planner: generating execution plan...');
end;

procedure AdvanceSimulation;
begin
  if not SimActive then Exit;

  Inc(SimStep);

  case SimStep of
    1:
      begin
        PromptState := 'SUBMITTING';
        ThinkingLines.Clear;
        ThinkingLines.Add('INPUT_SUBMITTED (provider_native_send)');
        ThinkingLines.Add('Verifying input ownership...');
        ThinkingLines.Add('Awaiting GENERATION_STARTED...');
      end;

    2:
      begin
        PromptState := 'GENERATING';
        ThinkingLines.Clear;
        ThinkingLines.Add('GENERATION_STARTED - message identified');
        ThinkingLines.Add('Baseline revision captured (r=0)');
        ThinkingLines.Add('Streaming GENERATION_DELTA...');
        AppendAssistantMessage('Analyzing GRISP architecture.');
      end;

    3:
      begin
        ThinkingLines.Clear;
        ThinkingLines.Add('Deltas applied: rev 1..3 (APPEND)');
        ThinkingLines.Add('Terminal stability check (1/2)...');
        ThinkingLines.Add('Fencing: prompt_request_id OK');
      end;

    4:
      begin
        ThinkingLines.Clear;
        ThinkingLines.Add('Terminal stability check (2/2)...');
        ThinkingLines.Add('done=true streaming=false continuation=false');
        ThinkingLines.Add('Submitting terminal candidate to arbiter');
      end;

    5:
      begin
        AppendAssistantMessage('Main components identified:');
        AppendAssistantMessage('  1. SIR - Source Intermediate Representation');
        AppendAssistantMessage('  2. Planner - Execution plan builder');
        AppendAssistantMessage('  3. Evaluator - Graph operations executor');
        AppendAssistantMessage('  4. Validator - Correctness and compliance');
        AppendAssistantMessage('  5. EIR - Optimized Execution IR');
        AppendAssistantMessage('  6. Runtime - Deterministic executor');
        AppendAssistantMessage('  7. WorldState - Persistent state');
        AppendAssistantMessage('Would you like me to generate a diagram?');

        ThinkingLines.Clear;
        ThinkingLines.Add('GENERATION_COMPLETED committed');
        ThinkingLines.Add('Terminal arbitration: winner = COMPLETED');

        PromptState := 'COMPLETED';
        StatusKind := 0;
        SimActive := False;
      end;
  end;
end;

procedure SubmitPrompt;
begin
  if Trim(InputBuffer) = '' then Exit;
  AppendUserMessage(InputBuffer);
  InputBuffer := '';
  BeginSimulation;
end;

{ ============================================================================
  SECTIE 11 — Muisafhandeling
  ----------------------------------------------------------------------------
  Codering van sleepdoelen:
    1   verticale lijn tussen linkerpaneel en chat
    2   verticale lijn tussen chat en rechterpaneel
    10  horizontale lijn boven de input area
    11  horizontale lijn tussen RESOURCES en SESSIONS
    12  horizontale lijn tussen CURRENT PLAN en WORK ITEMS
    13  horizontale lijn tussen WORK ITEMS en THINKING
  ============================================================================ }

procedure UpdateHoverTarget(MouseX, MouseY : Integer);
var
  RightPaneStart : Integer;
  RightPaneEnd   : Integer;
begin
  HoverTarget := 0;

  RightPaneStart := ColLeft + ColCenter + 4;
  RightPaneEnd   := ColLeft + ColCenter + ColRight + 3;

  if ShowLeftPanel and (MouseX = ColLeft + 2) then
    HoverTarget := 1
  else if ShowRightPanel and (MouseX = ColLeft + ColCenter + 3) then
    HoverTarget := 2
  else if MouseY = 4 + ContentHeight then
    HoverTarget := 10
  else if ShowLeftPanel and (MouseY = 4 + LeftSplitRow)
          and (MouseX >= 2) and (MouseX <= ColLeft + 1) then
    HoverTarget := 11
  else if ShowRightPanel and (MouseY = 4 + RightPlanRows)
          and (MouseX >= RightPaneStart) and (MouseX <= RightPaneEnd) then
    HoverTarget := 12
  else if ShowRightPanel
          and (MouseY = 4 + RightPlanRows + 1 + RightWorkRows)
          and (MouseX >= RightPaneStart) and (MouseX <= RightPaneEnd) then
    HoverTarget := 13;
end;

procedure ApplyDrag(const Ev : TInputEvent);
begin
  case DragTarget of
    1:
      begin
        UserColLeft := Ev.MouseX - 2;
        if UserColLeft < MIN_PANE_WIDTH then UserColLeft := MIN_PANE_WIDTH;
        if UserColLeft > ScreenWidth - MIN_CHAT_WIDTH - ColRight - 4 then
          UserColLeft := ScreenWidth - MIN_CHAT_WIDTH - ColRight - 4;
      end;

    2:
      begin
        UserColRight := ScreenWidth - Ev.MouseX - 1;
        if UserColRight < MIN_PANE_WIDTH then UserColRight := MIN_PANE_WIDTH;
        if UserColRight > ScreenWidth - MIN_CHAT_WIDTH - ColLeft - 4 then
          UserColRight := ScreenWidth - MIN_CHAT_WIDTH - ColLeft - 4;
      end;

    10:
      begin
        UserInputRows := ScreenHeight - 7 - (Ev.MouseY - 4);
        if UserInputRows < 1 then UserInputRows := 1;
        if UserInputRows > MAX_INPUT_ROWS then UserInputRows := MAX_INPUT_ROWS;
      end;

    11:
      begin
        UserLeftSplit := Ev.MouseY - 4;
        if UserLeftSplit < 3 then UserLeftSplit := 3;
        if UserLeftSplit > ContentHeight - 4 then
          UserLeftSplit := ContentHeight - 4;
      end;

    12:
      begin
        UserPlanRows := Ev.MouseY - 4;
        if UserPlanRows < 3 then UserPlanRows := 3;
        if UserPlanRows > ContentHeight - 8 then
          UserPlanRows := ContentHeight - 8;
      end;

    13:
      begin
        UserWorkRows := Ev.MouseY - (4 + RightPlanRows + 1);
        if UserWorkRows < 3 then UserWorkRows := 3;
        if UserWorkRows > ContentHeight - 8 then
          UserWorkRows := ContentHeight - 8;
      end;
  end;

  RecomputeLayout;
end;

procedure HandleMouseEvent(const Ev : TInputEvent);
var
  WheelLines : Integer;
begin
  UpdateHoverTarget(Ev.MouseX, Ev.MouseY);

  { Muiswiel scrolt de chat. }
  if Ev.WheelDelta <> 0 then
  begin
    WheelLines := (Ev.WheelDelta div 120) * 3;
    ChatScroll := ChatScroll + WheelLines;
  end;

  { Actieve drag voortzetten of beëindigen. }
  if IsDragging then
  begin
    if Ev.MouseButton = 1 then
      ApplyDrag(Ev)
    else
    begin
      IsDragging := False;
      DragTarget := 0;
    end;
    Exit;
  end;

  { Nieuwe klik. }
  if Ev.MouseButton = 1 then
  begin
    if (HoverTarget >= 1) and (HoverTarget <> 3) then
    begin
      IsDragging := True;
      DragTarget := HoverTarget;
    end
    else if (Ev.MouseY >= 4) and (Ev.MouseY <= ScreenHeight - 2) then
      ChatScroll := 0;
  end;
end;

{ ============================================================================
  SECTIE 12 — Toetsafhandeling
  ============================================================================ }

procedure ResetAllUserSizes;
begin
  UserColLeft := 0;
  UserColRight := 0;
  UserInputRows := 0;
  UserLeftSplit := 0;
  UserPlanRows := 0;
  UserWorkRows := 0;
  RecomputeLayout;
end;

procedure HandleKeyEvent(const Ev : TInputEvent);
begin
  { --- Speciale toetsen --- }
  if Ev.Kind = ikSpecial then
  begin
    case Ev.VK of
      VK_ESCAPE: QuitRequested := True;

      VK_F1:
        begin
          WantLeftPanel := not WantLeftPanel;
          RecomputeLayout;
        end;

      VK_F2:
        begin
          WantRightPanel := not WantRightPanel;
          RecomputeLayout;
        end;

      VK_F3:
        WantThinking := not WantThinking;

      VK_F4:
        if SimActive then AdvanceSimulation else SubmitPrompt;

      VK_F5:
        begin
          MouseEnabled := not MouseEnabled;
          ApplyInputMode;
        end;

      VK_F6:
        ResetAllUserSizes;

      VK_UP:
        if ActiveSessionIndex > 0 then
        begin
          Dec(ActiveSessionIndex);
          CurrentSessionName := SessionList[ActiveSessionIndex].Name;
        end;

      VK_DOWN:
        if ActiveSessionIndex < SessionList.Count - 1 then
        begin
          Inc(ActiveSessionIndex);
          CurrentSessionName := SessionList[ActiveSessionIndex].Name;
        end;

      VK_PRIOR: ChatScroll := ChatScroll + 5;
      VK_NEXT:  ChatScroll := ChatScroll - 5;
      VK_HOME:  ChatScroll := ChatMaxScroll;
      VK_END:   ChatScroll := 0;
    end;
    Exit;
  end;

  { --- Gewone tekens --- }
  if Ev.Kind <> ikChar then Exit;

  case Ev.Ch of
    #13: { Enter }
      if SimActive then AdvanceSimulation else SubmitPrompt;

    #8: { Backspace }
      if Length(InputBuffer) > 0 then
        Delete(InputBuffer, Length(InputBuffer), 1);
  else
    if Ord(Ev.Ch) >= 32 then
      InputBuffer := InputBuffer + Ev.Ch;
  end;
end;

{ ============================================================================
  SECTIE 13 — Hoofdlus
  ============================================================================ }

procedure RunApplication;
var
  Ev        : TInputEvent;
  NewWidth  : Integer;
  NewHeight : Integer;
begin
  InitializeConsole;
  InitializeApplicationData;

  try
    WriteAtomic(SCREEN_ALT_ON);
    WriteAtomic(ANSI_RESET + SCREEN_CLEAR);

    while not QuitRequested do
    begin
      { Resize-detectie. }
      ReadTerminalSize(NewWidth, NewHeight);
      if (NewWidth <> ScreenWidth) or (NewHeight <> ScreenHeight) then
      begin
        RecomputeLayout;
        WriteAtomic(ANSI_RESET + SCREEN_CLEAR);
      end;

      { Één frame, atomisch geschreven. }
      WriteAtomic(RenderFrame);

      { Alle wachtende input in één keer verwerken. }
      repeat
        Ev := ReadInputEvent;
        if Ev.Kind = ikNone then Break;

        case Ev.Kind of
          ikChar, ikSpecial: HandleKeyEvent(Ev);
          ikMouse:           if MouseEnabled then HandleMouseEvent(Ev);
        end;
      until False;

      { Simulatie-tick. }
      if SimActive and ((GetTickCount - SimTimer) > SIM_INTERVAL_MS) then
      begin
        SimTimer := GetTickCount;
        AdvanceSimulation;
      end;

      Sleep(25);
    end;
  finally
    MouseEnabled := False;
    ApplyInputMode;
    WriteAtomic(ANSI_RESET + CURSOR_SHOW + SCREEN_ALT_OFF);
    ShutdownApplicationData;
  end;
end;

{ ============================================================================
  SECTIE 14 — Programma-entry
  ============================================================================ }

begin
  try
    RunApplication;
  except
    on E : Exception do
    begin
      Writeln('Fatal error: ', E.Message);
      Halt(1);
    end;
  end;
end.
