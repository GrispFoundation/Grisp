...
const
  ...
  FG_CYAN   = ESC + '[38;5;51m';   // neon cyaan (accent)
  FG_CYAN2  = ESC + '[38;5;38m';   // dim cyaan (randen)
  FG_BLUE   = ESC + '[38;5;75m';   // licht blauw (user)
  FG_WHITE  = ESC + '[38;5;255m';  // hoofdtekst
  FG_GRAY   = ESC + '[38;5;245m';  // secundair
  FG_DGRAY  = ESC + '[38;5;240m';  // hints
  FG_GREEN  = ESC + '[38;5;46m';   // done
  FG_ORANGE = ESC + '[38;5;214m';  // running
  FG_SLATE  = ESC + '[38;5;103m';  // AI accentbalk
  ...
  BG_INPUT  = ESC + '[48;5;18m';   // inputveld
  BG_USER   = ESC + '[48;5;24m';   // user bubbel (diep blauw)
  BG_AI     = ESC + '[48;5;60m';   // AI bubbel (slate blue) ← FIX
...
// ═══════════════════════════════════════════════════════════════════
//  Render: chatpaneel — met "sink to bottom" gedrag
// ═══════════════════════════════════════════════════════════════════
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if M.Role = crUser then
          // User: geen accentbalk, effen blauwe bubbel
          Buf := BG_MAIN + '  ' + BG_USER + FG_WHITE
               + '  ' + Wrapped[J] + '  '
        else
          // AI: met ▌ accentbalk links
          Buf := BG_MAIN + '  ' + BG_AI + FG_SLATE + '▌'
               + BG_AI + FG_WHITE + ' ' + Wrapped[J] + '  ';
        AddLine(Buf);
      end;
    ...
    // ── FIX: sink naar beneden ──
    // Als er te veel is: verwijder vanaf index 2 (na header)
    if L.Count > Height then
      while L.Count > Height do
        L.Delete(2)
    // Als er te weinig is: voeg padding IN bij index 2 (na header)
    // Hierdoor zakt alle content naar beneden.
    else
      while L.Count < Height do
      begin
