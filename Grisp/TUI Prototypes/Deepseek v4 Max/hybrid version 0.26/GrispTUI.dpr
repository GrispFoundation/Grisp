...
{ ============================================================================
  Exit dialog — overlay, niet meer scherm-wissend
  ============================================================================ }
...
begin
  ...
  { Inner layout, 0-based:
      9..18 = [ Cancel ] (10 chars)
      19..26 = 8 spaties
      27..34 = [ Quit ] (8 chars) }
...
{ Rendert alleen de dialog; wist niets. Wordt ná RenderFrame in de buffer
  gezet zodat de dialog bovenop de normale UI verschijnt. }
function RenderExitDialogOverlay : string;
var
  SB : TStringBuilder;
  W, H, L, T, InnerW, PadL, I : Integer;
...
begin
  ...
  try
    ...
    T := ExitDialogTop;
    InnerW := W - 2;
    PadL := 9;  { cancel start op inner-kolom 9 }
    ...
    { 9 spaties + [ Cancel ] (10) + 8 spaties + [ Quit ] (8) = 35 }
...
begin
  SB := TStringBuilder.Create;
  try
    { Als de exit-dialog zichtbaar is, dimmen we het frame. }
...
begin
  ...
  begin
    ...
    if R1 = R2 then Result := Result + Copy(Plain, C1 + 1, C2 - C1)
      ...
      Result := Result + Copy(Plain, C1 + 1, MaxInt); Result := Result + #13#10; end
    else if R = R2 then Result := Result + Copy(Plain, 1, C2)
    else begin Result := Result + Plain; Result := Result + #13#10; end;
  end;
end;
...
begin
  ...
  { 3. Muiswiel: altijd verwerken, ongeacht hover state. }
  if Ev.WheelDelta <> 0 then
  begin
    WheelLines := (Ev.WheelDelta div 120) * 3;

    if (Ev.MouseY < 4) or (Ev.MouseY >= 5 + ContentHeight + InputHeight) then
    begin
      { Buiten scrollbare regio's — niets doen. }
    end
    ...
    else if Ev.MouseY >= 5 + ContentHeight then
    begin
  ...
  { 4. Drag in uitvoering: update en stop. }
  if IsDragging then
  begin
  ...
  { 6. Hover/DesiredShape bijwerken voor verdere checks. }
  ...
  { 7. Klik met linkermuisknop: prioriteit-volgorde. }
  if Ev.MouseButton = 1 then
  begin
    { 7a. Topbar actie. }
    ...
    { 7c. Divider / resize handle. }
    if HoverTarget in [1, 2, 10, 11, 12, 13] then
    begin
      IsDragging := True;
