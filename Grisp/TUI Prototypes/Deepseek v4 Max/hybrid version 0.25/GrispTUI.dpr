...
begin
  ...
  { v0.35: ruimere standaard paneelbreedtes zodat tekst niet afkapt. }
  ...
  { v0.35: inputhoogte standaard 3 i.p.v. 1. }
  if UserInputRows > 0 then InputHeight := UserInputRows else InputHeight := 3;
...
begin
  ...
  WantLeftPanel := True;
  WantRightPanel := True;
  WantThinking := True;    { v0.35: thinking standaard zichtbaar }
  MouseEnabled := True;
  ...
  try
    ...
    if ChatScroll > 0 then
      Header := Header + ANSI_RESET + BG_MAIN + '  ' + FG_AMBER + GLYPH_UP
              + ' +' + IntToStr(ChatScroll);
    Full.Add(Header);
    PlainList.Add(StripAnsi(Header));
    KindList.Add(0);

    Buf := BG_MAIN + ' ' + FG_BORDER + RepeatChar(GLYPH_HORZ, BodyW - 1);
    Full.Add(Buf);
    ...
    begin
      ...
      begin
        if (I >= 2) and EffectiveSelForLine(WS + I, S1, S2) then
        begin
...
begin
  case ParaIndex of
    ...
    4: Result := '[F4]Step';
    5: if ParaCompact then Result := '[F5]M' else Result := '[F5]Mouse';
...
var
  W, L, T, InnerW, PadL, RelX: Integer;
begin
  Result := 0;
  W := ExitDialogWidth;
  L := ExitDialogLeft;
  T := ExitDialogTop;
  InnerW := W - 2;
  PadL := (InnerW - 31) div 2;
  if PadL < 0 then PadL := 0;

  if ParaY <> T + 8 then Exit;
  RelX := ParaX - L - 1 - PadL;

  if (RelX >= 0) and (RelX <= 13) then Result := 1
  else if (RelX >= 17) and (RelX <= 30) then Result := 2;
end;
...
var
  W, H, L, T, InnerW, I, PadL: Integer;
  CancelBtn, QuitBtn, ButtonLine, BlankRow: string;
  CancelHot, QuitHot: Boolean;
begin
  SB.Append(SCREEN_CLEAR + CURSOR_HOME);
  BlankRow := BG_MAIN + StringOfChar(' ', ScreenWidth) + ANSI_RESET;
  for I := 1 to ScreenHeight do
    AppendAt(SB, 1, I, BlankRow);
  ...
  CancelHot := (HoverTarget = 61);
  QuitHot   := (HoverTarget = 62);
  ...
  AppendAt(SB, L, T + 2,
    BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
    + PadToWidth(BG_PANEL + '  ' + FG_RED + ANSI_BOLD + GLYPH_FDIAMOND
               + '  ' + FG_TEXT + ANSI_BOLD + 'EXIT GRISP OS', InnerW, BG_PANEL)
    + ANSI_RESET + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);

  AppendAt(SB, L, T + 3,
    BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
    + PadToWidth(BG_PANEL + '  ' + FG_BORDER
               + RepeatChar(GLYPH_HORZ, InnerW - 4), InnerW, BG_PANEL)
    + ANSI_RESET + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);
  ...
  AppendAt(SB, L, T + 5,
    BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
    + PadToWidth(BG_PANEL + '  ' + FG_TEXT
               + 'Close the GRISP terminal application?', InnerW, BG_PANEL)
    + ANSI_RESET + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);

  AppendAt(SB, L, T + 6,
    BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
    + PadToWidth(BG_PANEL + '  ' + FG_GRAY
               + 'All running activity will be stopped.', InnerW, BG_PANEL)
    + ANSI_RESET + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);
  ...
  if CancelHot then
    CancelBtn := BG_SELECT + FG_TEXT + ANSI_BOLD + '  Cancel  ' + ANSI_RESET + BG_PANEL
  else
    CancelBtn := BG_PANEL + FG_CYAN + '  Cancel  ' + ANSI_RESET + BG_PANEL;

  if QuitHot then
    QuitBtn := BG_SELECT + FG_TEXT + ANSI_BOLD + '   Quit   ' + ANSI_RESET + BG_PANEL
  else
    QuitBtn := BG_PANEL + FG_RED + ANSI_BOLD + '   Quit   ' + ANSI_RESET + BG_PANEL;

  PadL := (InnerW - 31) div 2;
  if PadL < 0 then PadL := 0;

  ButtonLine := BG_PANEL + StringOfChar(' ', PadL)
              + FG_DGRAY + '[ ' + CancelBtn + FG_DGRAY + ' ]'
              + '   '
              + FG_DGRAY + '[ ' + QuitBtn + FG_DGRAY + ' ]';
  ...
  AppendAt(SB, L, T + 9,
    BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
    + PadToWidth(BG_PANEL + '  ' + FG_DGRAY
               + 'Enter/Y = Quit      Esc/N = Cancel', InnerW, BG_PANEL)
    + ANSI_RESET + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);
...
var
  ...
  MouseLabel, ClockString, CloseColor, SelectionLabel : string;
...
begin
  ...
  SelActive := False; SelAnchorRow := -1; SelFocusRow := -1;
  ThinkingLines.Clear;
...
begin
  ...
  try
    Move(PWideChar(Text)^, Ptr^, ByteCount);
  finally
    GlobalUnlock(Data);
  end;
...
begin
  CopyTextToClipboard(InputBuffer);
end;

{ ============================================================================
  Muiscursor en hit-testing
  ============================================================================ }
...
var
  ChatTop, ChatBottom, VisRow : Integer;
begin
  Result := False;
  FullRow := -1; Col := 0;
  ...
  Col := X - ChatBodyLeftX;
  if Col < 0 then Col := 0;
  if Col > Length(ChatFullPlain[FullRow]) then
    Col := Length(ChatFullPlain[FullRow]);
...
begin
  ...
  begin
    ...
    if R1 = R2 then
      Result := Result + Copy(Plain, C1 + 1, C2 - C1)
    else if R = R1 then
    begin
      Result := Result + Copy(Plain, C1 + 1, MaxInt);
      Result := Result + #13#10;
    end
    else if R = R2 then
      Result := Result + Copy(Plain, 1, C2)
    else
    begin
      Result := Result + Plain;
      Result := Result + #13#10;
...
begin
  case ParaKeyIndex of
    ...
    5: begin
         ...
         ApplyInputMode;
         if MouseEnabled then PromptState := 'READY'
         else PromptState := 'TERM-SEL';
       end;
    6: ResetAllUserSizes;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      if DialogAction = 1 then CancelExit else ConfirmExit;
      NeedsRedraw := True;
    end;
    Exit;
  end;

  { v0.35 FIX: IsDragging moet hier staan — anders werkt resize niet meer. }
  if IsDragging then
  begin
  ...
  begin
    ...
    if Tgt <> 0 then
    begin
      IsDragging := True; DragTarget := Tgt; ApplyDrag(Ev); NeedsRedraw := True;
    end
    else if HoverTarget >= 1 then
    begin
      IsDragging := True; DragTarget := HoverTarget; NeedsRedraw := True;
    end
    ...
    else if HitTestChat(Ev.MouseX, Ev.MouseY, HitRow, HitCol) then
    begin
      IsSelecting := True;
  ...
  begin
    ...
    begin
      SelActive := False; SelAnchorRow := -1; SelFocusRow := -1;
      IsSelecting := False; NeedsRedraw := True;
    end;
  end;
end;
