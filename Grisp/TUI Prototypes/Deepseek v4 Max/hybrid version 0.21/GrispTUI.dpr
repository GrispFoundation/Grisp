...
const
  ...
  MIN_SECTION_ROWS      = 3; // header + separator + one visible content row
...
begin
  ...
  if MouseEnabled then Mode := (Mode or ENABLE_MOUSE_INPUT) and not ENABLE_QUICK_EDIT_MODE
  else Mode := (Mode and not ENABLE_MOUSE_INPUT) or ENABLE_QUICK_EDIT_MODE;
...
begin
  ...
  { Left pane: both Resources and Sessions always retain one visible body row.
    Each section has two fixed rows (header + separator), hence 3 rows total. }
  ...
  if LeftSplitRow > ContentHeight - (MIN_SECTION_ROWS + 1) then
    LeftSplitRow := ContentHeight - (MIN_SECTION_ROWS + 1);

  AvailableRows := ContentHeight - 2;

  { Right pane: Plan, Work and Thinking use the same minimum.
    Each gets exactly one content row at minimum. }
  ...
  while (RightPlanRows + RightWorkRows) >
        AvailableRows - MIN_SECTION_ROWS do
  begin
    if RightWorkRows > MIN_SECTION_ROWS then
      Dec(RightWorkRows)
...
begin
  ...
  try
    ...
    if ResBodyH < MIN_SECTION_BODY_ROWS then ResBodyH := MIN_SECTION_BODY_ROWS;
    SesBodyH := ContentHeight - LeftSplitRow - 3;
    if SesBodyH < MIN_SECTION_BODY_ROWS then SesBodyH := MIN_SECTION_BODY_ROWS;
...
begin
  ...
  try
    ...
    if PlanBodyH < MIN_SECTION_BODY_ROWS then PlanBodyH := MIN_SECTION_BODY_ROWS;
    WorkBodyH  := RightWorkRows - 2;
    if WorkBodyH < MIN_SECTION_BODY_ROWS then WorkBodyH := MIN_SECTION_BODY_ROWS;
    ThinkBodyH := ThinkRows - 2;
    if ThinkBodyH < MIN_SECTION_BODY_ROWS then ThinkBodyH := MIN_SECTION_BODY_ROWS;
...
begin
  ...
  try
    ...
    if InputFollowCaret and CursorVisualPos(InputLines, InputCursor, VisRow, VisCol) then
    begin
      ...
      CursorVisualPos(InputLines, InputCursor, VisRow, VisCol);
    ...
    { A manual scrollbar position must survive redraws. Editing or caret
      movement sets InputFollowCaret back to True before the next render. }
...
begin
  ...
  InputBuffer := ''; InputCursor := 0; InputScroll := 0; InputFollowCaret := True;
  BeginSimulation;
end;
...
begin
  case DragTarget of
    ...
    11: begin UserLeftSplit := Ev.MouseY - 4;
          if UserLeftSplit < MIN_SECTION_ROWS then UserLeftSplit := MIN_SECTION_ROWS;
          if UserLeftSplit > ContentHeight - (MIN_SECTION_ROWS + 1) then
            UserLeftSplit := ContentHeight - (MIN_SECTION_ROWS + 1); end;
    12: begin UserPlanRows := Ev.MouseY - 4;
          if UserPlanRows < MIN_SECTION_ROWS then UserPlanRows := MIN_SECTION_ROWS;
          if UserPlanRows > ContentHeight - 2 - MIN_SECTION_ROWS - MIN_SECTION_ROWS then
            UserPlanRows := ContentHeight - 2 - MIN_SECTION_ROWS - MIN_SECTION_ROWS; end;
    13: begin UserWorkRows := Ev.MouseY - (4 + RightPlanRows + 1);
          if UserWorkRows < MIN_SECTION_ROWS then UserWorkRows := MIN_SECTION_ROWS;
          if UserWorkRows > ContentHeight - 2 - MIN_SECTION_ROWS - MIN_SECTION_ROWS then
            UserWorkRows := ContentHeight - 2 - MIN_SECTION_ROWS - MIN_SECTION_ROWS; end;
    ...
    32:
      begin
        InputScroll := MouseToScrollTop(Ev.MouseY, 5 + ContentHeight,
...
begin
  ...
  begin
    ...
    if WheelLines = 0 then
      WheelLines := 3 * Sign(Ev.WheelDelta);

    { The input bar spans the full terminal width. It therefore has to be
      checked BEFORE the left/right pane X-routing. Otherwise a mouse wheel
      event over the input bar below the right/left pane can be incorrectly
      routed to that pane, especially near the right-hand edge. }
    if (Ev.MouseY >= 5 + ContentHeight)
       and (Ev.MouseY < 5 + ContentHeight + InputHeight) then
    begin
      InputFollowCaret := False;
      InputScroll := EnsureRange(InputScroll - WheelLines, 0, InputMaxScroll);
      NeedsRedraw := True;
      Exit;
    end;
    ...
    begin
      if Ev.MouseY < 4 + LeftSplitRow then
        ResScroll := EnsureRange(ResScroll - WheelLines, 0, ResMaxScroll)
      else
        SesScroll := EnsureRange(SesScroll - WheelLines, 0, SesMaxScroll);
    end
    else if ShowRightPanel and (Ev.MouseX >= ColLeft + ColCenter + 4) then
    begin
      if Ev.MouseY < 4 + RightPlanRows then
        PlanScroll := EnsureRange(PlanScroll - WheelLines, 0, PlanMaxScroll)
      else if Ev.MouseY < 4 + RightPlanRows + 1 + RightWorkRows then
        WorkScroll := EnsureRange(WorkScroll - WheelLines, 0, WorkMaxScroll)
      else
        ThinkScroll := EnsureRange(ThinkScroll - WheelLines, 0, ThinkMaxScroll);
    end
    else
    begin
      ChatScroll := EnsureRange(ChatScroll + WheelLines, 0, ChatMaxScroll);
    end;
    NeedsRedraw := True;
  end;
