{ ============================================================================
  GRISP OS — Terminal UI  ·  v0.26
  ----------------------------------------------------------------------------
  Baseline: ChatGPT's v0.23-fixed (WrapParagraph met CRLF/LF handling)
  Toegevoegd:
    • Cyaan input-randen (waren bijna onzichtbaar grijs op BG_INPUT)
    • Wrap-breedte fix: ColCenter - 5 i.p.v. ColCenter - 6 (betere benutting)
  ============================================================================ }
...
{ ChatGPT's fix: normaliseert CRLF/LF, splitst op LF, behoudt lege regels,
  hard-wrapt lange tokens. Geen enkele return-waarde bevat ooit #10/#13/#9. }
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        FlushCurrent;
        L.Add(Copy(Word, 1, MaxWidth));
        Delete(Word, 1, MaxWidth);
      end;
      ...
      if Cur = '' then
        Cur := Word
      else if Length(Cur) + 1 + Length(Word) <= MaxWidth then
        Cur := Cur + ' ' + Word
      else
      begin
        FlushCurrent;
...
begin
  Result := ScreenHeight - 7 - MIN_CONTENT_H;
...
begin
  ...
  try
    ...
    begin
      ...
      { Fix: ColCenter - 5 i.p.v. -6 — volledige bubbelbreedte benut.
        Prefix = '  ' + '  ' (4 zichtbaar), Emit pad naar ColCenter - 1,
        dus MaxWidth = ColCenter - 5. }
      Wrapped := WrapParagraph(Msg.Text, ColCenter - 5);
...
var
  ...
  LineText, PaddedContent, InputBorderL, InputBorderR : string;
begin
  ...
  try
    ...
    { Verbetering: cyaan input-randen. Content wordt apart gepad naar
      ScreenWidth - 2, randen in helder cyaan op BG_INPUT. }
    InputBorderL := BG_INPUT + FG_CYAN + ANSI_BOLD + GLYPH_VERT + ANSI_RESET;
    InputBorderR := BG_INPUT + FG_CYAN + ANSI_BOLD + GLYPH_VERT + ANSI_RESET;
    ...
    begin
      ...
      if SimActive and (I = 0) then
        PaddedContent := PadToWidth(
          BG_INPUT + FG_AMBER + ANSI_BOLD + '... Processing ['
          + PromptState + '] - [Ctrl+Enter] to step',
          ScreenWidth - 2, BG_INPUT)
      else
        PaddedContent := PadToWidth(
          BG_INPUT + FG_TEXT + LineText,
          ScreenWidth - 2, BG_INPUT);

      SB.Append(InputBorderL + PaddedContent + InputBorderR + NL);
    end;
