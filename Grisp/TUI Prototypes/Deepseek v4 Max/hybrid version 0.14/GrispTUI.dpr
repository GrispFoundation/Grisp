...
var
  StdOutHandle : THandle; StdInHandle : THandle;
  ...
  CurrentSessionName : string; ActiveSessionIndex : Integer = 0;
  TreeVisibleRows, TreeVisibleIdx : TArray<Integer>;
  SessionTitleRows, SessionIdxAtRow : TArray<Integer>;
  InputBuffer : string; InputCursor : Integer = 0; InputScroll : Integer = 0;
  WantLeftPanel : Boolean = True; WantRightPanel : Boolean = True;
  WantThinking : Boolean = True; MouseEnabled : Boolean = True;
  ShowLeftPanel : Boolean = True; ShowRightPanel : Boolean = True;
  ShowThinking : Boolean = True;
  UserColLeft : Integer = 0; UserColRight : Integer = 0;
  UserInputRows : Integer = 0; UserLeftSplit : Integer = 0;
  UserPlanRows : Integer = 0; UserWorkRows : Integer = 0;
  IsDragging : Boolean = False; DragTarget : Integer = 0; HoverTarget : Integer = 0;
  ...
  NeedsRedraw : Boolean = True; ForceFullRedraw : Boolean = False;
...
var Available : DWORD; Rec : INPUT_RECORD; Read : DWORD;
    ButtonState : DWORD; ControlKeys : DWORD; WheelRaw : SmallInt;
begin
...
begin
  ...
  try
    ...
    begin
      if Cur = '' then Cur := W[I]
      else if Length(Cur) + 1 + Length(W[I]) <= MaxWidth then Cur := Cur + ' ' + W[I]
...
begin
  ...
  { Bugfix: gebruik overal 'Tree', niet 'T'. }
...
var Lines : TList<string>; I, Indent, NameWidth, SessRows, RowUsed : Integer;
...
begin
  Lines := TList<string>.Create; Vis := TList<Integer>.Create;
  ...
  try
    ...
    begin
      ...
      AddLine(Buf); Vis.Add(4 + RowUsed); Inc(RowUsed);
    end;
    SetLength(TreeVisibleRows, Vis.Count);
    for I := 0 to Vis.Count - 1 do TreeVisibleRows[I] := Vis[I];
    SetLength(TreeVisibleIdx, Vis.Count);
    for I := 0 to High(TreeVisibleRows) do TreeVisibleIdx[I] := Visible[I];
    while RowUsed < LeftSplitRow do begin AddLine(BG_PANEL); Inc(RowUsed); end;
    ...
    begin
      ...
      begin
        if I = ActiveSessionIndex then
          ...
          Buf := BG_PANEL + FG_TEXT + ' ' + SessionTitle + FG_GRAY + MetaStr;
        AddLine(Buf); SessRowsList.Add(4 + RowUsed); Inc(RowUsed);
      end
      else
      begin
        if I = ActiveSessionIndex then
          Buf := BG_PANEL + FG_CYAN + ANSI_BOLD + ' ' + SessionTitle
        else Buf := BG_PANEL + FG_TEXT + ' ' + SessionTitle;
        AddLine(Buf); SessRowsList.Add(4 + RowUsed); Inc(RowUsed);
        if RowUsed < ContentHeight then
        begin AddLine(BG_PANEL + FG_GRAY + '   ' + Session.Meta); Inc(RowUsed); end;
      end;
    end;
    SetLength(SessionTitleRows, SessRowsList.Count);
    for I := 0 to SessRowsList.Count - 1 do SessionTitleRows[I] := SessRowsList[I];
...
begin if ShowLeftPanel then Result := ColLeft + ColCenter + 2
      else Result := ColCenter + 1; end;
function ScrollbarTop : Integer; begin Result := 4 + 2; end;
function ScrollbarBottom : Integer; begin Result := 4 + ContentHeight - 1; end;

function ChatScrollbarChar(VisRow : Integer) : string;
var ThumbTop, ThumbH, BodyH, Total : Integer;
begin
  BodyH := ContentHeight - 2; Total := ChatMaxScroll + BodyH;
  if (BodyH <= 0) or (Total <= BodyH) then
  begin Result := BG_MAIN + FG_DGRAY + GLYPH_VERT; Exit; end;
  ...
  if VisRow < 2 then begin Result := BG_MAIN + FG_DGRAY + GLYPH_VERT; Exit; end;
  if (VisRow - 2 >= ThumbTop) and (VisRow - 2 < ThumbTop + ThumbH) then
    Result := BG_MAIN + FG_CYAN + GLYPH_SCROLL
  else Result := BG_MAIN + FG_DGRAY + GLYPH_VERT;
end;

function RenderCenterPane : TArray<string>;
var Vis, Full : TList<string>;
  ...
  begin Vis.Add(PadToWidth(S, ColCenter - 1, BG_MAIN) + ChatScrollbarChar(VisRow)); end;
begin
  ...
  try
    ...
    BodyH := ContentHeight - 2; if BodyH < 1 then BodyH := 1;
    Total := Full.Count;
...
var Lines : TList<string>; I, ThinkRows, Div1, Div2 : Integer;
...
begin
  ...
  try
    ...
    Div1 := RightPlanRows; Div2 := RightPlanRows + 1 + RightWorkRows;
...
var
  ...
  MaxScroll, FirstInputRow, RowNum : Integer;
  ...
  LineText, FullLine : string;

  procedure GoToRow(R : Integer);
  begin SB.Append(ESC + '[' + IntToStr(R) + ';1H'); end;

  procedure WriteRow(R : Integer; const Content, PadBg : string);
  begin
    GoToRow(R);
    SB.Append(PadToWidth(Content, ScreenWidth, PadBg));
  end;
...
begin
  ...
  try
    ...
    { Row 1: top border }
    FullLine := FG_BORDER + GLYPH_TOPLEFT + RepeatChar(GLYPH_HORZ, ScreenWidth - 2)
              + GLYPH_TOPRIGHT + ANSI_RESET;
    WriteRow(1, FullLine, BG_BAR);
    ...
    TopBar := PadToWidth(TitleLine, ScreenWidth - 2 - Length(ClockString), BG_BAR)
            + FG_CYAN + ClockString;
    FullLine := FG_BORDER + GLYPH_VERT + ANSI_RESET + TopBar
              + FG_BORDER + GLYPH_VERT + ANSI_RESET;
    WriteRow(2, FullLine, BG_BAR);
    ...
    begin
      RowNum := 4 + RowIndex;
      FullLine := FG_BORDER + GLYPH_VERT + ANSI_RESET;
      if ShowLeftPanel then
      begin
        if RowIndex < Length(LeftPane) then FullLine := FullLine + LeftPane[RowIndex]
        else FullLine := FullLine + PadToWidth('', ColLeft, BG_PANEL) + ANSI_RESET;
        FullLine := FullLine + SepC1 + GLYPH_VERT + ANSI_RESET;
      end;
      if RowIndex < Length(CenterPane) then FullLine := FullLine + CenterPane[RowIndex]
      else FullLine := FullLine + PadToWidth('', ColCenter - 1, BG_MAIN)
                     + ChatScrollbarChar(RowIndex);
      if ShowRightPanel then
      begin
        FullLine := FullLine + SepC2 + GLYPH_VERT + ANSI_RESET;
        if RowIndex < Length(RightPane) then FullLine := FullLine + RightPane[RowIndex]
        else FullLine := FullLine + PadToWidth('', ColRight, BG_PANEL) + ANSI_RESET;
      end;
      FullLine := FullLine + FG_BORDER + GLYPH_VERT + ANSI_RESET;
      WriteRow(RowNum, FullLine, BG_MAIN);
    end;
    ...
    if ShowLeftPanel and ShowRightPanel then
      ...
      SepBot := SepBotAll + GLYPH_TEEEAST
              ...
              + GLYPH_TEEWEST + ANSI_RESET;
    WriteRow(4 + ContentHeight, SepBot, BG_MAIN);
    ...
    else begin InputScroll := 0; VisRow := 0; VisCol := 0; end;
    ...
    begin
      ...
      else InputLine := BG_INPUT + FG_TEXT + LineText;

      FullLine := FG_BORDER + GLYPH_VERT + ANSI_RESET + InputLine
                + ANSI_RESET + FG_BORDER + GLYPH_VERT + ANSI_RESET;
      WriteRow(FirstInputRow + I, FullLine, BG_INPUT);
    end;
    ...
    WriteRow(5 + ContentHeight + InputHeight,
             FG_BORDER + GLYPH_TEEEAST + RepeatChar(GLYPH_HORZ, ScreenWidth - 2)
             + GLYPH_TEEWEST + ANSI_RESET, BG_MAIN);

    case StatusKind of
      0 : TitleLine := FG_GREEN; 1 : TitleLine := FG_AMBER; else TitleLine := FG_RED;
    end;
    if MouseEnabled then MouseLabel := 'on' else MouseLabel := 'off';
    StatusLine := BG_BAR + ' ' + TitleLine + GLYPH_DOT + ANSI_RESET + BG_BAR + ' '
                + FG_TEXT + 'Connected'
    ...
    FullLine := FG_BORDER + GLYPH_VERT + ANSI_RESET + StatusLine
              + ANSI_RESET + FG_BORDER + GLYPH_VERT + ANSI_RESET;
    WriteRow(6 + ContentHeight + InputHeight, FullLine, BG_BAR);

    WriteRow(7 + ContentHeight + InputHeight,
             FG_BORDER + GLYPH_BOTLEFT + RepeatChar(GLYPH_HORZ, ScreenWidth - 2)
             + GLYPH_BOTRIGHT + ANSI_RESET, BG_MAIN);
...
begin if InputCursor > 0 then
  begin Delete(InputBuffer, InputCursor, 1); Dec(InputCursor); end; end;

procedure DeleteAtCaret;
begin if InputCursor < Length(InputBuffer) then
  Delete(InputBuffer, InputCursor + 1, 1); end;
...
begin Lines := BuildVisualLines(InputBuffer, InputWrapWidth);
  if CursorVisualPos(Lines, InputCursor, R, C) then
    InputCursor := Lines[R].BufStart - 1; ClampInputCursor; end;

procedure MoveCaretEnd;
var Lines : TArray<TVisualLine>; R, C : Integer;
begin Lines := BuildVisualLines(InputBuffer, InputWrapWidth);
  if CursorVisualPos(Lines, InputCursor, R, C) then
    InputCursor := (Lines[R].BufStart - 1) + Length(Lines[R].Text); ClampInputCursor; end;
...
begin
  HoverTarget := 0;
  RStart := ColLeft + ColCenter + 4; REnd := ColLeft + ColCenter + ColRight + 3;
  ...
  else if ShowRightPanel and (MouseY = 4 + RightPlanRows + 1 + RightWorkRows)
          and (MouseX >= RStart) and (MouseX <= REnd) then HoverTarget := 13
  else if (MouseX = ScrollbarX) and (MouseY >= ScrollbarTop)
          and (MouseY <= ScrollbarBottom) then HoverTarget := 20;
end;

procedure ApplyDrag(const Ev : TInputEvent);
var
  MyMax, RelY, BodyH : Integer;
begin
  case DragTarget of
...
begin
  BodyH := ContentHeight - 2; if BodyH <= 1 then Exit;
  RelY := Y - ScrollbarTop; if RelY < 0 then RelY := 0;
  if RelY >= BodyH then RelY := BodyH - 1;
  ChatScroll := (BodyH - 1 - RelY) * ChatMaxScroll div (BodyH - 1);
  if ChatScroll < 0 then ChatScroll := 0;
  if ChatScroll > ChatMaxScroll then ChatScroll := ChatMaxScroll;
...
var I, Idx : Integer; T : TTreeItem;
begin
  if (X = ScrollbarX) and (Y >= ScrollbarTop) and (Y <= ScrollbarBottom) then
  begin HandleScrollbarClick(Y); Exit; end;
  ...
  begin
    for I := 0 to High(TreeVisibleRows) do
      ...
      begin Idx := TreeVisibleIdx[I];
        ...
        begin T := TreeList[Idx]; T.Expanded := not T.Expanded; TreeList[Idx] := T; end;
        Exit; end;
    for I := 0 to High(SessionTitleRows) do
      if Y = SessionTitleRows[I] then
      begin SwitchSession(SessionIdxAtRow[I]); Exit; end;
  end;
  if (Y >= 4) and (Y < 4 + ContentHeight)
    and (X > ColLeft + 2) and (X < ScrollbarX) then ChatScroll := 0;
end;
...
begin
  ...
  begin WheelLines := (Ev.WheelDelta div 120) * 3;
    ChatScroll := ChatScroll + WheelLines; NeedsRedraw := True; end;
  if IsDragging then
  begin
  ...
  begin
    ...
    begin IsDragging := True; DragTarget := HoverTarget;
      if DragTarget = 20 then HandleScrollbarClick(Ev.MouseY);
      NeedsRedraw := True; end
    else begin HandleMouseClick(Ev.MouseX, Ev.MouseY); NeedsRedraw := True; end;
  end;
end;
