{ ============================================================================
  GRISP OS — Terminal UI  ·  v0.22
  ----------------------------------------------------------------------------
  Nieuw in v0.22
    • Bottom T-joints (┴) lichten mee met de verticale scheiders
    • Periodieke full-refresh tegen schermresidu (om de ~2 s in rust)
    • Inputveld schoon: caret staat direct na │, geen leading spaces meer
    • Inputgebied kan groeien tot contentHeight >= 6 (dus ScreenHeight - 13)
  ============================================================================ }
...
begin
  ...
  try
    ...
    { Fix 1: T-joints onder de verticale scheiders lichten mee. }
    if ShowLeftPanel and ShowRightPanel then
      SepBot := SepBotAll + GLYPH_TEEEAST + RepeatChar(GLYPH_HORZ, ColLeft)
              + SepC1 + GLYPH_TEEUP + SepBotAll
              + RepeatChar(GLYPH_HORZ, ColCenter)
              + SepC2 + GLYPH_TEEUP + SepBotAll
              + RepeatChar(GLYPH_HORZ, ColRight)
              + GLYPH_TEEWEST + ANSI_RESET
    else
    ...
    begin
      ...
      begin
        ...
        { Fix 3: geen leading spaces — caret direct na │. }
...
begin
  case DragTarget of
    ...
    10: begin
          { Fix 4: input mag groeien tot content min 6 blijft. }
...
begin
  ...
  try
    ...
    begin
      ...
      { Fix 2: periodieke full-refresh tegen schermresidu. }
      NowTick := GetTickCount;
      if (NowTick - LastFullRedraw) > FULL_REFRESH_MS then
      begin
        ForceFullRedraw := True;
        NeedsRedraw := True;
        LastFullRedraw := NowTick;
      end;
