...
const
  ...
  { ── Opties (zet op True/False naar wens) ── }
  OPT_SHOW_DIAMONDS         : Boolean = False;  { ◆ voor titels en berichten }
  OPT_SHOW_RESOURCE_ARROWS  : Boolean = False;  { ▸/▾ voor resource-tree }
  OPT_SHOW_SESSION_ARROWS   : Boolean = False;  { ▸ voor sessies }
  OPT_SHOW_PRIORITY_LABELS  : Boolean = False;  { [HIGH]/[MED]/[LOW]; anders kleur hele tekst }
  OPT_SHOW_CHAT_DATE        : Boolean = True;   { datum in chat wanneer ruimte }
  ...
  SCREEN_CLEAR   = ESC + '[3J' + ESC + '[2J' + ESC + '[H';   { v0.39: ook scrollback }
  CURSOR_HOME    = ESC + '[H';
  ...
  MIN_PANE_WIDTH  = 20;
  MIN_CHAT_WIDTH  = 34;
  MIN_CONTENT_H   = 3;   { v0.39: header + divider + 1 body rij }
...
{ v0.39: komma weg, alleen spatie tussen datum en tijd. }
...
begin
  ...
  { v0.39: klein scherm → input hoogte 1, minimum content 3. }
...
var
  ...
  Buf, PermColor, NameField, DividerCol, SessionTitle, MetaStr, SessTitleBuf, Arrow : string;
...
begin
  ...
  try
    ...
    { v0.39: header zonder diamond als OPT_SHOW_DIAMONDS=False. }
    if OPT_SHOW_DIAMONDS then
    ...
    begin
      ...
      { v0.39: voeg datum toe aan timestamp als er ruimte is. }
      if OPT_SHOW_CHAT_DATE and (BodyW >= 60) then
  ...
  try
    ...
    begin
      ...
      begin
        ...
        { v0.39: kleur hele tekst als OPT_SHOW_PRIORITY_LABELS=False. }
...
function TopBarSegmentText(ParaIndex: Integer): string;
begin
  case ParaIndex of
    1: Result := '[F1]Left';
    2: Result := '[F2]Right';
...
{ v0.39: dynamische topbar — voeg segmenten toe zolang er ruimte is. }
procedure ComputeTopBar(out Title: string; out VisibleFKeys: TArray<string>;
  out VisibleExtras: TArray<string>; out ClockStr: string;
  out FKeyStart, ExtraStart, CloseStart: Integer);
var
  I, TotalW, AvailW, TitleW, ClockW : Integer;
...
begin
  ...
  { v0.39: 1 spatie links, 1 spatie tussen titel en F1 }
  Title := ' ' + TitleText;
  TitleW := VisibleLength(Title);

  if ScreenWidth < 80 then
    ClockStr := ' ' + FormatDateTime('hh:nn', Now) + ' [X]'
  else
    ClockStr := ' ' + CurrentLongTime + ' [X]';
  ClockW := VisibleLength(ClockStr);

  CloseStart := ScreenWidth - 3;   { [X] positie }
  AvailW := ScreenWidth - 2 - ClockW - 1;   { -2 voor randen, -1 voor spatie voor clock }

  { Kandidaten in prioriteitsvolgorde }
  for I := 1 to 6 do Candidates[I-1] := TopBarSegmentText(I);
  for I := 1 to 3 do Candidates[5+I] := TopBarExtraText(I);

  SetLength(VisibleFKeys, 0);
  SetLength(VisibleExtras, 0);

  TotalW := TitleW;
  FKeyStart := 2 + TitleW + 1;   { kolom waar F1 begint }
  ExtraStart := FKeyStart;

  { Voeg F-keys toe zolang ruimte }
  for I := 0 to 5 do
  begin
    if TotalW + 1 + VisibleLength(Candidates[I]) <= AvailW then
    begin
      Inc(TotalW, 1 + VisibleLength(Candidates[I]));
      SetLength(VisibleFKeys, Length(VisibleFKeys) + 1);
      VisibleFKeys[High(VisibleFKeys)] := Candidates[I];
      ExtraStart := FKeyStart + TotalW - TitleW;
    end
    else Break;
  end;

  { Voeg extras toe zolang ruimte }
  for I := 6 to 8 do
  begin
    if TotalW + 1 + VisibleLength(Candidates[I]) <= AvailW then
    begin
      Inc(TotalW, 1 + VisibleLength(Candidates[I]));
      SetLength(VisibleExtras, Length(VisibleExtras) + 1);
      VisibleExtras[High(VisibleExtras)] := Candidates[I];
    end
    else Break;
  end;
end;
...
var
  Title, ClockStr : string;
  FKeys, Extras : TArray<string>;
  FKeyStart, ExtraStart, CloseStart, I, XStart, XEnd : Integer;
begin
  Result := 0;
  if ParaY <> 2 then Exit;

  ComputeTopBar(Title, FKeys, Extras, ClockStr, FKeyStart, ExtraStart, CloseStart);
...
var W, L, T, InnerW, PadL, RelX : Integer;
begin
  Result := 0;
  W := ExitDialogWidth;
  L := ExitDialogLeft;
...
begin
  ...
  try
    ...
    AppendAt(SB, L, T + 1,
      ...
      + PadToWidth(BG_PANEL + '  ' + FG_RED + ANSI_BOLD
                 + 'Exit GRISP OS ?', InnerW, BG_PANEL)
...
var
  ...
  MouseLabel, ClockStr, CloseColor, SelectionLabel : string;
...
begin
  ...
  try
    ...
    ComputeTopBar(MainTitle, TopBarFKeys, TopBarExtraKeys, ClockStr,
    ...
    TopBar := PadToWidth(TopBar, ScreenWidth - VisibleLength(ClockStr) - 2, BG_BAR);
    if HoverTarget = 60 then CloseColor := FG_RED + ANSI_BOLD else CloseColor := FG_TEXT;
    TopBar := TopBar + BG_BAR + FG_CYAN + ClockStr + ANSI_RESET;
    { Herstel CloseColor in clock }
    TopBar := Copy(TopBar, 1, Length(TopBar) - 10)
            + CloseColor + '[X]' + ANSI_RESET;
...
var M : TChatMessage;
begin M.Role := crUser; M.Text := Text; M.Time := CurrentShortTime; M.Date := CurrentShortDate; ChatMessages.Add(M); end;
procedure AppendAssistantMessage(const Text : string);
var M : TChatMessage;
begin M.Role := crAssistant; M.Text := Text; M.Time := CurrentShortTime; M.Date := CurrentShortDate; ChatMessages.Add(M); end;
...
{ v0.39: muiswiel — scrollbar-kolom heeft prioriteit; anders X/Y-routing. }
...
begin
  ...
  { Belangrijk: als het wiel-event op een scrollbar-kolom valt, gebruik die. }
  if Tgt = 0 then
  begin
    { Fallback: X/Y-routing }
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        RecomputeLayout;
        WriteAtomic(SCREEN_CLEAR);   { direct wissen om scroll-artefacten weg te nemen }
