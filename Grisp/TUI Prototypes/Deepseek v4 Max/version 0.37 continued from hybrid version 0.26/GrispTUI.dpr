...
var
  ...
  AutoScrollDir : Integer = 0;   // -1 = naar boven, +1 = naar beneden, 0 = uit
...
{ Rendert een regel met optionele selectie. LeadLen = aantal leading chars
  die op BG_MAIN staan (niet de bubble-achtergrond). }
...
begin
  ...
  try
    ...
    begin
      ...
      Full.Add(Buf);
      PlainList.Add(StripAnsi(Buf));
      KindList.Add(6);   { bericht-header — selecteerbaar }
...
var ClosePos: Integer; KeyColor, TextColor: string;
begin
  if ParaHot then
...
procedure GetTopBarLayout(
  out ParaTitlePlain: string; out ParaClockPlain: string;
  out ParaFKeys: TArray<string>; out ParaExtraKeys: TArray<string>;
  out ParaFKeyStart: Integer; out ParaExtraStart: Integer;
  out ParaCloseStart: Integer; out ParaShowExtra: Boolean;
  out ParaCompactFKeys: Boolean; out ParaCompactClock: Boolean);
var
  I, FKeyWidth, ExtraWidth, RightWidth, RightStart, LeftLimit, CandidateWidth : Integer;
begin
  SetLength(ParaFKeys, 6);
  for I := 1 to 6 do ParaFKeys[I - 1] := TopBarSegmentText(I, False);
  ...
  { v0.37: ruimere spacing tussen titel en F1. }
  ...
  FKeyWidth := VisibleLength(ParaTitlePlain) + 4;   { 4 spaties ertussen }
  ...
  begin
    ...
    for I := 1 to 6 do ParaFKeys[I - 1] := TopBarSegmentText(I, True);
    FKeyWidth := VisibleLength(ParaTitlePlain) + 4;
    for I := 0 to High(ParaFKeys) do
    begin
      if I > 0 then Inc(FKeyWidth);
      Inc(FKeyWidth, VisibleLength(ParaFKeys[I]));
  ...
  if ParaTitlePlain <> '' then Inc(ParaFKeyStart, 4);   { v0.37: 4 spaties }
  ...
  begin
    ...
    for I := 1 to 3 do ParaExtraKeys[I - 1] := TopBarExtraText(I);
  end;
end;
...
begin
  ...
  GetTopBarLayout(TitlePlain, ClockPlain, FKeys, Extras,
...
begin
  ...
  if ParaY <> T + 7 then Exit;
  RelX := ParaX - L - 1;

  if (RelX >= 9) and (RelX <= 18) then Result := 1
  else if (RelX >= 27) and (RelX <= 34) then Result := 2;
end;
...
var
  ...
  CancelTxt, QuitTxt, CancelBtn, QuitBtn, ButtonRow, HintRow : string;
  ...
  try
    ...
    AppendAt(SB, L, T + 2,
      ...
      + PadToWidth(BG_PANEL + '  ' + FG_RED + ANSI_BOLD + GLYPH_FDIAMOND
                 + '  ' + FG_TEXT + ANSI_BOLD + 'Exit GRISP OS?',
                 InnerW, BG_PANEL)
    ...
    AppendAt(SB, L, T + 4,
      ...
      + PadToWidth(BG_PANEL + '  ' + FG_TEXT
                 + 'Close the GRISP terminal application?',
                 InnerW, BG_PANEL)
      + ANSI_RESET + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);
    ...
    AppendAt(SB, L, T + 5,
      ...
      + PadToWidth(BG_PANEL + '  ' + FG_GRAY
                 + 'All running activity will be stopped.',
                 InnerW, BG_PANEL)
      + ANSI_RESET + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);
    ...
    if CancelHot then
      CancelBtn := BG_SELECT + FG_TEXT + ANSI_BOLD + CancelTxt + ANSI_RESET + BG_PANEL
    else
      CancelBtn := FG_CYAN + CancelTxt + ANSI_RESET + BG_PANEL;

    if QuitHot then
      QuitBtn := BG_SELECT + FG_TEXT + ANSI_BOLD + QuitTxt + ANSI_RESET + BG_PANEL
    else
      QuitBtn := FG_RED + ANSI_BOLD + QuitTxt + ANSI_RESET + BG_PANEL;

    ButtonRow := BG_PANEL + StringOfChar(' ', PadL)
               + CancelBtn + StringOfChar(' ', 8) + QuitBtn;
    AppendAt(SB, L, T + 7,
      BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
    ...
    HintRow := BG_PANEL + '  ' + FG_DGRAY
             + 'Enter = Quit        Esc = Cancel';
    AppendAt(SB, L, T + 9,
      BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
      + PadToWidth(HintRow, InnerW, BG_PANEL)
      + ANSI_RESET + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);
...
begin
  ...
  try
    ...
    GetTopBarLayout(TitleLine, ClockString, TopBarFKeys, TopBarExtraKeys,
    ...
    if TitleLine <> '' then
      TopBar := TopBar + FG_CYAN + ANSI_BOLD + TitleLine + ANSI_RESET + BG_BAR
              + BG_BAR + '    ';   { v0.37: extra ruimte }

    for I := 1 to 6 do
    begin
      if I > 1 then TopBar := TopBar + BG_BAR + ' ';
      TopBar := TopBar + StyleTopBarSegment(TopBarFKeys[I - 1], HoverTarget = 39 + I);
    end;
    ...
    begin
      ...
      begin
        if I > 1 then TopBar := TopBar + BG_BAR + '  ';
        TopBar := TopBar + StyleTopBarSegment(TopBarExtraKeys[I - 1], HoverTarget = 46 + I);
      end;
    end;

    TopBar := PadToWidth(TopBar, ScreenWidth - VisibleLength(ClockString) - 2, BG_BAR);

    if HoverTarget = 50 then CloseColor := FG_RED + ANSI_BOLD else CloseColor := FG_TEXT;
...
begin
  ...
  begin
    ...
    begin
      { Trim trailing whitespace om lege rijen op te ruimen, maar behoud
        het onderscheid tussen berichten. }
      Trimmed := Plain;
...
{ v0.37: wheel-routing is nu puur gebaseerd op X-positie (met de divider-kolom
  meegerekend), en scroll-ranges worden altijd geclamped. }
procedure HandleWheel(const Ev : TInputEvent);
var WheelLines : Integer;
begin
  WheelLines := (Ev.WheelDelta div 120) * 3;

  if ShowLeftPanel and (Ev.MouseX >= 2) and (Ev.MouseX <= ColLeft + 2) then
  begin
  ...
  else if ShowRightPanel and (Ev.MouseX >= ColLeft + ColCenter + 3) then
  begin
  ...
  begin
    { Center pane: chat of input. }
    if (Ev.MouseY >= 5 + ContentHeight) and
       (Ev.MouseY < 5 + ContentHeight + InputHeight) then
    begin
...
begin
  { 1. Exit-dialoog heeft absolute prioriteit. }
  if ExitDialogVisible then
  begin
  ...
  { 2. Rechter-muisklik: kopieer selectie of input. }
  ...
  { 5. Selectie in uitvoering — met edge-detectie voor auto-scroll. }
  if IsSelecting then
  begin
    if Ev.MouseButton = 1 then
    begin
      ...
      if Ev.MouseY < 4 + 2 then
        AutoScrollDir := -1
      else if Ev.MouseY > 4 + ContentHeight - 1 then
        AutoScrollDir := +1
      else
        AutoScrollDir := 0;
  ...
  begin
    ...
    begin
      ...
      else if TopAction = 10 then RequestExit;
      NeedsRedraw := True;
      Exit;
    end;
...
begin
  ...
  try
    ...
    begin
      ...
      { v0.37: auto-scroll tijdens selectie-drag. }
      ...
      begin
        ...
        begin
          ...
          if SelFocusRow >= 0 then
          begin
            if SelFocusRow < High(ChatFullPlain) then
              Inc(SelFocusRow);
      ...
      begin
        ...
        Frame := RenderFrame;
        if ExitDialogVisible then
          Frame := Frame + ANSI_RESET + RenderExitDialogOverlay;
