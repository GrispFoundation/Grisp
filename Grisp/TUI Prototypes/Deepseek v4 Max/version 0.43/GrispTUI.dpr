...
const
  ...
  OPT_SHOW_KEY_BRACKETS     : Boolean = False;
  OPT_SHOW_X_BRACKETS       : Boolean = False;
...
var
  ...
  LeftSplitRow, RightPlanRows, RightWorkRows, RightThinkRows : Integer;
  ...
  WantLeftPanel, WantRightPanel, MouseEnabled : Boolean;
  ShowLeftPanel, ShowRightPanel : Boolean;
  ...
  PlanCollapsed  : Boolean = False;
  WorkCollapsed  : Boolean = False;
  ThinkCollapsed : Boolean = False;
  ResCollapsed   : Boolean = False;
  SesCollapsed   : Boolean = False;

  LastClickTime : Cardinal = 0;
  LastClickX    : Integer = 0;
  LastClickY    : Integer = 0;
  ...
  SelAnchorCol  : Integer = 1;
  SelFocusRow   : Integer = -1;
  SelFocusCol   : Integer = 1;
...
begin
  ...
  begin
    ...
    begin
      ...
      { Wheel event heeft prioriteit — niet door button-detectie laten lopen. }
      if (Rec.Event.MouseEvent.dwEventFlags and EVT_MOUSE_WHEELED) <> 0 then
      begin
        Result.WheelDelta := SmallInt(ButtonState shr 16);
        Exit;
      end;
...
var
  TotalRight, ExpandedCount, FixedOverhead, Remaining, PerBody, Extra : Integer;
  PlanNeed, WorkNeed, ThinkNeed : Integer;
begin
  ...
  { Linkerpaneel }
  if ResCollapsed and SesCollapsed then LeftSplitRow := 1
  else if ResCollapsed then LeftSplitRow := 1
  else if SesCollapsed then LeftSplitRow := ContentHeight - 2
  else
  begin
  ...
  { Rechterpaneel }
  TotalRight := ContentHeight;
  FixedOverhead := 2;
  ExpandedCount := 0;

  if not PlanCollapsed then begin Inc(FixedOverhead, 2); Inc(ExpandedCount); end
  else Inc(FixedOverhead, 1);
  if not WorkCollapsed then begin Inc(FixedOverhead, 2); Inc(ExpandedCount); end
  else Inc(FixedOverhead, 1);
  if not ThinkCollapsed then begin Inc(FixedOverhead, 2); Inc(ExpandedCount); end
  else Inc(FixedOverhead, 1);

  Remaining := TotalRight - FixedOverhead;
  if Remaining < ExpandedCount then Remaining := ExpandedCount;
  if ExpandedCount > 0 then
  begin
    PerBody := Remaining div ExpandedCount;
    Extra := Remaining - PerBody * ExpandedCount;
  end
  else begin PerBody := 0; Extra := 0; end;

  if PlanCollapsed then PlanNeed := 1
  else
  begin
    PlanNeed := 2 + PerBody;
    if Extra > 0 then begin Inc(PlanNeed); Dec(Extra); end;
    if UserPlanRows > 0 then PlanNeed := UserPlanRows;
  end;

  if WorkCollapsed then WorkNeed := 1
  else
  begin
    WorkNeed := 2 + PerBody;
    if Extra > 0 then begin Inc(WorkNeed); Dec(Extra); end;
    if UserWorkRows > 0 then WorkNeed := UserWorkRows;
  end;

  if ThinkCollapsed then ThinkNeed := 1
  else
  begin
    ThinkNeed := TotalRight - 2 - PlanNeed - WorkNeed;
    if ThinkNeed < 3 then ThinkNeed := 3;
  end;

  { Krimp als som te groot is. }
  while (PlanNeed + WorkNeed + ThinkNeed + 2) > TotalRight do
  begin
    if (not ThinkCollapsed) and (ThinkNeed > 3) then Dec(ThinkNeed)
    else if (not WorkCollapsed) and (WorkNeed > 3) then Dec(WorkNeed)
    else if (not PlanCollapsed) and (PlanNeed > 3) then Dec(PlanNeed)
    else Break;
  end;

  { v0.42: groei als som te klein is. }
  while (PlanNeed + WorkNeed + ThinkNeed + 2) < TotalRight do
  begin
    if not ThinkCollapsed then Inc(ThinkNeed)
    else if not WorkCollapsed then Inc(WorkNeed)
    else if not PlanCollapsed then Inc(PlanNeed)
    else Break;
  end;

  RightPlanRows  := PlanNeed;
  RightWorkRows  := WorkNeed;
  RightThinkRows := ThinkNeed;
end;
...
begin
  ...
  WantLeftPanel := True; WantRightPanel := True;
...
var
  ...
  ContentW : Integer;
  ResBodyH, SesBodyH, TotalSessLines, SesTop : Integer;
  ...
  procedure AddPlainRow(const Text : string);
  begin
    Lines.Add(PadToWidth(Text, ContentW, BG_PANEL) + ANSI_RESET + BG_PANEL + ' ' + ANSI_RESET);
  end;
...
begin
  ...
  try
    ...
    ResBodyH := 0;
    SesBodyH := 0;
    if not ResCollapsed then
    begin
      ResBodyH := LeftSplitRow - 2;
      if ResBodyH < 0 then ResBodyH := 0;
    end;
    SesTop := LeftSplitRow + 1;
    if not SesCollapsed then
    begin
      SesBodyH := ContentHeight - SesTop - 2;
    ...
    if OPT_SHOW_DIAMONDS then
      TitleText := ' ' + GLYPH_FDIAMOND + ' RESOURCES'
    else
      TitleText := ' RESOURCES';
    if ResCollapsed then
      TitleText := TitleText + ' ' + GLYPH_COLLAPSED
    else
      TitleText := TitleText + ' ' + GLYPH_EXPANDED;
    AddPlainRow(BG_PANEL + FG_CYAN + ANSI_BOLD + TitleText);
    if not ResCollapsed then
    begin
      AddPlainRow(BG_PANEL + FG_BORDER + ' ' + RepeatChar(GLYPH_HORZ, ContentW - 1));
      RowUsed := 2;
      I := ResScroll;
    ...
    while Lines.Count < LeftSplitRow do AddPlainRow('');
    ...
    if OPT_SHOW_DIAMONDS then
      TitleText := ' ' + GLYPH_FDIAMOND + ' SESSIONS'
    else
      TitleText := ' SESSIONS';
    if SesCollapsed then
      TitleText := TitleText + ' ' + GLYPH_COLLAPSED
    else
      TitleText := TitleText + ' ' + GLYPH_EXPANDED;
    AddPlainRow(BG_PANEL + FG_CYAN + ANSI_BOLD + TitleText);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          AddSessionsRow(SessRowText[I], RowUsed - SesTop - 2);
          Inc(I);
        end
        else
          AddSessionsRow(BG_PANEL, RowUsed - SesTop - 2);
        Inc(RowUsed);
      end;
    end;

    SetLength(SessionTitleRows, SessTitleRowsArr.Count);
    for I := 0 to SessTitleRowsArr.Count - 1 do SessionTitleRows[I] := SessTitleRowsArr[I];
    SetLength(SessionIdxAtRow, SessTitleIdxArr.Count);
    for I := 0 to SessTitleIdxArr.Count - 1 do SessionIdxAtRow[I] := SessTitleIdxArr[I];

    { v0.42: exact ContentHeight — geen extra lege regel. }
    while Lines.Count > ContentHeight do Lines.Delete(Lines.Count - 1);
    while Lines.Count < ContentHeight do AddPlainRow('');
...
{ v0.42: fix — was 3 bij verborgen linkerpaneel, moet 2 zijn. }
...
var
  ...
  BodyW : Integer;
  S1, S2, R1, R2, C1, C2, TmpI, TmpC : Integer;
  ...
  begin
    ...
    begin TmpI := R1; R1 := R2; R2 := TmpI; TmpC := C1; C1 := C2; C2 := TmpC; end;
    if (Row < R1) or (Row > R2) then Exit;
    if R1 = R2 then
    begin A := C1 - 1; B := C2; end
    else if Row = R1 then
    begin A := C1 - 1; B := Length(PlainList[Row]); end
    else if Row = R2 then
    begin A := 0; B := C2; end
    else
    begin A := 0; B := Length(PlainList[Row]); end;
    Result := True;
  end;
...
begin
  ...
  try
      ...
      begin
        ...
        begin
          ...
          Vis.Add(PadToWidth(Buf, BodyW, BG_MAIN) + ANSI_RESET
                + BG_MAIN + ChatThumbChar(ChatScroll, ChatMaxScroll, BodyH, I) + ANSI_RESET);
        end
        else
    ...
    while Vis.Count < ContentHeight do
      Vis.Add(PadToWidth(BG_MAIN, BodyW, BG_MAIN) + ANSI_RESET + BG_MAIN + ' ' + ANSI_RESET);
...
var
  Lines : TList<string>; I, Div1, Div2, BodyW : Integer;
  ...
  procedure AddPlain(const S : string);
  begin
    Lines.Add(PadToWidth(S, BodyW, BG_PANEL) + ANSI_RESET + BG_PANEL + ' ' + ANSI_RESET);
  end;
...
begin
  ...
  try
    ...
    PlanBodyH := 0; WorkBodyH := 0; ThinkBodyH := 0;
    if not PlanCollapsed then
    begin PlanBodyH := RightPlanRows - 2; if PlanBodyH < 0 then PlanBodyH := 0; end;
    if not WorkCollapsed then
    begin WorkBodyH := RightWorkRows - 2; if WorkBodyH < 0 then WorkBodyH := 0; end;
    if not ThinkCollapsed then
    begin ThinkBodyH := RightThinkRows - 2; if ThinkBodyH < 0 then ThinkBodyH := 0; end;
    ...
    if PlanCollapsed then TitleText := TitleText + ' ' + GLYPH_COLLAPSED
    else TitleText := TitleText + ' ' + GLYPH_EXPANDED;
    AddPlain(BG_PANEL + FG_CYAN + ANSI_BOLD + TitleText);
    if not PlanCollapsed then
    begin
      AddPlain(BG_PANEL + FG_BORDER + ' ' + RepeatChar(GLYPH_HORZ, BodyW - 1));
    ...
    while Lines.Count < Div1 do AddPlain('');
    ...
    if WorkCollapsed then TitleText := TitleText + ' ' + GLYPH_COLLAPSED
    else TitleText := TitleText + ' ' + GLYPH_EXPANDED;
    AddPlain(BG_PANEL + FG_CYAN + ANSI_BOLD + TitleText);
    if not WorkCollapsed then
    begin
    ...
    while Lines.Count < Div2 do AddPlain('');
    ...
    if ThinkCollapsed then TitleText := TitleText + ' ' + GLYPH_COLLAPSED
    else TitleText := TitleText + ' ' + GLYPH_EXPANDED;
    AddPlain(BG_PANEL + FG_PURPLE + ANSI_BOLD + TitleText);
    ...
    { v0.42: exact ContentHeight — geen extra lege regel. }
    while Lines.Count > ContentHeight do Lines.Delete(Lines.Count - 1);
    while Lines.Count < ContentHeight do AddPlain('');
...
begin
  ...
  begin
    SepPos := Pos(' ', ParaText);
    if SepPos > 0 then
      Result := KeyColor + Copy(ParaText, 1, SepPos - 1)
              + TextColor + Copy(ParaText, SepPos, MaxInt)
              + ANSI_RESET + BG_BAR
    else Result := TextColor + ParaText + ANSI_RESET + BG_BAR;
  end;
end;
...
begin
  ...
  CloseStart := ScreenWidth - XW;
  AvailW := ScreenWidth - 2 - ClockW - XW - 2;
...
begin
  Result := 0;
  W := ExitDialogWidth; L := ExitDialogLeft; T := ExitDialogTop;
  PadL := 4;
...
begin
  ExitDialogVisible := False; HoverTarget := 0; NeedsRedraw := True;
end;
...
begin
  ...
  try
    ...
    AppendAt(SB, L, T, BG_PANEL + FG_BORDER + GLYPH_TOPLEFT
      + RepeatChar(GLYPH_HORZ, InnerW) + GLYPH_TOPRIGHT + ANSI_RESET);

    AppendAt(SB, L, T + 1,
      BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
      + PadToWidth(BG_PANEL + '  ' + FG_RED + ANSI_BOLD + 'Exit GRISP OS ?', InnerW, BG_PANEL)
      + ANSI_RESET + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);

    AppendAt(SB, L, T + 2, BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
      + Blank + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);
    AppendAt(SB, L, T + 3, BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
      + Blank + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);
    ...
    AppendAt(SB, L, T + 5, BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
      + Blank + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);

    AppendAt(SB, L, T + H - 1, BG_PANEL + FG_BORDER + GLYPH_BOTLEFT
...
begin
  ...
  try
    ...
    begin
      ...
      else SB.Append(PadToWidth('', ColCenter - 1, BG_MAIN) + ANSI_RESET + BG_MAIN + ' ' + ANSI_RESET);
      if ShowRightPanel then
      begin
    ...
    begin
      ...
      if VisRow >= InputScroll + InputHeight then InputScroll := VisRow - InputHeight + 1;
      if InputScroll < 0 then InputScroll := 0;
      if InputScroll > InputMaxScroll then InputScroll := InputMaxScroll;
    end;
    ...
    begin
      ...
      if SimActive and (I = 0) then
        ...
        InputPadded := PadToWidth(BG_INPUT + FG_TEXT + LineText, ScreenWidth - 3, BG_INPUT);

      SB.Append(FG_BORDER + GLYPH_VERT + ANSI_RESET
              + InputPadded + ANSI_RESET
              + BG_INPUT + ScrollThumbChar(InputScroll, InputMaxScroll, InputHeight, I) + ANSI_RESET
              + FG_BORDER + GLYPH_VERT + ANSI_RESET + NL);
    end;
    ...
    StatusLine := BG_BAR + ' ' + TitleLine + GLYPH_DOT + ANSI_RESET
                ...
                + IntToStr(ActiveSessionIndex + 1) + '/' + IntToStr(SessionList.Count)
                + ' ' + FG_DGRAY + GLYPH_VERT + ANSI_RESET + BG_BAR + ' '
...
{ v0.42: per-karakter iteratie — geen off-by-one meer. }
function ExtractSelectionText : string;
var
  R1, R2, C1, C2, R, K, StartC, EndC, Len : Integer;
  Plain : string;
  TmpI, TmpC : Integer;
begin
  Result := '';
  ...
  begin
    ...
    if R = R1 then StartC := C1 else StartC := 1;
    if R = R2 then EndC := C2 else EndC := Len;

    if StartC < 1 then StartC := 1;
    if EndC > Len then EndC := Len;

    if (Len > 0) and (StartC <= EndC) then
    begin
      for K := StartC to EndC do
        Result := Result + Plain[K];
    end;
...
var ChatTop, ChatBottom, VisRow, Len : Integer;
begin
  ...
  Len := Length(ChatFullPlain[FullRow]);
  if Len = 0 then begin Col := 0; Result := True; Exit; end;
  Col := X - ChatBodyLeftX + 1;
  if Col < 1 then Col := 1;
  if Col > Len then Col := Len;
  Result := True;
end;
...
begin
  ...
  PlanCollapsed := False; WorkCollapsed := False; ThinkCollapsed := False;
  ResCollapsed := False; SesCollapsed := False;
  RecomputeLayout;
end;
...
begin
  case ParaKeyIndex of
    ...
    3: begin ThinkCollapsed := not ThinkCollapsed; RecomputeLayout; end;
...
begin
  Result := 0;
  LH := LeftPaneBarX; RH := RightPaneBarX;
  ...
  if (X = ChatBarX) or (X = ChatBarX - 1) then
    if (Y >= 4 + 2) and (Y < 4 + ContentHeight) then
...
begin
  ...
  begin if ExitDialogButtonAt(x, y) <> 0 then Result := msHand; Exit; end;
...
begin
  case DragTarget of
    ...
    35: ThinkScroll := MouseToScrollTop(Ev.MouseY,
                     ...
                     RightThinkRows - 2, ThinkMaxScroll);
  end;
  RecomputeLayout;
end;
...
var
  WheelLines, Tgt : Integer;
begin
  if Ev.WheelDelta = 0 then Exit;

  { v0.43: Windows gebruikt normaal WHEEL_DELTA = 120, maar
    high-resolution wielen en trackpads sturen kleinere delta's.
    Zorg dat er altijd minstens 3 regels gescrold wordt. }
  if Abs(Ev.WheelDelta) >= 120 then
    WheelLines := (Ev.WheelDelta div 120) * 3
  else if Ev.WheelDelta > 0 then
    WheelLines := 3
  else
    WheelLines := -3;
  ...
  begin
    ...
    { Rechterpaneel }
      ...
      and (Ev.MouseX >= ColLeft + ColCenter + 4) then
    begin
    ...
    { Input }
    else if (Ev.MouseY >= 5 + ContentHeight)
      and (Ev.MouseY < 5 + ContentHeight + InputHeight) then
      Tgt := 32

    { Chat — volledige message-area, ongeacht scrollbar-positie }
    else if (Ev.MouseX >= ChatBodyLeftX)
      and (Ev.MouseX <= ChatBarX) then
      Tgt := 20;
  end;
  ...
  case Tgt of
    ...
    31: begin
          SesScroll := SesScroll - WheelLines;
          if SesScroll < 0 then SesScroll := 0;
          if SesScroll > SesMaxScroll then SesScroll := SesMaxScroll;
        end;
    32: begin
    ...
    35: begin
          ThinkScroll := ThinkScroll - WheelLines;
          if ThinkScroll < 0 then ThinkScroll := 0;
          if ThinkScroll > ThinkMaxScroll then ThinkScroll := ThinkMaxScroll;
        end;
  end;
...
function SectionTitleAt(X, Y : Integer) : Integer;
begin
  Result := 0;
  if ShowLeftPanel and (X >= 2) and (X <= ColLeft + 1) then
  begin
    if Y = 4 then begin Result := 40; Exit; end
    else if Y = 4 + LeftSplitRow + 1 then begin Result := 41; Exit; end;
  end;
  if ShowRightPanel and (X >= ColLeft + ColCenter + 4) then
  begin
    if Y = 4 then begin Result := 42; Exit; end
    else if Y = 4 + RightPlanRows + 1 then begin Result := 43; Exit; end
    else if Y = 4 + RightPlanRows + 1 + RightWorkRows + 1 then
    begin Result := 44; Exit; end;
  end;
end;

procedure HandleMouseEvent(const Ev : TInputEvent);
var Tgt, TopAction, DialogAction : Integer; HitRow, HitCol : Integer;
    IsDoubleClick : Boolean; SecID : Integer;
begin
  if ExitDialogVisible then
  begin
  ...
  if Ev.WheelDelta <> 0 then begin HandleWheel(Ev); Exit; end;
  ...
  if Ev.MouseButton = 1 then
  begin
    IsDoubleClick := (GetTickCount - LastClickTime < DOUBLE_CLICK_MS) and
                     (Abs(Ev.MouseX - LastClickX) <= 1) and
                     (Abs(Ev.MouseY - LastClickY) <= 1);
    LastClickTime := GetTickCount;
    LastClickX := Ev.MouseX;
    LastClickY := Ev.MouseY;

    if IsDoubleClick then
    begin
      SecID := SectionTitleAt(Ev.MouseX, Ev.MouseY);
      case SecID of
        40: ResCollapsed   := not ResCollapsed;
        41: SesCollapsed   := not SesCollapsed;
        42: PlanCollapsed  := not PlanCollapsed;
        43: WorkCollapsed  := not WorkCollapsed;
        44: ThinkCollapsed := not ThinkCollapsed;
      end;
      if SecID <> 0 then
      begin
        RecomputeLayout;
...
begin
  ...
  try
    ...
    { v0.43: één initiële render zodat ChatMaxScroll, InputMaxScroll
      en alle andere Scroll-variabelen geldig zijn voordat de eerste
      input-poll plaatsvindt. }
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin Dec(SelFocusRow);
            SelFocusCol := Length(ChatFullPlain[SelFocusRow]);
            if SelFocusCol < 1 then SelFocusCol := 1; end;
        end
        else
        begin
          ChatScroll := ChatScroll - 1;
          ...
          begin
            ...
            SelFocusCol := AutoScrollX - ChatBodyLeftX + 1;
            if SelFocusCol < 1 then SelFocusCol := 1;
