...
var
  ...
  InputSelectionActive : Boolean = False;
...
{ Geeft het teken dat op rij Row (0-based binnen body) van een scrollbar moet
  staan. TopConvention = True → Scroll telt vanaf bovenkant; anders vanaf
  onderkant (chat-stijl). }
...
begin
  ...
  S := Scroll;
  if S < 0 then S := 0;
  if S > MaxScroll then S := MaxScroll;
...
{ Voor chat die vanaf onderkant telt: converteer naar top-conventie. }
function ChatThumbChar(Scroll, MaxScroll, BodyH, Row : Integer) : string;
begin
  Result := ScrollThumbChar(MaxScroll - Scroll, MaxScroll, BodyH, Row);
end;

{ Zet een muispositie om naar een scroll-waarde (top-conventie). }
...
{ Idem voor chat (onderkant-conventie). }
...
begin
  ...
  if UserInputRows > 0 then InputHeight := UserInputRows else InputHeight := 1;
...
var
  ...
  Buf, PermColor, NameField, DividerCol, SessionTitle, MetaStr, SessTitleBuf : string;
...
begin
  ...
  try
    ...
    // Vooraf: resources-visible + max scroll
    ...
    // Vooraf: sessions voorbereiden als vlakke lijst van regels
    ...
    // --- RESOURCES header ---
    Lines.Add(PadToWidth(BG_PANEL + FG_CYAN + ANSI_BOLD + ' ' + GLYPH_FDIAMOND
      + ' RESOURCES', ContentW, BG_PANEL) + ANSI_RESET + BG_PANEL + ' ' + ANSI_RESET);
    ...
    // --- SESSIONS header ---
    Lines.Add(PadToWidth(BG_PANEL + FG_CYAN + ANSI_BOLD + ' ' + GLYPH_FDIAMOND
      + ' SESSIONS', ContentW, BG_PANEL) + ANSI_RESET + BG_PANEL + ' ' + ANSI_RESET);
    Lines.Add(PadToWidth(BG_PANEL + FG_BORDER + ' ' + RepeatChar(GLYPH_HORZ, ContentW - 1),
      ContentW, BG_PANEL) + ANSI_RESET + BG_PANEL + ' ' + ANSI_RESET);
    RowUsed := RowUsed + 2;
...
var
  ...
  begin
    Vis.Add(PadToWidth(S, BodyW, BG_MAIN) + ANSI_RESET
...
begin
  ...
  try
    ...
    begin
      if I < Full.Count - WS then Emit(Full[WS + I], I)
      else Emit(BG_MAIN, I);
    end;
...
var
  ...
  procedure AddLine(const S : string; BarRow : Integer;
                    const BarKind : Char);
  begin
    case BarKind of
...
begin
  ...
  try
    ...
    // --- CURRENT PLAN ---
    AddLine(BG_PANEL + FG_CYAN + ANSI_BOLD + ' ' + GLYPH_FDIAMOND + ' CURRENT PLAN', 0, 'X');
    ...
    begin
      AddLine(BG_PANEL, PlanRowUsed, 'P');
      Inc(PlanRowUsed);
    end;
    ...
    // --- WORK ITEMS ---
    AddLine(BG_PANEL + FG_CYAN + ANSI_BOLD + ' ' + GLYPH_FDIAMOND + ' WORK ITEMS', 0, 'X');
    AddLine(BG_PANEL + FG_BORDER + ' ' + RepeatChar(GLYPH_HORZ, BodyW - 1), 0, 'X');
    ...
    begin
      AddLine(BG_PANEL, WorkRowUsed, 'W');
      Inc(WorkRowUsed);
    end;
    ...
    if ShowThinking then
      AddLine(BG_PANEL + FG_PURPLE + ANSI_BOLD + ' ' + GLYPH_FDIAMOND
            + ' THINKING  ' + FG_DGRAY + '[F3]', 0, 'X')
    else
      AddLine(BG_PANEL + FG_DGRAY + ' ' + GLYPH_FDIAMOND
            + ' THINKING  [F3 - hidden]', 0, 'X');
    AddLine(BG_PANEL + FG_BORDER + ' ' + RepeatChar(GLYPH_HORZ, BodyW - 1), 0, 'X');
    ...
    while Lines.Count < ContentHeight do
    begin
      AddLine(BG_PANEL, ThinkRowUsed, 'T');
      Inc(ThinkRowUsed);
    end;
...
function TopBarSegmentText(ParaIndex: Integer; ParaCompact: Boolean): string;
begin
  case ParaIndex of
    1: if ParaCompact then Result := '[F1]L' else Result := '[F1]Left';
    2: if ParaCompact then Result := '[F2]R' else Result := '[F2]Right';
    3: if ParaCompact then Result := '[F3]T' else Result := '[F3]Think';
    4: Result := '[F4]Step';
    5: Result := '[F5]Sel';
    6: if ParaCompact then Result := '[F6]Rst' else Result := '[F6]Reset';
    else Result := '';
  end;
end;
...
begin
  case ParaIndex of
    1: Result := '[Ctrl+Enter]Send';
    2: Result := '[Ctrl+C]Copy';
    3: Result := '[Esc]Quit';
    else Result := '';
  end;
end;
...
begin
  if ParaHot then
  begin
    KeyColor := FG_CYAN + ANSI_BOLD;
    TextColor := FG_TEXT + ANSI_BOLD;
  end
  else
  begin
    KeyColor := FG_CYAN;
  ...
  ClosePos := Pos(']', ParaText);
  if ClosePos > 0 then
    Result := KeyColor + Copy(ParaText, 1, ClosePos)
            + TextColor + Copy(ParaText, ClosePos + 1, MaxInt)
            + ANSI_RESET + BG_BAR
  else
    Result := TextColor + ParaText + ANSI_RESET + BG_BAR;
end;

procedure GetTopBarLayout(
  out ParaTitlePlain: string;
  out ParaClockPlain: string;
  out ParaFKeys: TArray<string>;
  out ParaExtraKeys: TArray<string>;
  out ParaFKeyStart: Integer;
  out ParaExtraStart: Integer;
  out ParaCloseStart: Integer;
  out ParaShowExtra: Boolean;
  out ParaCompactFKeys: Boolean;
  out ParaCompactClock: Boolean);
var
  I: Integer;
  FKeyWidth, ExtraWidth: Integer;
  RightWidth, RightStart, LeftLimit: Integer;
  CandidateWidth: Integer;
begin
  SetLength(ParaFKeys, 6);
  for I := 1 to 6 do
    ParaFKeys[I - 1] := TopBarSegmentText(I, False);

  SetLength(ParaExtraKeys, 0);

  ParaCompactClock := ScreenWidth < 110;
  if ParaCompactClock then
    ParaClockPlain := ' ' + FormatDateTime('hh:nn', Now) + ' [X]'
  else
    ParaClockPlain := ' ' + CurrentLongTime + ' [X]';

  ParaTitlePlain := ' ' + GLYPH_DIAMOND + ' ' + APP_NAME + ' ' + APP_VERSION;
  if ScreenWidth < 100 then
    ParaTitlePlain := ' ' + GLYPH_DIAMOND + ' ' + APP_NAME;

  RightWidth := VisibleLength(ParaClockPlain);
  RightStart := ScreenWidth - RightWidth;
  ParaCloseStart := ScreenWidth - 3;

  ParaCompactFKeys := False;
  FKeyWidth := VisibleLength(ParaTitlePlain) + 1;
  for I := 0 to High(ParaFKeys) do
  begin
    if I > 0 then Inc(FKeyWidth);
    Inc(FKeyWidth, VisibleLength(ParaFKeys[I]));
  end;

  LeftLimit := RightStart - 2;
  if FKeyWidth > LeftLimit - 1 then
  begin
    ParaCompactFKeys := True;
    for I := 1 to 6 do
      ParaFKeys[I - 1] := TopBarSegmentText(I, True);
  ...
  ParaFKeyStart := 2 + VisibleLength(ParaTitlePlain);
  if ParaTitlePlain <> '' then Inc(ParaFKeyStart);

  ParaExtraStart := ParaFKeyStart + VisibleLength(ParaFKeys[0]);
  for I := 1 to High(ParaFKeys) do
    Inc(ParaExtraStart, 1 + VisibleLength(ParaFKeys[I]));
  Inc(ParaExtraStart, 3);

  ExtraWidth := VisibleLength(TopBarExtraText(1)) + 2
              + VisibleLength(TopBarExtraText(2)) + 2
              + VisibleLength(TopBarExtraText(3));
  CandidateWidth := ParaExtraStart + ExtraWidth + 1;
  ParaShowExtra := CandidateWidth < RightStart;

  if ParaShowExtra then
  begin
    SetLength(ParaExtraKeys, 3);
    for I := 1 to 3 do
      ParaExtraKeys[I - 1] := TopBarExtraText(I);
  end;
end;

function TopBarActionAt(ParaX, ParaY: Integer): Integer;
var
  TitlePlain, ClockPlain: string;
  FKeys, Extras: TArray<string>;
  FKeyStart, ExtraStart, CloseStart: Integer;
  ShowExtra, CompactFKeys, CompactClock: Boolean;
...
begin
  ...
  GetTopBarLayout(
    TitlePlain, ClockPlain, FKeys, Extras,
    FKeyStart, ExtraStart, CloseStart,
    ShowExtra, CompactFKeys, CompactClock);

  if (ParaX >= CloseStart) and (ParaX <= ScreenWidth - 1) then
  begin
    Result := 10;
    Exit;
  end;
...
begin
  Result := 58;
  if Result > ScreenWidth - 6 then Result := ScreenWidth - 6;
  if Result < 36 then Result := 36;
end;

function ExitDialogTop: Integer;
begin
  Result := (ScreenHeight - 9) div 2 + 1;
  if Result < 4 then Result := 4;
end;

function ExitDialogButtonAt(ParaX, ParaY: Integer): Integer;
var
  DialogWidth, DialogLeft, DialogTop, RelativeX: Integer;
begin
  Result := 0;
  DialogWidth := ExitDialogWidth;
  DialogLeft := (ScreenWidth - DialogWidth) div 2 + 1;
  DialogTop := ExitDialogTop;
  RelativeX := ParaX - DialogLeft + 1;

  if ParaY = DialogTop + 6 then
  begin
    if (RelativeX >= 5) and (RelativeX <= 14) then Result := 1
    else if (RelativeX >= 25) and (RelativeX <= 32) then Result := 2;
  end;
end;
...
procedure AppendAt(const SB: TStringBuilder; ParaX, ParaY: Integer; const ParaText: string);
begin
  SB.Append(ESC + '[' + IntToStr(ParaY) + ';' + IntToStr(ParaX) + 'H' + ParaText);
end;

procedure RenderExitDialog(const SB: TStringBuilder);
var
  W, H, L, T, InnerW, I: Integer;
  ContentRow, MessageRow, HintRow, ButtonRow: Integer;
  BlankLine, ButtonLine, CancelColor, QuitColor: string;
begin
  { Verberg het volledige frame voordat de popup wordt getekend. Hierdoor kan
    geen chat/paneeltekst tussen de popup-regels zichtbaar blijven. }
  SB.Append(SCREEN_CLEAR + CURSOR_HOME);
  BlankLine := BG_BAR + StringOfChar(' ', ScreenWidth) + ANSI_RESET;
  for I := 1 to ScreenHeight do
    AppendAt(SB, 1, I, BlankLine);

  W := ExitDialogWidth;
  H := 9;
  L := (ScreenWidth - W) div 2 + 1;
  T := ExitDialogTop;
  InnerW := W - 2;

  AppendAt(SB, L, T,
    BG_PANEL + FG_BORDER + GLYPH_TOPLEFT
    + RepeatChar(GLYPH_HORZ, InnerW)
    + GLYPH_TOPRIGHT + ANSI_RESET);

  AppendAt(SB, L, T + 1,
    BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
    + PadToWidth(BG_PANEL + FG_CYAN + ANSI_BOLD + '  EXIT GRISP OS', InnerW, BG_PANEL)
  ...
  AppendAt(SB, L, ContentRow,
    BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
    + PadToWidth(BG_PANEL + FG_TEXT + '  Close the GRISP terminal application?', InnerW, BG_PANEL)
    + ANSI_RESET + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);

  AppendAt(SB, L, MessageRow,
    BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
    + PadToWidth(BG_PANEL + FG_GRAY + '  All running activity will be stopped.', InnerW, BG_PANEL)
    + ANSI_RESET + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);

  AppendAt(SB, L, HintRow,
    BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
    + PadToWidth(BG_PANEL + FG_DGRAY + '  Enter/Y = Quit     Esc/N = Cancel', InnerW, BG_PANEL)
    + ANSI_RESET + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);

  if HoverTarget = 61 then CancelColor := FG_CYAN + ANSI_BOLD
  else CancelColor := FG_TEXT;
  if HoverTarget = 62 then QuitColor := FG_RED + ANSI_BOLD
  else QuitColor := FG_RED;

  ButtonLine :=
    BG_PANEL + '    ' + CancelColor + '[ CANCEL ]'
    + '          ' + QuitColor + '[ QUIT ]';
  AppendAt(SB, L, ButtonRow,
    BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
    + PadToWidth(ButtonLine, InnerW, BG_PANEL)
    + ANSI_RESET + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);

  AppendAt(SB, L, T + 7,
    BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET
    + PadToWidth(BG_PANEL + ' ', InnerW, BG_PANEL)
    + ANSI_RESET + BG_PANEL + FG_BORDER + GLYPH_VERT + ANSI_RESET);
...
var
  ...
  TitleLine, TopBar, SepTop, SepBot, StatusLine : string;
  MouseLabel, Hints, ClockString, CloseColor, SelectionLabel : string;
  TopBarFKeys, TopBarExtraKeys : TArray<string>;
  TopBarFKeyStart, TopBarExtraStart, TopBarCloseStart : Integer;
  TopBarShowExtra, TopBarCompactFKeys, TopBarCompactClock : Boolean;
  V1Hot, V2Hot, H10Hot : Boolean;
  SepC1, SepC2, SepBotAll, TJLeft, TJRight : string;
  LineText, InputPadded : string;
  InputBarX : Integer;
...
begin
  ...
  try
    ...
    GetTopBarLayout(
      TitleLine, ClockString, TopBarFKeys, TopBarExtraKeys,
      TopBarFKeyStart, TopBarExtraStart, TopBarCloseStart,
      TopBarShowExtra, TopBarCompactFKeys, TopBarCompactClock);

    TopBar := BG_BAR;
    if TitleLine <> '' then
      TopBar := TopBar + FG_CYAN + ANSI_BOLD + TitleLine + ANSI_RESET + BG_BAR;

    for I := 1 to 6 do
    begin
      if I > 1 then TopBar := TopBar + BG_BAR + ' ';
      TopBar := TopBar + StyleTopBarSegment(
        TopBarFKeys[I - 1], HoverTarget = 39 + I);
    end;

    if TopBarShowExtra then
    begin
      TopBar := TopBar + BG_BAR + '   ';
      for I := 1 to Length(TopBarExtraKeys) do
      begin
        if I > 1 then TopBar := TopBar + BG_BAR + '  ';
        TopBar := TopBar + StyleTopBarSegment(
          TopBarExtraKeys[I - 1], HoverTarget = 46 + I);
      end;
    end;

    TopBar := PadToWidth(TopBar, ScreenWidth - VisibleLength(ClockString) - 2, BG_BAR);

    if HoverTarget = 50 then
      CloseColor := FG_RED + ANSI_BOLD
    else
      CloseColor := FG_TEXT;

    if TopBarCompactClock then
      TopBar := TopBar + BG_BAR + FG_CYAN + ' ' + FormatDateTime('hh:nn', Now)
              + ' ' + CloseColor + '[X]'
    else
      TopBar := TopBar + BG_BAR + FG_CYAN + ' ' + CurrentLongTime
              + ' ' + CloseColor + '[X]';
    ...
    begin
      ...
      else if InputSelectionActive and (InputBuffer <> '') then
        InputPadded := PadToWidth(BG_USER + FG_TEXT + LineText,
          ScreenWidth - 3, BG_USER)
      else
    ...
    if InputSelectionActive then SelectionLabel := 'on' else SelectionLabel := 'off';
...
begin
  InputSelectionActive := False;
  if InputCursor > 0 then begin Delete(InputBuffer, InputCursor, 1); Dec(InputCursor); end; end;

procedure DeleteAtCaret;
begin
  InputSelectionActive := False;
  if InputCursor < Length(InputBuffer) then Delete(InputBuffer, InputCursor + 1, 1); end;
...
begin
  if InputSelectionActive and (InputBuffer <> '') then
    Result := InputBuffer
  else
    Result := InputBuffer;
end;
...
var
  ...
  SelectedText : string;
begin
  SelectedText := SelectedInputText;
  if SelectedText = '' then Exit;
  ByteCount := (Length(SelectedText) + 1) * SizeOf(WideChar);
  ...
  try
    Move(PWideChar(SelectedText)^, Ptr^, ByteCount);
  finally
    GlobalUnlock(Data);
  end;
  ...
  try
    EmptyClipboard;
    if SetClipboardData(MY_CF_UNICODETEXT, Data) = 0 then
      GlobalFree(Data);
  finally
    CloseClipboard;
  end;
...
{ Helper: X-positie van scrollbar van een sectie }
...
begin
  case ParaKeyIndex of
    ...
    5: begin
         ...
         begin
           InputSelectionActive := False;
           if not SimActive then PromptState := 'READY';
         end
         else
         begin
           InputSelectionActive := not InputSelectionActive;
           if InputSelectionActive then PromptState := 'SELECTED'
           else if not SimActive then PromptState := 'READY';
         end;
       end;
    6: ResetAllUserSizes;
  end;
...
{ Bepaal of (X, Y) op een scrollbar valt; geeft 30..35 of 0 }
...
begin
  ...
  // Input bar (Y >= 5+ContentHeight, binnen input-rijen)
...
begin
  ...
  if TopBarActionAt(MouseX, MouseY) > 0 then
    HoverTarget := 40 + TopBarActionAt(MouseX, MouseY)
...
begin
  case DragTarget of
    ...
    10: begin
          ...
          MyMax := MaxInputRows;
          if UserInputRows > MyMax then UserInputRows := MyMax;
        end;
    ...
    20: ChatScroll := MouseToChatScroll(Ev.MouseY, 4 + 2, ContentHeight - 2,
                     ChatMaxScroll);
    30: ResScroll := MouseToScrollTop(Ev.MouseY, 4 + 2, LeftSplitRow - 2,
                    ResMaxScroll);
    ...
    33: PlanScroll := MouseToScrollTop(Ev.MouseY, 4 + 2, RightPlanRows - 2,
                     PlanMaxScroll);
    ...
    35: ThinkScroll := MouseToScrollTop(Ev.MouseY,
                     ...
                     ContentHeight - 2 - RightPlanRows - RightWorkRows,
                     ThinkMaxScroll);
  end;
  RecomputeLayout;
end;
...
var
  WheelLines, Tgt, TopAction, DialogAction: Integer;
begin
  if ExitDialogVisible then
  begin
    HoverTarget := 0;
    DialogAction := ExitDialogButtonAt(Ev.MouseX, Ev.MouseY);
    if DialogAction <> 0 then HoverTarget := 60 + DialogAction;
    CurrentMouseShape := msArrow;
    if DialogAction <> 0 then CurrentMouseShape := msHand;
    if MouseEnabled then SetMouseShape(CurrentMouseShape);

    if (Ev.MouseButton = 1) and (DialogAction <> 0) then
    begin
      if DialogAction = 1 then CancelExit
      else ConfirmExit;
      NeedsRedraw := True;
    end;
    Exit;
  ...
  { De close-knop wordt bewust voor alle andere muislogica afgehandeld.
    Daardoor kan [X] nooit als splitter/drag-target worden geïnterpreteerd. }
  TopAction := TopBarActionAt(Ev.MouseX, Ev.MouseY);
  if (Ev.MouseButton = 1) and (TopAction = 10) then
  begin
    RequestExit;
    Exit;
  end;
  ...
  begin
    TopAction := TopBarActionAt(Ev.MouseX, Ev.MouseY);
    if (Ev.MouseButton = 1) and (TopAction <> 0) then
    begin
      if TopAction <= 6 then
        ExecuteFunctionKey(TopAction)
      ...
      else if (TopAction = 9) or (TopAction = 10) then
        RequestExit;
      NeedsRedraw := True;
      Exit;
    end;
  ...
  // --- Wiel: route naar de sectie onder de cursor ---
  if Ev.WheelDelta <> 0 then
  begin
    WheelLines := (Ev.WheelDelta div 120) * 3;
    if ShowLeftPanel and (Ev.MouseX >= 2) and (Ev.MouseX <= ColLeft + 1) then
    begin
      if Ev.MouseY < 4 + LeftSplitRow then
        ResScroll := ResScroll - WheelLines
      else SesScroll := SesScroll - WheelLines;
    end
    else if ShowRightPanel and (Ev.MouseX >= ColLeft + ColCenter + 4) then
    begin
      if Ev.MouseY < 4 + RightPlanRows then
        PlanScroll := PlanScroll - WheelLines
      else if Ev.MouseY < 4 + RightPlanRows + 1 + RightWorkRows then
        WorkScroll := WorkScroll - WheelLines
      else ThinkScroll := ThinkScroll - WheelLines;
    end
    else
    begin
      // Center pane
      if Ev.MouseY >= 5 + ContentHeight then
        InputScroll := InputScroll - WheelLines
      else
  ...
  begin
    ...
    begin
      IsDragging := True;
      DragTarget := Tgt;
      ApplyDrag(Ev);  // meteen naar geklikte positie springen
      NeedsRedraw := True;
    end
    ...
    else if ShowLeftPanel and (Ev.MouseX >= 2) and (Ev.MouseX <= ColLeft + 1) then
    begin
      // Klik op boom of sessie
    ...
    else if (Ev.MouseY >= 4) and (Ev.MouseY < 4 + ContentHeight) then
      ChatScroll := 0;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      case Ev.VK of
        VK_ESCAPE: CancelExit;
        VK_RETURN: ConfirmExit;
      end;
    end
...
begin
  ...
  try
    WriteAtomic(SCREEN_ALT_ON); WriteAtomic(ANSI_RESET + SCREEN_CLEAR);
    ...
    begin
      ...
      begin RecomputeLayout; ForceFullRedraw := True; NeedsRedraw := True; end;
        ...
        case Ev.Kind of
          ...
          ikMouse:
            ...
            begin HoverBefore := HoverTarget;
              HandleMouseEvent(Ev);
              if (HoverTarget <> HoverBefore) or Ev.IsMove then NeedsRedraw := True;
            end;
        end;
      until False;
      ...
      begin
        ...
        begin WriteAtomic(ANSI_RESET + SCREEN_CLEAR); ForceFullRedraw := False; end;
