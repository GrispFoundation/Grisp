...
type
  ...
  TInputEvent = record Kind: TInputKind; Ch: WideChar; VK: Word; end;

var
  hOut, hIn: THandle;
  ScreenW, ScreenH: Integer;
  // Layout (recomputed each frame in RecomputeLayout)
...
// Column layout rule:
//   [ S_V | ColL | S_V | ColC | S_V | ColR | S_V ]  == ScreenW
//   ColL + ColC + ColR + 4 == ScreenW
// We give the CHAT area all remaining space after the two side panels.
procedure RecomputeLayout;
begin
  ReadTermSize(ScreenW, ScreenH);

  // Auto-collapse side panels on very narrow terminals
  if ScreenW < 110 then ShowLeftPanel := False;
  if ScreenW < 78  then ShowRightPanel := False;

  // Side column widths
  if ShowLeftPanel then
    ColL := EnsureRange(ScreenW div 7, 22, 28)
  else
    ColL := 0;

  if ShowRightPanel then
    ColR := EnsureRange(ScreenW div 5, 30, 40)
  else
    ColR := 0;

  // Chat gets everything that's left (and it's always the largest column)
  ColC := ScreenW - ColL - ColR - 4;

  // If the chat column collapsed, drop side panels and try again
  if ColC < 30 then
  begin
    if ShowRightPanel then begin ShowRightPanel := False; RecomputeLayout; Exit; end;
    if ShowLeftPanel  then begin ShowLeftPanel  := False; RecomputeLayout; Exit; end;
    ColC := ScreenW - 4;
  end;
end;
...
// ═══════════════════════════════════════════════════════════════════
//  Center pane — padded to ColC (the biggest column)
// ═══════════════════════════════════════════════════════════════════
...
var
  ...
  Buf, SenderCol, Prefix, TimePart: string;
  RightPad: Integer;
  procedure AddLine(const S: string);
...
begin
  ...
  try
    ...
    AddLine(BG_MAIN + ' ' + FG_BORDER + Rep(S_H, ColC - 2));
    ...
    while L.Count < Height do
      L.Add(PadTo(BG_MAIN, ColC, BG_MAIN) + RST);
...
//  Frame assembly
//
//  Every row's total VISIBLE width is exactly ScreenW:
//    content row = 1 + ColL + 1 + ColC + 1 + ColR + 1 = ScreenW
//    separator   = 1 + ColL + 1 + ColC + 1 + ColR + 1 = ScreenW
//    top border  = 1 + (ScreenW - 2) + 1          = ScreenW
//
//  This is why ColL + ColC + ColR == ScreenW - 4.
// ═══════════════════════════════════════════════════════════════════
...
begin
  ...
  try
    ...
    if ShowLeftPanel and ShowRightPanel then
      SepTop := S_BW + Rep(S_H, ColL) + S_TE
              + Rep(S_H, ColC) + S_TE
              + Rep(S_H, ColR) + S_TW
    else if ShowRightPanel then
      SepTop := S_BW + Rep(S_H, ScreenW - ColR - 3) + S_TE
              + Rep(S_H, ColR) + S_TW
    ...
    if ShowLeftPanel and ShowRightPanel then
      SepBot := S_BW + Rep(S_H, ColL) + S_BE
              + Rep(S_H, ColC) + S_BE
              + Rep(S_H, ColR) + S_TW
    else if ShowRightPanel then
      SepBot := S_BW + Rep(S_H, ScreenW - ColR - 3) + S_BE
              + Rep(S_H, ColR) + S_TW
    else if ShowLeftPanel then
      SepBot := S_BW + Rep(S_H, ColL) + S_BE
              + Rep(S_H, ScreenW - ColL - 3) + S_TW
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        RecomputeLayout;
        WriteOut(CLRSCR);  // wipe stale content after resize
      end;
