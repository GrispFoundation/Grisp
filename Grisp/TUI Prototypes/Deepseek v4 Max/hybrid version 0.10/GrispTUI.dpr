{ ============================================================================
  GRISP OS — Terminal UI  ·  v0.20 (bugfix)
  ----------------------------------------------------------------------------
  Nieuw in v0.20
    • Volledige cyan-highlight op verticale én horizontale scheiders + T-joints
    • Adaptieve muis: resize-cursor op lijnen, I-beam in chat, hand op
      tree/sessions (best effort — Windows Terminal negeert SetCursor)
    • Boom-items klapbaar: klik op een ouder met kinderen om te toggelen
    • Sessies klikbaar: laadt de chat-geschiedenis van die sessie
    • Chat scrollbar rechts in het middenpaneel
    • Blauwe blok-glyph verwijderd; chat sluit strak aan op de randen
    • Militair tijd (24u) overal; am/pm weg
    • Sessie-titel toont "(24 sep 2026 14:30:23)" wanneer er ruimte is
    • Vlottere typing: alleen hertekenen bij wijziging + WaitForSingleObject
  ============================================================================ }
...
begin
  ...
  if (y >= 4) and (y < 4 + ContentHeight)
    and (x > ColLeft + 2)
    and (x < ColLeft + ColCenter + 3) then
    Result := msIBeam;
end;
