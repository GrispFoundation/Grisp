...
begin
  ...
  { v0.38: exact 1 spatie tussen alle segmenten. }
  ...
  { Title + 1 spatie + segments met 1 spatie ertussen. }
  ...
  { Screen col 2 is het begin van de content (na │). }
  ParaFKeyStart := 2 + VisibleLength(ParaTitlePlain);
  if ParaTitlePlain <> '' then Inc(ParaFKeyStart);   { 1 spatie tussen titel en F1 }
  ...
  Inc(ParaExtraStart, 1);   { 1 spatie tussen F6 en extras }

  ExtraWidth := VisibleLength(TopBarExtraText(1)) + 1
              + VisibleLength(TopBarExtraText(2)) + 1
...
begin
  ...
  begin
    ...
    XStart := XEnd + 2;   { 1 spatie tussen segments }
  end;
  ...
  begin
    ...
    begin
      ...
      XStart := XEnd + 2;   { 1 spatie }
    end;
  end;
end;

{ ============================================================================
  Exit dialog — compact en helder
  ============================================================================ }
...
{ Berekent welk van de drie exit-knoppen op (ParaX, ParaY) valt:
    1 = Yes, 2 = No, 3 = Cancel. 0 = geen. }
...
begin
  ...
  T := ExitDialogTop;
  InnerW := W - 2;
  { Button row: '    (Y/y)es    (N/n)o    (ESC/C/c/)ancel'
     4 leading spaces, dan 3 buttons, 4 spaces tussen elk. }
  PadL := 4;

  if ParaY <> T + 4 then Exit;
  RelX := ParaX - L - 1 - PadL;

  { (Y/y)es            : inner offset 0..6   (7 chars)
    (N/n)o             : inner offset 11..16 (6 chars)  [na 4 spaces]
    (ESC/C/c/)ancel    : inner offset 21..35 (15 chars) [na 4 spaces] }
...
var
  ...
  YesBtn, NoBtn, CancelBtn, ButtonRow : string;
...
begin
  ...
  try
    ...
    if YesHot then
      ...
      YesBtn := FG_GREEN + ANSI_BOLD + '(Y/y)es' + ANSI_RESET + BG_PANEL;

    if NoHot then
      NoBtn := BG_SELECT + FG_TEXT + ANSI_BOLD + '(N/n)o' + ANSI_RESET + BG_PANEL
    else
      NoBtn := FG_CYAN + ANSI_BOLD + '(N/n)o' + ANSI_RESET + BG_PANEL;

    if CancelHot then
      CancelBtn := BG_SELECT + FG_TEXT + ANSI_BOLD + '(ESC/C/c/)ancel' + ANSI_RESET + BG_PANEL
    else
      CancelBtn := FG_AMBER + ANSI_BOLD + '(ESC/C/c/)ancel' + ANSI_RESET + BG_PANEL;

    { 4 leading spaces + button + 4 spaces + button + 4 spaces + button }
  ...
  try
    ...
    if TitleLine <> '' then
      TopBar := TopBar + FG_CYAN + ANSI_BOLD + TitleLine + ANSI_RESET + BG_BAR
              + BG_BAR + ' ';   { 1 spatie tussen titel en F1 }
    ...
    begin
      TopBar := TopBar + BG_BAR + ' ';   { 1 spatie tussen F6 en extras }
...
{ v0.38: scrollbars hebben prioriteit. Als de cursor op een scrollbar staat
  scrollt het wiel die sectie, ongeacht X-positie-overlap. }
...
begin
  ...
  { Muiswiel — altijd als eerste, zodat scrollbars prioriteit hebben. }
  ...
  begin
    ...
    begin ApplyDrag(Ev); NeedsRedraw := True; end
    else begin IsDragging := False; DragTarget := 0; NeedsRedraw := True; end;
    Exit;
  end;

  { Selectie in uitvoering. }
  if IsSelecting then
  begin
    if Ev.MouseButton = 1 then
    begin
