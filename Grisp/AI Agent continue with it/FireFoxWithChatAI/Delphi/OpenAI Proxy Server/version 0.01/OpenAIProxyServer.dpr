program OpenAIProxyServer;

{$APPTYPE CONSOLE}

uses
  System.SysUtils,
  Unit_ProxyService in 'Unit_ProxyService.pas',
  Unit_ProxyServiceImpl in 'Unit_ProxyServiceImpl.pas',
...
begin
  try
    // 1. REST server – in‑memory, root 'root'
    Server := TRestServerFullMemory.CreateWithOwnModel([], True, 'root');
    try
      // 2. Create service instance
      Proxy := TOpenAIProxy.Create(FIREFOX_PORT);

      // 3. Register the instance (your mORMot2 version requires explicit cast)
      Server.ServiceRegister(
        TInterfacedObject(Proxy),
        [TypeInfo(IOpenAIProxy)],
        ''
      );

      // 5. Create HTTP server
      HttpServer := TRestHttpServer.Create(
        RawUtf8(IntToStr(HTTP_PORT)),
        [Server],
        '+',
        HTTP_DEFAULT_MODE,
        32
      ...
      try
        Writeln('OpenAI Proxy Server running on http://localhost:' + IntToStr(HTTP_PORT));
        Writeln('Endpoint: http://localhost:' + IntToStr(HTTP_PORT) +
                '/root/IOpenAIProxy/ChatCompletion');
        Writeln('Forwarding requests to Firefox extension on port ' + IntToStr(FIREFOX_PORT));
        Writeln;
        Writeln('Example curl (Windows CMD):');
        Writeln('curl -X POST "http://localhost:' + IntToStr(HTTP_PORT) +
                '/root/IOpenAIProxy/ChatCompletion" ^');
        Writeln('  -H "Content-Type: application/json" ^');
        Writeln('  -d "{\"model\":\"deepseek-chat\",\"messages\":[{\"role\":\"user\",\"content\":\"Hello\"}]}"');
        Writeln;
