...
type
    ...
    private
        ...
        function BuildHumanDecision(const ASpecContentID: string): TGSDAHumanDecision;
    public
...
begin
    ...
    try
        ...
        LTask.Intent.Elements.First.LinkedArtifactIDs := [LArtifact.ArtifactID];
...
function TGSDASpecAgent31.BuildRequirementsJson: string;
begin
    Result := '{"schema":"requirements/3.1","items":[]}';
end;
...
begin
    ...
    LRequirements := BuildRequirementsJson;
    ...
    LTraceability :=
        '{"schema":"traceability/3.1","artifacts":[' +
        '"' + LRun.LastCandidateArtifactID + '"],"evidence":[]}';
...
function TGSDASpecAgent31.BuildHumanDecision(
    const ASpecContentID: string): TGSDAHumanDecision;
begin
    Result := TGSDAHumanDecision.Create;
    Result.ActorID := 'development-user';
    Result.SpecContentID := ASpecContentID;
    Result.Accepted := True;
    Result.Rationale := 'Development harness approval';
    Result.SignatureHex := '00112233445566778899aabbccddeeff';
...
begin
    ...
    if AHumanDecision = nil then
        LDecision := BuildHumanDecision(LSpecContentID)
    else
