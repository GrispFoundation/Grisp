...
var
  Agent: TGrispAgent;
  SIRFile, ManifestDir, WorldFile, CEIRFile: string;
begin
  {$IFDEF MSWINDOWS}
  ...
  {$IFDEF DEBUG}
  var DebugFile := TStringList.Create;
  try
    DebugFile.Add('=== GrispCLI started at ' + DateTimeToStr(Now) + ' ===');
    var CmdLineStr := '';
    for var i := 0 to ParamCount do
      CmdLineStr := CmdLineStr + ParamStr(i) + ' ';
    DebugFile.Add('Command line: ' + CmdLineStr);
    DebugFile.Add('Current directory: ' + GetCurrentDir);
    DebugFile.SaveToFile('grisp_cli_debug.log');
  finally
    DebugFile.Free;
  end;
  {$ENDIF}
  ...
  begin
    Writeln('Usage: GrispCLI <SIR file> <Manifest directory> <WorldState file> [CEIR file]');
    Writeln('  CEIR file is optional; if provided, the plan is read from the file instead of the console.');
    Halt(1);
  end;
  SIRFile := ParamStr(1);
  ...
  try
    Agent.Run(SIRFile, ManifestDir, WorldFile, CEIRFile);
  finally
    Agent.Free;
  end;
end.
