  ...
  async executeCommand(cmd) {
    try {
      switch (cmd.command) {
        ...
        case "prompt":
          return AIAutomationUtils.handlePrompt(
            cmd.site,
            cmd.text,
            cmd.tab ?? -1
          );
...
const AIAutomationUtils = {
  ...
  async handlePrompt(site, text, tabSpec = -1) {
    const urls = {
    ...
    try {
      ...
      if (tab) {
        ...
        dump("AIAutomation: Opening new tab for " + site + "\n");
      ...
      dump("AIAutomation: Initializing smart polling for " + site + "...\n");
      await AIAutomationUtils.waitForReady(browser, site, win);
      ...
      if (
        ...
        return {
          error: "Please sign in to " + site + " manually in the browser first.",
        };
      }

      // --- Phase 2: Inject the prompt (actor returns immediately after send) ---
      dump("AIAutomation: Contacting actor for " + site + "\n");
      let actor = AIAutomationUtils.getActor(browser);
      if (!actor) {
        throw new Error("Could not find AIAutomation actor");
      }

      let injectResult = await actor.sendQuery("AIAutomation:Inject", {
        prompt: text,
        site,
      });
      ...
      dump("AIAutomation: Polling for response on " + site + "...\n");
      let result = await AIAutomationUtils.pollForResponse(browser, site, win, text);
   ...
   *
   * Strategy:
   *  - Wait up to 5s for the page to settle after submission (navigation).
   *  - Then poll FetchResponse every 500ms.
   *  - When we see stable text (unchanged for 3s) AND done=true, resolve.
   *  - Timeout after 120s.
   */
  pollForResponse(browser, site, win, prompt) {
    return new Promise((resolve, reject) => {
      // Settle delay before polling starts.
      // SPA sites (Gemini, ChatGPT) navigate to a new URL after submit, so
      // we need ~3s for the navigation + new actor to settle.
      // Non-SPA sites (Copilot, Claude, Grok, etc.) respond in-place — 1s is enough.
      const SPA_SITES = ["gemini", "chatgpt", "perplexity", "inception"];
      const settleMs = SPA_SITES.includes(site) ? 3000 : 1000;
