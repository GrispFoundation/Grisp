...
begin
  ...
  begin
    ...
    try
      try
        Rules := Parser.Parse;
        Diagnostics := Format('Plan parsed successfully with %d rule(s)', [Length(Rules)]);
        Result := True;
      except
        on E: Exception do
        begin
          Diagnostics := 'Plan syntax error: ' + E.Message;
          Result := False;
        end;
      end;
    finally
  ...
  begin
    // Freeform code plan
    Diagnostics := 'Freeform code plan accepted';
    Result := True;
  end;
end;
