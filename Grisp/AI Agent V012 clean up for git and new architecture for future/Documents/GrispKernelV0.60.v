...
Module SOS.
  ...
  Definition match_lt_bool (m1 m2 : Match.t) : bool :=
    match Z.compare (score m1 0 1 1) (score m2 0 1 1) with
    | Lt => true
    | Eq =>
        match compare_match_key m1 m2 with
        | Lt => true
        | _  => false
        end
    | Gt => false
  ...
  Fixpoint insert_match (m : Match.t) (l : list Match.t) : list Match.t :=
    match l with
    | [] => [m]
    | h :: t =>
        if match_lt_bool m h
        then m :: l
        else h :: insert_match m t
    end.

  Fixpoint sort_matches (l : list Match.t) : list Match.t :=
    match l with
    | [] => []
    | h :: t => insert_match h (sort_matches t)
    end.
  ...
  Definition select (cfg : Config.t) (M : list Match.t) : select_result :=
    ...
    | [] => NoMatch
    | _  =>
        let sorted := sort_matches M_rem in
  ...
  Definition no_match (cfg : Config.t) : Config.t :=
    let oldC := cfg.(Config.C) in
    {| Config.G := cfg.(Config.G);
       Config.C := {| Counters.tick_counter := (oldC.(Counters.tick_counter) + 1)%Z;
                      Counters.seq_counter := oldC.(Counters.seq_counter);
                      Counters.type_extent_version := oldC.(Counters.type_extent_version);
                      Counters.element_version := oldC.(Counters.element_version);
                      Counters.adjacency_version := oldC.(Counters.adjacency_version) |};
  ...
  Definition delta (cfg : Config.t) (ir : IRImpl.t) : Config.t :=
    ...
    let fix loop (retries : nat)
      ...
      match retries with
      | O => no_match current_cfg
      | S retries' =>
          let select_cfg :=
            {| Config.G := current_cfg.(Config.G);
               Config.C := current_cfg.(Config.C);
               Config.M_prev := current_cfg.(Config.M_prev);
               Config.Rejected := rejected |} in

          match select select_cfg M with
          | NoMatch =>
              no_match current_cfg

          | Selected m =>
              match plan current_cfg.(Config.G) m with

              | None =>
                  loop retries' current_cfg (add m.(Match.key) tt rejected)
    ...
    in loop (length M) cfg_after_discover empty.
...
Module DeterminismProofs.
  ...
  Proof.
    intros cfg ir.
    (* 1. get existence from your termination lemma *)
    destruct (loop_termination cfg ir) as [cfg' Hdelta].
    (* 2. use it as the unique witness *)
    exists cfg'. split.
      - exact Hdelta.
      - intros cfg'' Hdelta'.
      eapply delta_functional; eauto.
  Qed.
